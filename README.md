...
## 지금까지 만들어진 것 (게임 진행 상황)

`docs/DESIGN.md`/`docs/STATUS.md`/`docs/feedback/INBOX.md`가 권위 있는 최신 상태다 —
아래는 감을 잡기 위한 요약이라 시간이 지나면 낡을 수 있다.

- **기반**: 메인메뉴/캐릭터 슬롯/커스터마이징, WASD 이동+마우스 조준, 해상도 설정,
  일시정지 메뉴.
- **생존 루프**: 사냥(총+기본탄/마취탄, 탄창/재장전), 채집·채광(곡괭이낫), 농사, 목장,
  낮/밤·계절·강수 시스템.
- **멀티플레이**: 방 코드 발급/참가, 같은 로컬 네트워크 안에서 동기화(ENet).
- **인벤토리/도구**: 슬롯 기반 인벤토리(일반 18 + 장비 9), 드래그앤드롭, 도구별
  들기/사용 애니메이션(AnimatedSprite2D, PixelLab).
- **아이템/제작 테크**: 원재료 → 가공물 → 완성품, 식재료 → 가공식품 → 요리까지
  전체 트리 구축. 제작은 수량 지정 + 타이머 + 제작대 출력 버퍼 + 수동 수령 방식.
- **건축/방(Room) 시스템**: 격자 기반 벽/문 배치, flood-fill 방 감지, 방 종류별 규칙
  (주방/제작소/대장간/연구소/농장/목장), 방-상자 자동 연동(제작대가 같은 방 상자
  재료를 자동 사용).
- **QA**: 마지막 전체 스윕(#124) 결함 0건, 알려진 버그 없음.

**아직 없는 것**: 캐릭터 그림체 리메이크(보류 중), 연구소/표본 시스템, 전기/기계 테크의
실제 기능(배터리/조명/발전기는 아이템으로만 존재), 방 이름 UI 표시.
...
## 지금 상태 (2026-09-05 기준, 바퀴 177)

- `origin`이 `https://github.com/kimtuna/life_game.git`로 연결되어 있고, GitHub Pages
  (`https://kimtuna.github.io/life_game/`)도 켜져 있다. launchd 등록/가동 중.
- `docs/feedback/INBOX.md`의 "처리 대기"가 비어 있다 — 마지막 QA 전체 스윕(#124)이
  결함 0건으로 끝나서 재확인 스윕도 다시 큐에 넣지 않았다. 즉 지금 `loop/ctl.sh start`를
  해도 세션 없이 바로 종료한다(정상). **새 INBOX 항목을 추가해야 다음 바퀴가 시작된다.**
- 사람과의 인터랙티브 세션이 다음에 이어갈 때는 `SESSION_HANDOFF.md`를 먼저 읽고,
  그다음 `docs/DESIGN.md`/`docs/STATUS.md`로 사실관계를 확인할 것.
