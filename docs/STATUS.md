...
- INBOX #11 완료: 농사(밭 심기 → 성장 → 수확)를 새로 만들었다.
  - `game/scenes/farm_plot/farm_plot.gd` + `farm_plot.tscn`: 밭 한 칸을 나타내는
    새 씬. `resource_point.gd`와 같은 F키 상호작용 패턴을 쓰되, 상태가
    EMPTY → GROWING → READY → (수확 후) EMPTY로 순환하는 상태머신이라는 점이 다르다.
    EMPTY 상태에서는 `InventoryData`에 `rice_seed`가 있을 때만 "F: 씨앗 심기"
    프롬프트가 뜨고, F를 누르면 씨앗 1개를 소비하고 GROWING으로 전환(성장 타이머
    60초 시작, 상호작용 불가/프롬프트 숨김). 타이머가 끝나면 READY로 자동 전환되고
    "F: 수확" 프롬프트가 뜨며, F를 누르면 `rice` 2개를 인벤토리에 넣고 다시 EMPTY로
    돌아간다(채집 포인트처럼 "재사용 가능한" 순환 구조 — 아래 결정 로그 참고).
  - `InventoryData`(`game/scripts/inventory_data.gd`)에 `has_item()`과
    `remove_item()`을 추가했다(기존엔 `add_item`만 있었음) — 씨앗을 심을 때
    보유량을 확인하고 차감하는 데 필요.
  - `world.gd`에 `_spawn_farm_plots()`를 추가해, 채집/채광 포인트처럼 필드에
    무작위로 흩뿌리지 않고 플레이어 스폰 지점 기준 고정 오프셋에 3x2 격자(6칸)로
    배치했다 — DESIGN.md "밭(정해진 구역)" 요구를 "매 게임 항상 같은 자리"로
    해석함. HUD 인벤토리 라벨의 `ITEM_LABELS`에 `rice: "벼"`를 추가해 수확물이
    표시되게 했다.
  - 그림은 PixelLab로 3장 생성했다: 밭 갈이 흙(`soil.png`, 배경 있음 — 타일이라
    불투명해야 자연스러움), 벼 새싹(`rice_sprout.png`, 배경 투명), 다 자란 벼
    (`rice_grown.png`, 배경 투명). 다 자란 벼는 처음 생성한 버전이 옥수수처럼
    보여서("드리워진 황금 벼 이삭" 프롬프트로) 한 번 더 생성해 더 벼답게 보이는
    쪽으로 교체했다.
  - 검증: `godot --headless --script <임시스크립트>.gd`로 밭 한 칸을 인스턴스화해
    EMPTY(씨앗 없으면 프롬프트 숨김) → 씨앗 지급 → F입력 → GROWING(씨앗 소모,
    프롬프트 숨김) → 타이머 강제 만료 → READY(프롬프트 표시) → F입력 → EMPTY
    복귀 + 인벤토리에 `rice` 2개 확인, 전 과정을 `assert`로 통과시켰다. 이어서
    `godot --script <임시스크립트>.gd`(헤드리스 아님)로 world.tscn을 띄우고 밭
    격자 중 한 칸은 GROWING, 한 칸은 READY로 만든 뒤 실제 렌더링을 PNG로 캡처해
    눈으로 확인했다.

## 스스로 판단해서 고친 부분 (바퀴 13, 코드 아님 — 검증 중 발견한 버그 수정)

- 첫 스크린샷에서 밭 타일(Soil 스프라이트)이 플레이어보다 나중에 씬 트리에
  추가되어(월드가 `_ready()`에서 프로그램적으로 스폰) 플레이어 다리가 흙 타일에
  가려 잘려 보이는 렌더링 버그를 발견했다. `Soil` 스프라이트에 `z_index = -1`을
  줘서 항상 플레이어보다 아래에 그려지도록 고쳤다(같은 씬 안의 `Crop` 스프라이트는
  기본 z_index를 유지 — Soil보다는 위, 문제가 재현되지 않아 추가로 손대지 않음).
  이건 INBOX #11 지시엔 없던 내용이지만 PROMPT.md ①의 합격 기준(다른 사람이 봤을
  때 퀄리티가 떨어지지 않는가)을 스크린샷으로 직접 확인하다가 발견해서 그 자리에서
  고쳤다 — 이 기준 자체가 "발견하면 고치고 커밋 전에 통과시켜라"는 뜻이라고 판단.

- INBOX #10 완료: 채집/채광과 인벤토리 시스템을 새로 만들었다.
  - `scripts/inventory_data.gd`를 새 오토로드(`InventoryData`)로 추가했다.
    `CharacterData`/`SettingsData`와 같은 패턴(딕셔너리 상태 + `user://inventory.save`
    JSON 저장/로드, 변경 시 `changed` 신호 emit)이다.
  - `scenes/resource_point/resource_point.gd` 하나를 채집 포인트/채광 포인트가
    공유한다. DESIGN.md의 "삽/곡괭이를 한 세트 도구로 다룬다"는 요구를, 포인트
    종류(아이템 이름)에 따라 자동으로 맞는 판정을 적용하고 도구 선택 UI를 따로
    두지 않는 방식으로 구현했다(바퀴 11이 STATUS.md에 남긴 해석을 그대로 이어받음).
    `gathering_point.tscn`(벼 씨앗, "F: 채집")과 `mining_point.tscn`(철,
    "F: 채광")은 `@export`로 아이템 종류/프롬프트 문구/텍스처 경로만 다르게 준
    같은 씬이다.
  - 상호작용: 플레이어가 반경 70 안에 들어오면 포인트 위에 "F: ..." 프롬프트가
    뜨고, F키를 누르면 `InventoryData.add_item`으로 아이템이 들어간 뒤 포인트는
    20초 동안 회색(dim)으로 표시되며 재상호작용이 막힌다(회수 후 재생성 방식).
    사슴처럼 완전히 사라지지 않고 20초 뒤 원래 색으로 돌아와 다시 캘 수 있다 —
    자원이 유한 소모되면 초반에 다 캐버린 뒤 할 게 없어지는 문제를 피하기 위한
    선택(밸런스는 나중에 조정 가능).
  - `world.gd`가 사슴과 같은 패턴(`player_ref` 주입, 스폰 반경 안에서 플레이어와
    최소 거리를 두고 배치)으로 채집 포인트 5개 + 채광 포인트 5개를 필드에 뿌린다.
    HUD에 `AmmoPanel` 아래 `InventoryPanel`을 추가해 보유 아이템(벼 씨앗/철) 수량을
    실시간으로 보여준다(`InventoryData.changed` 신호 구독).
  - 그림은 PixelLab로 생성했다(`PIXELLAB_API_KEY` 환경변수 사용, `/v1/generate-image-pixflux`
    엔드포인트, 64x64, `no_background=true`). 채집 포인트는 벼 이삭 다발, 채광
    포인트는 철맥이 비치는 회색 바위로 생성해서 채택했다 — 기존 사슴/캐릭터
    스프라이트와 같은 도트 톤으로 잘 어울림을 스크린샷으로 확인했다.
  - 검증: `godot --headless --path . --script <임시스크립트>.gd`로 world.tscn을
    인스턴스화해 포인트 5+5개가 정확히 스폰되는지, F키 입력(`Input.parse_input_event`)
    시 인벤토리 카운트가 실제로 늘어나는지 확인했다. 이어서 `godot --path . --script
    <임시스크립트>.gd`(헤드리스 아님, 바퀴 2/8 방식)로 플레이어를 포인트 사이에
    세운 뒤 실제 렌더링 결과를 PNG로 캡처해서 프롬프트 라벨 가독성과 스프라이트
    조화를 눈으로 확인했다. 임시 스크립트와 캡처 이미지는 게임 코드가 아니므로
    커밋 전에 삭제했다.
  - 새 PNG는 아직 Godot 에디터가 import를 실행한 적이 없어서 `.import` 파일이
    없었고, 처음 헤드리스 스크립트 실행 시 "No loader found for resource" 에러가
    났다 — `godot --headless --editor --path . --quit-after 3`로 에디터를 한 번
    띄워 강제로 (재)import를 돌려서 해결했다(다음 바퀴가 새 PNG를 추가할 때 같은
    에러를 보면 이 명령으로 먼저 import부터 시킬 것).

## 다음에 할 것

- `docs/feedback/INBOX.md` "처리 대기"의 다음 항목은 **#12(목장)**이다. #9에서
  마취탄으로 포획한 사슴을 목장 구역에 배치해서 기를 수 있어야 한다(도망가지 않고
  구역 안에 머무름). 아직 "포획된 사슴을 어떻게 보관/선택해서 목장에 배치하는지"
  UI/흐름이 정해져 있지 않다 — `deer.gd`의 포획 처리 결과가 지금 뭘 하는지부터
  다시 확인하고 시작할 것(사망과 동일하게 그냥 사라지는지, 별도 상태로 남는지).
  목장 구역도 이번 바퀴의 밭과 같은 패턴(고정 오프셋 격자/구역, `world.gd`에서
  스폰)으로 만들면 일관적일 것.
...
- 성장 시간은 60초로 정했다(임의 — DESIGN.md 미정). 낮/밤 시스템(#13)이 아직 없어서
  "며칠" 단위 성장은 못 만들었다 — #13이 구현되면 성장 시간을 게임 내 날짜 기준으로
  바꿀지 재검토할 것(지금은 실시간 초 단위).
