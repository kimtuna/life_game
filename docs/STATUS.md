...
- **(2026-09-04 갱신, 바퀴 138) INBOX #85([BUILD] 사슴을 기본탄으로 사살하면 고기 드롭)
  완료.** 위 "마지막 갱신"/"끝난 것" 참고. **다음 `[BUILD]` 항목은 `#87`**(가공대 —
  #86은 [DESIGN] 태그라 이 하네스가 처리하지 않음). `[DESIGN]` 큐는 `#86`(대기, 이번에
  `#85`가 끝났으니 착수 가능 — 돌/유황광석/고기 아이콘). `[QA]` 큐는 비어 있다
  (BUILD/DESIGN 완료 카운터 4, 5 도달까지 1개 더 필요 — 다음 BUILD/DESIGN 완료 시
  자동으로 QA 전체 스윕 항목이 큐에 추가됨). **참고**: `meat` 아이템은 이번 항목 지시대로
  전용 아이콘 없이 추가했다 — `dropped_item.gd`의 `ITEM_ICONS`에 `meat` 키가 없어서
  바닥에 떨어진 고기는 스프라이트 없이 완전히 투명하게 보인다(인벤토리/핫바 UI는 텍스트
  라벨이라 "고기 x2"처럼 정상 표시됨, 스크린샷 `/tmp/qa85/meat_drop.png`로 확인). 다음
  `[DESIGN]` 바퀴(`#86`)가 새 PNG를 저장한 뒤에는 반드시
  ...
  있어 원인을 코드 버그로 오인하기 쉽다(바퀴 135/136이 각각 겪음, 아래 "헤드리스
  CLI 검증" 절 참고).
