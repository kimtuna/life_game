...
- **CanvasLayer를 새로 추가할 때는 `layer` 속성을 기본값(1)에 맡기지 말고 명시적으로
  지정할 것 (바퀴 120, INBOX #68).** 이 프로젝트의 렌더링 설정(Godot 4.7.2,
  `gl_compatibility`)에서는 `layer` 기본값 1로는 CanvasLayer가 월드 콘텐츠(기본 캔버스)
  보다 확실히 위에 그려지지 않는 것을 실제로 재현해 확인했다 — `world.tscn`의 `UI`
  CanvasLayer에 `layer = 10`을 명시해서 고쳤다. 앞으로 새 CanvasLayer(예: 전역 알림,
