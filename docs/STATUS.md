...
- **(바퀴 70 재확인, 최우선) INBOX #50(green 캐릭터 idle/walk, SpriteCook)을 이어서
  진행하기 전에 `GET /v1/api/credits`로 SpriteCook 잔액부터 확인할 것.** 바퀴 70 종료
  시점에도 여전히 8크레딧(자연 회복 없음) — 관측된 생성 호출 최소 비용(20크레딧)보다
  적어 아무 생성도 못한다. 잔액이 회복되지 않았다면 무리하게 절반만 만들다 죽지 말고,
  이 사실만 다시 확인해서 짧게 기록하고 종료할 것(바퀴 58~68이 PixelLab에 대해 했던
  것과 같은 패턴).
