...
- **(2026-09-04 갱신, 바퀴 142) INBOX #89([BUILD] 화약/탄약 레시피) 완료.** 위
  "마지막 갱신" 참고. **다음 `[BUILD]` 항목은 `#90`**(조리대+조리용 화로, #87/#88과
  같은 프레임워크를 `scenes/cooking_table/`·`scenes/cooking_stove/`로 새로 만들어야
  함 — #89와 달리 새 오브젝트 2개가 필요. 레시피: 벼 1→밥 1, 고기 1→익힌고기 1).
  **다음 `[QA]` 항목은 `#92`**(전체 스윕, 바퀴 139에서 이미 큐에 등록돼 대기 중 —
  별도 QA 하네스 세션 몫이라 이번 바퀴도 건드리지 않았다). **BUILD/DESIGN 완료
  카운터는 바퀴 139(#86)에서 5에 도달해 0으로 리셋된 뒤 140(#87)=1 → 141(#88)=2 →
  이번(#89)=3이 됐다** — 다음 QA 스윕 자동 등록까지 2개 더 필요(`#90`, `#91` 완료
  시 5). `[DESIGN]` 큐 `#91`(가공대/제련로/조리대/조리용 화로 4개 오브젝트 그림 +
  가공물/완성품/가공식품 아이콘 일괄 제작)은 여전히 `#90`이 먼저 끝나야 의미가 있어
  대기. **참고**: 화약/탄약도 철/숯과 마찬가지로 아직 전용 아이콘이 없어 핫바/인벤토리에
  텍스트로만 표시된다 — 결함이 아니라 `#91`이 처리할 범위(INBOX 원문이 명시).
