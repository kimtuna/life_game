...
- **(2026-09-04 갱신, 바퀴 137) INBOX #84([BUILD] 채광 포인트 돌/철광석/유황광석
  확률 스폰) 완료.** 위 "마지막 갱신"/"끝난 것" 참고. **다음 `[BUILD]` 항목은 `#85`**
  (사슴을 기본탄으로 사살하면 고기 드롭). `[DESIGN]` 큐는 `#86`(대기, `#84`/`#85` 완료
  후 진행 — 이번에 `#84`가 끝났으니 `#85`만 끝나면 착수 가능), `[QA]` 큐는 비어 있다
  (BUILD/DESIGN 완료 카운터 3, 5 도달까지 2개 더 필요). **다음 BUILD/DESIGN 바퀴
  필독**: 새 PNG(아이콘 등)를 저장한 뒤 게임에서 확인하기 전에 반드시
  ...
  CLI 검증" 절 참고). (바퀴 137은 새 PNG를 만들지 않고 기존 `mining_point.png`를
  `modulate`로만 재사용해서 이 함정을 겪지 않았다 — `#86`이 실제 새 PNG를 도입할
  때는 이 절차를 반드시 지킬 것.)
