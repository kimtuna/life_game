...
바퀴 71 / 2026-09-03 (**INBOX #50 착수 보류 재확인.** `GET /v1/api/credits` 재호출 결과
여전히 `{"total":8,"subscription_credits":8,"topup_credits":0,"tier":"free"}` — 바퀴 70
종료 시점과 정확히 동일(자연 회복 없음). 참고로 `GET /v1/balance`(PixelLab)도 함께
재확인했으나 여전히 `{"type":"usd","usd":0.0}` — 두 도구 모두 막혀 있다. 바퀴 69/70이
남긴 지시대로 재조사 없이 잔액 확인만 하고 바로 기록·종료했다. 게임 코드/에셋은 전혀
건드리지 않았다(`git status` 변경 없음). INBOX #50은 `- [ ]`로 그대로 둔다.
...
한 방향씩, 프레임별 발 위치 명시, 즉시 크롭+확대 검사)를 그대로 따라 진행할 것.

바퀴 70 / 2026-09-03 (**INBOX #50 착수 보류 재확인.** `GET /v1/api/credits` 재호출 결과
여전히 `{"total":8,"subscription_credits":8,"topup_credits":0,"tier":"free"}` — 바퀴 69
종료 시점과 정확히 동일(자연 회복 없음). 관측된 생성 호출 최소 비용(20크레딧)보다 적어
`animate-sync`든 무엇이든 어떤 생성 호출도 못 한다. 바퀴 69가 이미 남긴 "SpriteCook API
실측 조사" 절(아래)이 이미 충분히 상세해서 재조사하지 않고 잔액 확인 한 번만 하고 바로
기록·종료했다(PixelLab 잔액이 $0이었을 때 바퀴 58~68이 썼던 것과 같은 패턴). 게임
코드/에셋은 전혀 건드리지 않았다(`git status` 변경 없음). INBOX #50은 `- [ ]`로 그대로
둔다.
**다음 바퀴가 참고할 것**: 잔액이 회복됐는지 `GET /v1/api/credits`로 먼저 확인하고,
8~19 사이라면 여전히 생성 불가이니 이 문단처럼 짧게 기록만 하고 종료할 것. 20 이상으로
회복되면 아래 "SpriteCook API 실측 조사" 절의 권장 경로(② `animate-sync`, south부터
...
- **(바퀴 71 재확인, 최우선) INBOX #50(green 캐릭터 idle/walk, SpriteCook)을 이어서
  진행하기 전에 `GET /v1/api/credits`로 SpriteCook 잔액부터 확인할 것.** 바퀴 71 종료
  시점에도 여전히 8크레딧(자연 회복 없음, PixelLab도 여전히 $0) — 관측된 생성 호출
  최소 비용(20크레딧)보다 적어 아무 생성도 못한다. 잔액이 회복되지 않았다면 무리하게
  절반만 만들다 죽지 말고, 이 사실만 다시 확인해서 짧게 기록하고 종료할 것(바퀴
  58~68이 PixelLab에 대해 했던 것과 같은 패턴).
- **(바퀴 69 신규) 잔액이 회복되면, 아래 "SpriteCook API 실측 조사" 절을 먼저 전부
  읽고 시작할 것 — 같은 시행착오(특히 "함정" 항목)를 반복하면 크레딧만 또 날린다.**
  요약 권장 경로: ①공식 `/v1/api/characters/{id}/animations`(topdown 워크플로우)는
  방향당 20+12(prep)크레딧이라 무료 티어로는 감당 불가 — 대신 ②기존 정지 이미지를
  `/v1/api/assets/import`(무료)로 올리고 `/v1/api/animate-sync`(20크레딧/콜, prep 없음)로
  걷기만 만드는 경로를 쓸 것. idle은 이미 있는 `green_south/east/north/west.png`를
  그대로 재사용(INBOX #50 원문이 허용함). ③walk 프롬프트는 "프레임별로 어느 발이
  앞으로 나오는지"를 명시적으로 지정할 것(이번 바퀴 실패 원인 — 아래 "품질 판정" 참고).
  ④한 방향씩 만들고 그 즉시 확대 검사(다리 위주로 크롭+확대)한 뒤 다음 방향으로 넘어갈
  것 — 4방향을 한 번에 쏘고 나중에 몰아서 검사하면 실패했을 때 크레딧 낭비가 더 크다.
  ⑤west는 예산이 부족하면 기존 관행대로 east를 좌우 반전(flip_h)해서 대체하는 것도
  검토 가능(단, 걷기 자세가 좌우 대칭이 아닌 프롬프트를 썼다면 반전이 부자연스러울 수
  있으니 먼저 눈으로 확인).
  - 이번 바퀴가 이미 20크레딧을 써서 만든 south 방향 시험 결과물(품질 미달 판정,
    아래 참고)이 `/tmp/walk_south_sheet.png`(스프라이트시트)·
    `/tmp/walk_south_sheet_big.png`(4배 확대)에 남아있다 — 이 머신은 세션이 지나도
    `/tmp`가 지워지지 않는다. 다시 만들기 전에 먼저 이 파일을 Read로 열어서 정말
    재생성이 필요한지(혹시 처음 판정이 너무 엄격했는지) 재확인해도 좋다.
