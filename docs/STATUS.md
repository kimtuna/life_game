...
- **(2026-09-04 갱신, 바퀴 154) INBOX #101([DESIGN] 나무벽/나무문/석제벽 아이콘) 완료.**
  위 "마지막 갱신" 참고. **INBOX에 남은 미완료 항목(번호 순): `#102`([QA] 전체 스윕,
  BUILD/DESIGN 카운터 5 도달로 등록됨, 다음 QA 차례) → `#103`([BUILD] 모래/구리광석
  채광 포인트 추가, 다음 BUILD 차례) → `#104`([DESIGN] 모래/구리광석 아이콘, 다음
  DESIGN 차례, #103 완료 후 진행) → `#105`/`#106`(BUILD, 제련로·가공대 레시피 확장)
  → `#107`(DESIGN, #105/#106 아이콘) → `#108`~`#110`(BUILD, 강철 도구/건축/전기 계열)
  → `#111`(DESIGN, #108~#110 아이콘).** **BUILD/DESIGN 완료 카운터는 #98에서 5 도달로
  0 리셋된 뒤 #99(바퀴152)=1 → #100(바퀴153)=2 → #101(바퀴154, 이번)=**3**이 됐다**
  — 5 도달까지 2개 남음. `#103`/`#104`가 완료되면 5에 도달해 새 QA 스윕이 자동
  등록되지만, `#102`가 이미 큐에 있으니 그보다 먼저 처리될 것이다. (옛 스냅샷 메모는
  이미 다 처리되어 아래로 넘어갔다 — 갱신 시점 기준 최신 스냅샷만 여기 남김.)
