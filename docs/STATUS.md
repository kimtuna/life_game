...
- INBOX #17 완료: 밭 작물이 플레이어보다 앞에 그려지던 렌더 순서 버그 수정.
  - 원인: `farm_plot.tscn`의 `Crop` 노드(새싹/다 자란 작물 스프라이트)에 z_index가
    지정돼 있지 않아 기본값 0을 썼는데, 이는 `Player`(기본값 0)와 정확히 같은 값이다.
    `Crop`을 담은 `FarmPlot`은 `world.gd`가 `_ready()`에서 `add_child`로 나중에 트리에
    추가하는 노드라서, 같은 z_index에서는 씬 트리 순서(나중에 추가된 쪽이 위)에 따라
    항상 플레이어 위에 그려지고 있었다.
  - 수정: `Crop` 노드에 `z_index = -1`을 추가했다. 이미 형제 노드인 `Soil`이 같은 값을
    쓰고 있어서(바퀴 13/14 결정 로그: Ground(-2) < Soil/장식(-1) < Player(0)), 그
    체계를 그대로 따른 것이라 새 규칙을 만들지 않았다. 다른 z_index 값은 손대지 않았다.
  - 검증: (1) `godot --headless --path . --quit-after 3`로 씬 파싱 에러 없음 확인.
    (2) `godot --path . --script <임시스크립트>.gd`로 실제 `world.tscn`을 띄우고, 밭 한
    칸을 강제로 `READY`(다 자란 벼) 상태로 만든 뒤 플레이어를 그 위로 옮겨 스크린샷을
    찍어 확인 — 플레이어가 작물 줄기 앞(위)에 온전히 보이고 가려지지 않음을 눈으로
    확인했다. 임시 스크립트/이미지(`_tmp_shot17.gd`, `_tmp_shot17.png`)는 커밋 전에
    삭제했다.
  - 변경 파일은 `game/scenes/farm_plot/farm_plot.tscn` 한 줄뿐이다. 커밋에는 세션
    시작 전부터 미커밋 상태였던 `game/project.godot`(아래 "막힌 것/보류" 참고)와
    untracked `.uid` 파일들을 포함하지 않았다 — 이번 항목과 무관한 변경이라 섞지 않음.

## 다음에 할 것

- `docs/feedback/INBOX.md` "처리 대기"에 #18~#20이 미완료로 남아있다. 번호가 가장 작은
  **#18(사슴이 경계에서 도주하다 멈춤)** 이 다음 바퀴가 처리할 항목이다. 도주 로직이
  "플레이어 반대 방향"으로만 이동하는 것으로 보이는 파일(`deer.gd`)을 먼저 읽고, 막히면
  다른 방향(예: 벽/경계에 수직인 방향, 또는 여러 후보 방향 중 갈 수 있는 곳)으로 피하도록
  고칠 것.
- #19(사슴 체력바 UI), #20(전체 그래픽 통일성 — 바닥 텍스처 등)도 대기 중이다.
