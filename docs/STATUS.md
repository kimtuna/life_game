...
- `docs/feedback/INBOX.md` "처리 대기"의 다음 미완료 항목은 `#22`(도구를 실제
  아이템으로 만들기 — 총/도끼/곡괭이/낫/낚싯대를 인벤토리 아이템으로 추가하고
  숫자키 1~9로 핫바에서 골라 손에 드는 것)다. `#21`에서 만든 슬롯 구조
  (`InventoryData.get_general_slots()`, `HOTBAR_SIZE`)를 그대로 활용하면 된다 —
  아직 없는 도끼/곡괭이/낫/낚싯대 아이콘은 PixelLab로 새로 생성해야 한다.
