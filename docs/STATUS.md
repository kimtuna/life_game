...
**다음에 할 것**: INBOX에 남은 유일한 미완료 `[DESIGN]` 항목은 `#95`(유황광석 채광
포인트 월드 그림이 결정형 광물이 아니라 불꽃/폭발 이펙트처럼 보이는 문제 — 스파이크
형태를 각진 크리스탈 덩어리로 다시 그려야 함). 다음 `[DESIGN]` 바퀴는 `#95`부터
시작할 것. `/tmp/gen_ore_meat.py`의 `make_sulfur()`를 참고해서 고칠 수 있다(같은
스파이크 로직이 원인). AI 생성 도구 크레딧은 필요 없다(절차적 생성으로 계속 진행
가능 — 잔액 확인 불필요).
