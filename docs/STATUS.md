...
- `docs/feedback/INBOX.md` "처리 대기"에 남은 미완료 항목이 없다 (#1~#14 전부 완료).
  다음 바퀴는 INBOX.md에 새 지시가 추가되기를 기다리거나(비어 있으면 loop.sh가 세션을
  아예 열지 않는다), 아래 "막힌 것/보류"에 쌓인 항목 중 사람의 판단이 필요한 것들을
  해결해줄 새 지시를 기다릴 것.
- 이번 바퀴(#14)가 실제 두 클라이언트 간 연결/동기화를 실기기(스크린샷 등)로 검증하지
  못했다 — `--script` 헤드리스 테스트 하네스의 한계(아래 "헤드리스 CLI 검증" 참고) 때문에
  ENet 핸드셰이크 레벨 자동 테스트가 막혔고 예산이 소진됐다. 다음 바퀴가 멀티플레이 관련
  버그 리포트를 받으면, 먼저 정상 부팅 경로(`--script` 없이)로 호스트/참가자 두 프로세스를
  띄워 실제로 재현해볼 것 — 이번 바퀴의 코드 리뷰만으로는 놓쳤을 수 있는 문제가 있을 수
  있다.
