...
**다음에 할 것**: **다음 [BUILD] 세션은 INBOX #110**(전기 계열 완성품 3종 + 사료 +
급수 장치를 가공대 레시피로 추가, #105/#106 완료로 이미 진행 가능)을 처리한다.
참고할 것: (1) `#110` 완료 후 `#111`([DESIGN], `#108`~`#110`의 완성품 아이콘 11개+
가공식품 1개)로 진행 가능 — `#107`이 만든 `/tmp/gen_107.py`의 헬퍼 함수를 재사용할
것(강철 도구 3종은 기존 도구 아이콘과 같은 구도에 은회색 톤만 다르게 하라는 지시가
있으니 그 생성 방식도 먼저 조사할 것). (2) `#113`/`#114`([QA] 전체 스윕)는 번호
순서상 `#110`/`#111`보다 뒤에 자연스럽게 대기한다. (3) **새 QA 스크립트를 짤 때 슬롯
0 캐릭터의 인벤토리가 이전 바퀴들의 QA로 누적돼 가득 찼을 수 있다는 것을 항상
의심할 것** — 이번 바퀴가 겪은 것처럼 재료 부족이 아니라 "받을 공간이 없어서"
수령이 실패하는 것처럼 보일 수 있다. 위 "지시받지 않았지만 추가한 개선"의 초기화
패턴(`InventoryData._general_slots.fill(null)` + `_save()`)을 검증 시작 전에 넣어두는
것을 권장.
...
- **(2026-09-04 갱신, 바퀴 160) INBOX #106([BUILD]) 완료 — 가공대 부재료 5종(못/
  경첩/톱니바퀴/구리선/유리병) 레시피 추가.** 위 "마지막 갱신" 참고. **INBOX에 남은
  미완료 항목(번호 순): `#107`([DESIGN] #105/#106의 8개 가공물 아이콘, 다음 차례) →
  `#108`~`#110`(BUILD, 강철 도구/건축+갑옷/전기 계열, 전부 재료 준비 완료라 순서대로
  진행 가능) → `#111`(DESIGN, #108~#110 완성품 아이콘) → `#113`([QA] 전체 스윕,
  아래 참고).** **BUILD/DESIGN 완료 카운터**: #104(DESIGN)=1 → #105(BUILD)=2 →
  이번 #106(BUILD)으로 **3**이 됐다(5 도달까지 2개 남음 — `#107`/`#108`이 완료되면
  자동으로 `#113`과 별개의 새 QA 스윕이 큐에 추가될 것). `#113`([QA] 전체 스윕) 자체는
  이미 큐에 있고 번호 순서상 `#107`~`#111`보다 뒤에 자연스럽게 대기한다 — 처리될 때
  `game/qa/full_sweep.gd`(바퀴 155가 저장 상자/가공대/제련로/조리대/조리용 화로
  창까지 스텝을 추가해둔 버전)를 재사용하되, 모래/구리광석 채광 포인트를 실제로
