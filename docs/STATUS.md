...
바퀴 192 / 2026-09-05 ([BUILD] **INBOX #139 완료 — 숨은 동물 방향 힌트(노란
삼각형, `noise_hint_overlay.gd`)가 화면 하단 핫바 슬롯과 겹쳐 그려지던 버그를
고쳤다(핫바의 실제 화면 사각형을 조회해서, 겹칠 자리면 방향(x)은 유지하고 y만
핫바 위로 밀어올림).**)
1) **원인 확인**: `noise_hint_overlay.gd`의 `_draw()`가 힌트 위치를 계산할 때
   화면 크기와 고정 여백(`EDGE_MARGIN`)만 쓰고, `world.tscn`의 `UI/HUD/HotbarBar`가
   실제로 화면 하단 어디를 차지하는지는 전혀 몰랐다 — INBOX #139 원문 그대로였다.
2) **고친 방법**: 힌트 좌표 계산을 `_compute_point()`로 분리하고, 새 함수
   `_avoid_hotbar(point)`를 추가했다 — `@onready var _hotbar_bar`로 형제 노드
   (`../HUD/HotbarBar`)의 `get_global_rect()`를 매 프레임 조회해서, 계산된 point가
   그 사각형(+ 여유 `HOTBAR_CLEARANCE=14` + 삼각형이 튀어나올 수 있는 최대 반경
   `ICON_LENGTH=22`)과 겹칠 자리면 **x는 그대로 두고 y만** 핫바 바로 위로 밀어올렸다
   (`safe_bottom = rect.position.y - HOTBAR_CLEARANCE - ICON_LENGTH`). x를 안 건드린
   이유: 힌트의 목적이 "이 방향에 뭔가 있다"이므로, 남쪽 방향 힌트를 좌/우로 옮기면
   방향 정보 자체가 틀려진다 — 세로로만 밀어 올리면 "대략 남쪽"이라는 방향 정보는
   그대로 유지되면서 핫바만 피할 수 있다. 핫바가 없는 씬(단위 테스트 등)에서는
   `_hotbar_bar`가 null이라 이 로직이 조용히 건너뛰어진다.
3) **검증**: `game/qa/inbox139_check.gd`(영구 보존)를 새로 작성 — `inbox136_check.gd`와
   같은 부팅 패턴(플레이 → 슬롯 → 확정 → 싱글플레이)으로 월드에 들어간 뒤, 플레이어
   남쪽(핫바와 겹칠 확률이 가장 높다고 버그 리포트가 지목한 방향)으로 `notify()`를
   호출해 `_compute_point()`가 반환한 좌표가 핫바 사각형(+여유)과 안 겹치는지 코드로
   직접 확인했고, 스크린샷도 찍어서 Read로 직접 봤다 — 노란 삼각형이 핫바 1번 슬롯
   바로 위 빈 여백에 뚜렷하게 떠 있고 슬롯과 안 겹침을 확인. 회귀 방지용으로 서쪽
   방향(핫바와 무관) 힌트 위치가 이번 수정으로 안 바뀌었는지도 같은 스크립트에서
   확인했다. `project.godot [autoload]`에 임시 등록해 `godot --path .`(실제 렌더러)로
   실행 후 `QA_INBOX139_CHECK_PASS`를 확인했고, autoload 임시분은 되돌렸다(`git diff`
   빈 결과로 확인).
4) `docs/feedback/INBOX.md`의 `#139`를 `[x]`로 갱신했다. **INBOX에 남은 미완료
   항목이 없다** — 사람이 새 지시를 추가하기 전까지 `loop.sh`는 새 세션을 열지
   않는다. **BUILD/DESIGN 완료 카운터**: 직전 `#138`(BUILD)=1 → 이번 `#139`(BUILD)로
   **2**가 됐다(5 미만, 새 QA 스윕 추가 없음).

## 지난 바퀴 기록 (바퀴 191, 그대로 보존)

바퀴 191 / 2026-09-05 ([BUILD] **INBOX #138 완료 — 방 안 오브젝트가 벽을 가리는
그리기 순서 버그를 고쳤다(YSort만으로는 안 됨, 벽/문을 YSort 밖으로 빼고 고정
z_index 부여).**)
1) **시작 시 상태**: 세션 시작 시 `world.gd`/`world.tscn`/가공대류 4개 씬/저장
   상자 씬이 이미 미커밋 상태로 바뀌어 있었고 `game/qa/inbox138_repro.gd`(미추적)도
   남아 있었다 — 이전 바퀴가 예산 소진으로 커밋 못 하고 죽은 것으로 보였다. 진단:
   이전 세션은 "벽/문/가공대/제련로/조리대/조리용 화로/저장 상자/사슴/플레이어를 전부
   `YSort`(y_sort_enabled=true) 컨테이너 안에 넣고, 가공대류 스프라이트의 기존
   `z_index=-1`을 제거"하는 접근까지만 해두고 검증 전에 멈춰 있었다 — 이 상태 그대로는
   버그가 고쳐지지 않는다(아래 3번 참고). 이어서 처음부터 다시 만들지 않고 이 상태를
   기반으로 계속 진행했다(바퀴 3→4/24/39 등과 같은 패턴 — 이미 되어 있는 부분을
   버리면 예산 이중 낭비).
2) **실제로 재현해서 확인**: `game/qa/inbox138_repro.gd`(임시, 이번 바퀴에
   `inbox138_check.gd`로 재작성)를 `project.godot` `[autoload]`에 임시로 등록하고
   `godot --path .`(실제 렌더러)로 1칸짜리 최소 방(벽 4개) 한가운데 가공대를 놓는
   시나리오를 재현했다. 이전 세션의 YSort 상태 그대로는 가공대(140×116px)가 인접한
   벽 4칸을 여전히 완전히 가렸다 — 벽 스프라이트의 `z_index`를 100까지 강제로 올려도
   전혀 달라지지 않았다(스크린샷으로 확인).
3) **근본 원인 규명(WebSearch로 Godot 엔진 자체 동작 확인)**: Godot 공식 이슈
   [godotengine/godot#62715](https://github.com/godotengine/godot/issues/62715)
   ("Node2D Z-Index ignored when Y-Sort enabled")에 나온 그대로, **`y_sort_enabled`인
   부모 밑에 있는 자식들끼리는 `z_index`를 줘도 정렬에 반영되지 않고 Y좌표로만
   그려진다.** 즉 이전 세션의 "벽도 가공대도 다 같은 YSort 안에 넣기" 접근 자체가
   구조적으로 이 버그를 못 고치는 방향이었다 — 벽이 아무리 z_index가 높아도 YSort
   안에서는 무시된다. 최소 재현 스크립트로 이 사실을 직접 검증했다(같은 YSort 브랜치
   안에서는 z_index 차이가 안 먹히고, YSort 밖에서 비교하면 z_index가 정상 작동함을
   빨강/파랑 사각형으로 대조 확인).
4) **고친 방법**: 벽/문(`_spawn_structure()`)만 `ysort_layer` 밖(월드 최상위, `World`의
   직계 자식)으로 다시 빼고, 새 상수 `STRUCTURE_Z_INDEX := 5`를 줘서 YSort 브랜치
   (플레이어/동물/가공대류/저장 상자 — 전부 z_index 0 기본값) 전체보다 항상 위에
   그려지게 했다. 플레이어/동물/가공대류/저장 상자는 이전 세션이 만든 대로 YSort
   안에 그대로 둬서(옵션 (a)+(b) 조합) 서로는 Y좌표로 자연스럽게 앞뒤가 갈리되, 벽/문은
   무엇이 놓여 있든 항상 방 테두리로 보이도록 했다. 이동 충돌/방 감지(`_grid_occupancy`,
   `_recompute_rooms()`, `has_structure_at()`)는 딕셔너리 참조만 쓰고 노드의 부모가
   무엇인지는 신경 쓰지 않아서 건드릴 필요가 없었다(그대로 둠).
5) **검증**: `game/qa/inbox138_check.gd`(영구 보존, 기존 `_check.gd` 관례를 따름)를
   새로 작성했다 — 벽만 있는 기준 스크린샷과 가공대를 놓은 스크린샷을 같은 카메라
   좌표로 찍어서, 벽 4칸 각각의 중심 픽셀 색이 두 스크린샷에서 동일한지(=여전히
   벽이 그 자리에 그려지고 있는지) 코드로 직접 비교하는 자동 판정을 넣었다.
   `godot --path .`로 실행해 4칸 전부 `match=true`, `QA_INBOX138_CHECK_PASS`를 확인.
   **스크린샷을 원본 해상도(1280×720)로 눈으로만 봤을 때는 벽과 가공대가 둘 다 갈색
   나무 톤이라 경계가 잘 안 보여서 처음엔 "여전히 안 고쳐졌다"고 잘못 판단할 뻔했다**
   — 픽셀 비교와 4배 확대 크롭으로 실제로는 벽의 세로 판자 무늬가 가공대 표면과
   구분되게 정상적으로 그려지고 있음을 재확인했다(아래 "막힌 것/보류"에 남김). 이
   착오가 반복될 수 있으니 자동 픽셀 비교 방식을 스크립트에 영구히 남겨뒀다.
6) `project.godot`의 `[autoload]` 임시 추가분은 되돌렸다(`git diff` 빈 결과로 확인).
   `docs/feedback/INBOX.md`의 `#138`을 `[x]`로 갱신했다. **BUILD/DESIGN 완료
   카운터**: 직전 `#137`(QA 스윕, 카운터에 안 들어감)이 0으로 리셋해뒀으므로, 이번
   `#138`(BUILD)로 카운터는 **1**이 됐다(5 미만, 새 QA 스윕 추가 없음).

## 지난 바퀴 기록 (바퀴 190, 그대로 보존)

바퀴 190 / 2026-09-05 ([QA] **INBOX #137 완료 — 전체 스윕, 결함 2건 발견(#138 방
안 오브젝트가 벽을 완전히 가리는 그리기 순서 문제, #139 방향 힌트가 핫바와 겹침),
#138~#139로 티켓 발행.**)
1) **검사 범위/방법**: `game/qa/full_sweep.gd`(기존 스크립트 재사용, 수정 없음)를
   `project.godot` `[autoload]`에 `QASweep="*res://qa/full_sweep.gd"`로 임시
   추가하고 `godot --path .`(실제 렌더러, 헤드리스 아님)로 실행해 메인 메뉴부터
   캐릭터 생성 → 월드 스폰 → 도구 4종(idle/사용/이동) → 인벤토리 → 저장 상자/가공대/
   제련로/조리대/조리용 화로 → 농사/채집/채광(돌/철/모래/구리)/벌목/목장 → 건축
   격자(벽/문 고스트·설치·해제)/방 감지/방-상자 연동 → 사냥 → 낮밤/날씨까지 56장을
   `/tmp/qa67/`에 캡처했다. 실행 전 기존 세이브(`characters.save`/`inventory.save`)를
   지우고 깨끗한 상태에서 시작했다. 이번 스윕에서 특히 확인한 것: `#132`~`#134`가 고친
   벽/문 격자 정합·크기·그림(실제로 32px 1:1로 자연스럽게 이어 붙는지, "서 있는 벽"
   느낌이 나는지)과 `#135`~`#136`이 새로 만든 시야 안개(콘 밖 옅은 어둠, 벽 예외,
   동물 완전 숨김+방향 힌트)가 실제 화면에서 어떻게 보이는지. 실행 후 `[autoload]`
   임시 추가분은 되돌렸다(`git diff project.godot` 빈 결과로 확인).
2) **결함 1 (#138, [BUILD])**: `45_room_with_table.png`를 확대해서 보니(가공대를
   중심에 놓고 사방 벽 4개로 최소 크기 방을 만든 장면), 벽 4개가 안 보이는 게 아니라
   **가공대(140×116px) 스프라이트가 인접한 32px짜리 벽 칸들을 완전히 뒤덮어서** 벽/
   플레이어/가공대가 위아래로 삐져나온 널빤지 조각 몇 개만 보이는 알아볼 수 없는
   덩어리로 보였다. `#132` 리포트가 이미 같은 현상을 언급했었지만(그때는 "벽이
   16px라 작아서 안 보임"이 주 원인으로 지목됨), `#133`(격자=시각 크기 32px 통일)과
   `#134`(벽 그림 리메이크) 이후에도 **이 특정 증상(오브젝트가 벽을 가림)은 그대로
   남아있다** — 원인이 벽 크기가 아니라 `world.gd`가 벽이든 가공대든 전부 YSort/
   z_index 구분 없이 `add_child()` 순서(나중에 추가된 게 항상 위)로만 그리기 때문임을
   확인했다. 새 항목 `#138`로 등록(원인 분석과 해결 방향 후보까지 티켓에 적어둠 —
   YSort 도입 또는 Y기반 z_index 규칙).
3) **결함 2 (#139, [BUILD])**: 같은 `45_room_with_table.png`에서 화면 하단 핫바 9번
   슬롯 자리에 `#136`이 만든 노란 방향 힌트 삼각형(`noise_hint_overlay.gd`)이 정확히
   겹쳐서 아이템 아이콘처럼 보이는 걸 발견했다. `40~44`/`48`/`49` 등 다른 스텝에서도
   핫바 8~9번 슬롯 경계 근처에 반복적으로 나타나는 걸 확인 — 우연한 1회성이 아니라
   숨은 동물이 플레이어 남쪽에 있을 때 구조적으로 재현되는 문제다(힌트가 화면 가장자리
   전체를 대상으로 계산되는데, 하단 가장자리의 상당 부분이 핫바 UI와 겹치는 영역이라서).
   새 항목 `#139`로 등록.
4) **결함 없다고 판단한 부분**: 벽/문 격자 타일링(`40`~`44`, `50`)은 이어붙임에 빈틈/
   겹침 착시가 없었고, 벽 단독 그림(크롭해서 확대 확인)은 "서 있는 벽" 느낌(밝은
   윗면 + 세로 판자 무늬 + 어두운 밑단)이 잘 살아 있었다. 시야 안개는 프레임 정지
   상태로 캡처하는 QA 스크립트 특성상(`world.set_physics_process(false)`로 멈추는
   동안 `_aim_direction`도 같이 멈춤 — 이건 스크립트의 한계이지 실제 플레이에서는
   물리가 항상 돌아가 조준 방향이 매 프레임 갱신되므로 발생하지 않는 문제, 새 항목
   등록 안 함) 콘 방향이 프레임마다 고정돼 있었지만, 알파 0.22 톤·벽 즉시 노출 예외는
   여러 스텝에서 일관되게 관찰됐다. 밤/비 오버레이와 안개가 동시에 있어도 화면이
   과도하게 어두워지거나 깨지지 않았다(`55`/`56`). 목장 안 사슴은 안개와 무관하게
   항상 보였다(`is_ranched` 예외, 의도된 동작).
5) `docs/feedback/INBOX.md`의 `#137`을 `[x]`로 갱신하고 `#138`/`#139`를 새로
   추가했다. 전체 스윕이므로(좁은 범위 QA와 달리) 재확인 스윕 항목은 추가하지 않았다.

**다음에 할 것**: `docs/feedback/INBOX.md` "처리 대기"에서 번호가 가장 작은
미완료 항목은 `#139 [BUILD]`(방향 힌트가 핫바와 겹침)다 — `game/scripts/
noise_hint_overlay.gd`의 `EDGE_MARGIN`/좌표 계산이 화면 하단 핫바 영역을 고려하지
않는 문제, 티켓에 적힌 해결 방향 후보(핫바 영역 제외/좌우상단 전용/그리기 순서 조정)
중 스크린샷으로 비교해서 고른다. BUILD/DESIGN 카운터는 현재 1(5 미만, 새 QA 스윕
불필요).

## 지난 바퀴 기록 (바퀴 189, 그대로 보존)
...
그대로).**)
1) **구조**: `game/scripts/fog_of_war.gd`(새 파일, `class_name FogOfWar`)를
   `room_overlay.gd`와 똑같은 패턴(월드 좌표계 `Node2D`, `world.gd`가 `setup(world,
   grid_size)`로 자기 참조와 `BUILD_GRID_SIZE`를 넘겨줌, 카메라 뷰포트 범위만큼의
   격자 칸을 매 프레임 `_draw()`)으로 `world.tscn`에 새 노드(`FogOfWar`,
   `z_index=15` — `RoomOverlay`의 10보다 위)로 추가했다. 토글이 없다(`room_overlay`는
   버튼으로 켜고 끄지만, 안개는 DESIGN.md대로 항상 켜져 있는 별개 레이어).
2) **판정 로직**: 화면에 보이는 격자 칸마다 (a) 플레이어→칸 중심 거리가
   `FOG_RADIUS`(820, 1280×720 대각선 절반(~734)보다 살짝 크게 잡아 해상도가 달라져도
   화면 구석까지 덮음) 이내인지, (b) 그 방향이 조준 방향(아래 3번) 기준 ±55도
   (총 110도 콘) 안인지, (c) 플레이어 칸→대상 칸 사이(양 끝 제외)에 벽/닫힌 문이
   있는지(Bresenham 정수 라인으로 지나가는 칸들을 훑어 확인 — "레이캐스트든 그리드
   기반 shadowcasting이든 자유"라는 DESIGN.md 문구대로 가장 단순한 방식을 택함)를
   확인해서, 셋 다 통과 못 하면 그 칸을 완전한 검은색(불투명) 사각형으로 덮는다.
   열린 문은 (c)에서 막지 않는다(`world.gd`의 기존 `_is_position_blocked()`가 이동
   충돌에 쓰는 것과 같은 "열린 문 예외" 규칙을 시야 판정에도 그대로 적용).
3) **조준 방향 재사용**: 총 조준에 이미 쓰던 `to_mouse`(마우스 방향) 계산을 그대로
   재사용했다 — `world.gd`에 연속 각도용 `_aim_direction: Vector2`(정규화 벡터, 기본값
   `Vector2.DOWN`)를 새로 추가하고, 기존에 `_facing`(4방향 문자열)을 갱신하던 바로 그
   자리(`_physics_process`, `to_mouse.length() > 1.0`일 때)에서 같이 갱신한다. 공개
   접근자 `get_aim_direction()`/`is_cell_sight_blocked(cell)`을 `world.gd`에 추가해서
   `fog_of_war.gd`가 `room_overlay.gd`처럼 `world_ref`의 내부 상태(`_grid_occupancy` 등)를
   직접 건드리지 않고 공개 API만 쓰게 했다.
4) **검증**: `game/qa/inbox135_check.gd`를 새로 만들어 실제 렌더러로 확인했다.
   Ground 스프라이트가 실제로 그려지는 범위(원점 근처 -4000~4000, `region_rect`
   8000×8000)를 벗어난 좌표(예: 기존 #134 검증이 썼던 250000대)에서 테스트하면
   "안개로 검게 덮인 것"과 "애초에 바닥이 안 그려지는 빈 영역"이 스크린샷에서 구분이
   안 된다는 걸 처음에 겪어서, 기존 스폰물(사슴/밭/목장 등, 플레이어 시작점 반경
   1600~2000)과 안 겹치면서도 잔디가 실제로 그려지는 `(-3200,-3200)`/`(-3200,3200)`
   근방으로 좌표를 옮겨 다시 찍었다. 5×5 돌벽 방(남쪽 벽 중앙 나무문 포함)을 짓고
   (a) 방 한가운데서 동쪽 조준 — 동쪽(콘 안) 잔디만 보이고 등 뒤(콘 밖)·반대편 벽
   너머는 검음, (b) 같은 자리에서 북쪽 조준 — 보이는 칸이 실제로 바뀜(콘이 조준을
   따라 회전), (c) 방 밖에서 닫힌 문을 향해 조준 — 문 앞까지만 보이고 문 너머(방
   내부)는 완전히 검음, (d) 문을 열고 같은 조준 — 문이 있던 자리만 폭 32px짜리 시야
   "통로"가 뚫려 방 내부 북쪽 벽까지 곧장 보이고 문 양옆(여전히 닫힌 돌벽 구간)은
   그대로 검음, (e) 트인 벌판에서 동쪽 조준 — 110도 부채꼴이 화면 오른쪽 절반을
   넓게 덮고 왼쪽(등 뒤)은 거리와 무관하게 전부 검음 — 5장 전부 Read로 직접 열어
   확인했고 다섯 경우 모두 DESIGN.md 설명과 정확히 일치했다. `_recompute_rooms()`
   이후 `get_room_id_at()`이 정상적으로 방을 인식하는 회귀 확인(`room_id=1`)도 통과.
   실행은 기존 패턴대로 `project.godot` `[autoload]`에 임시 등록 →
   `godot --headless --editor --quit-after 30`(재가져오기) →
   `godot --path .`(실제 렌더러, `QA_INBOX135_CHECK_PASS` 확인) → `project.godot`
   원상복구(`diff`로 빈 diff 재확인)까지 마쳤다.
5) 이동 충돌/방 감지(`#122`)/총알 물리는 전혀 건드리지 않았다(DESIGN.md 명시대로
   렌더링 전용) — `_grid_occupancy`/`_recompute_rooms()`/`_is_position_blocked()`는
   손대지 않고 새 공개 접근자 2개만 추가했다. 멀티플레이는 각 클라이언트가 자기
   `world_ref.player_sprite`만 기준으로 계산하므로(원래 로컬 렌더링 전용 구조) 추가
   구현 없이 DESIGN.md 요구사항(각자 자기 화면만)을 만족한다 — 다만 실제 두 클라이언트
   동시 접속으로 "서로의 안개가 독립적으로 계산되는지"까지는 이번에도 실기기 검증을
   못 했다(멀티플레이 실기기 검증 자체가 바퀴 17부터 이월된 오래된 미해결 항목이라
   새로 막힌 것은 아님 — 아래 "막힌 것/보류" 참고).
6) `docs/feedback/INBOX.md`의 `#135`를 `[x]`로 갱신했다. **BUILD/DESIGN 완료
   카운터**: 직전 `#134`(DESIGN)로 카운터가 3이었으므로, 이번 `#135`(BUILD)로 **4**가
   됐다(전체 스윕까지 1개 남음 — 다음 BUILD/DESIGN 1개가 더 완료되면 5가 되어 QA 전체
   스윕 항목을 큐에 추가해야 한다).

**다음에 할 것**: `docs/feedback/INBOX.md` "처리 대기"에 `[BUILD]`/`[DESIGN]` 미완료
항목이 남아있는지 번호 순으로 확인할 것 — 이번 바퀴 기준으로는 `#135`가 마지막으로
등록된 항목이라 이 문서를 쓰는 시점엔 새 미완료 항목이 없다(다음 바퀴가 INBOX를 다시
읽어서 없으면 조용히 종료할 것). **주의**: BUILD/DESIGN 카운터가 4이므로, 다음
BUILD든 DESIGN이든 1개만 더 끝나면(카운터 규칙대로) 그 바퀴의 5번째 단계에서
`[QA] 전체 스윕` 항목을 새로 추가하고 카운터를 0으로 리셋해야 한다 — 잊지 말 것.

## 지난 바퀴 기록 (바퀴 187, 그대로 보존)

바퀴 187 / 2026-09-05 ([DESIGN] **INBOX #134 완료 — 나무벽/석제벽/강철벽/나무문/
강철문/창문 6종을 "서 있는 벽"처럼 보이도록 절차적(Python/Pillow) 생성으로 다시
그림(AI 생성 도구 호출 없음).**)
1) **도구 선택**: PROMPT_DESIGN.md③/DESIGN.md "그래픽 파이프라인"에 따라 정적
   오브젝트(벽/문/창문은 유기체가 아님)는 절차적 생성이 명시적으로 허용된다.
   STATUS.md 결정 로그(바퀴 135 전후)에 SpriteCook 유료 구독이 두 번 결제에도
   잔액 0으로 막혀 캐릭터 그림 작업이 전면 보류된 기록이 있고, INBOX #79 이후
   정적 오브젝트는 크레딧 없는 절차적 생성으로 전환하기로 이미 정해져 있었다 —
   이번에도 PixelLab/SpriteCook 잔액을 재확인하지 않고 곧장 Python(PIL)로
   진행했다(③의 "먼저 잔액부터 확인" 절차는 AI 생성 경로를 탈지 여부를 정할 때만
   적용되는데, 이미 정적 오브젝트는 절차적 생성이 기본 경로로 정해져 있어 해당 없음).
2) **원인 분석**: 기존 6개 아이콘(`wood_wall.png` 등, 전부 32×32)을 PIL로 열어보니
   전체 픽셀의 약 34%가 알파 0(둥근 아이템 카드 형태의 여백)이었다 — 즉 벽을 인접
   배치해도 실제 "칠해진" 벽 그림은 격자 한 칸을 꽉 채우지 못하고, 상/하단에 별다른
   구분(캡/그림자) 없이 세로줄 무늬만 반복돼 사용자가 "밭처럼 보인다"고 지적한
   원인이 됐다.
3) **해법**: `game/assets/sprites/items/{wood_wall,stone_wall,steel_wall,wood_door,
   steel_door,window}.png`를 32×32 **풀블리드(투명 여백 0)**로 다시 그렸다. 공통
   레이어 구조(위→아래): (a) 최상단 1px 림 하이라이트, (b) 5px 캡 밴드(재질 밝은
   톤 — 벽 "윗면"이 빛을 받는 것처럼), (c) 1px 심(가장 어두운 톤, 캡/몸통 경계),
   (d) 20px 몸통(재질별 텍스처 — 나무는 세로 판자+홈+결 무늬, 돌은 어긋쌓기 벽돌+
   모르타르선, 강철은 2단 패널+리벳 4개), (e) 3px 전이 그라데이션, (f) 2px 베이스
   그림자(가장 어두운 톤 — 바닥과 닿는 지점). 좌측 1열은 밝게, 우측 1열은 어둡게
   블렌드해서 미세한 베벨(입체감)도 더했다. 문/창문은 같은 셸(캡/그림자/베벨)
   위에 재질 프레임을 두르고, 문은 상/하 2패널(손잡이 색상 유지: 나무문=노랑,
   강철문=하늘색)로, 창문은 기존처럼 4분할 유리+대각선 하이라이트로 채웠다.
   `/tmp/wall_gen/gen.py`에 전체 생성 코드가 있다(재사용 가능 — 다음에 재질을
   추가하려면 `make_shell()`/`apply_bevel()`을 그대로 쓰고 `draw_*()`만 새로
   작성하면 됨).
4) **검증**: `game/qa/inbox134_check.gd`를 새로 만들어 (a) 6종을 한 줄로 배치 +
   캐릭터를 옆에 세운 스크린샷, (b) 나무벽 5개 인접 배치 이음매 스크린샷(캐릭터가
   벽 바로 위/앞에 선 구도), (c) 문 열림/닫힘 상태 전환 스크린샷 2장, (d) 벽 4x4
   회귀(방 인식 `room_id != -1`) 코드 확인까지 전부 했다. `project.godot`
   `[autoload]`에 임시 등록 → `godot --headless --editor --quit-after 30`(신규
   PNG를 못 찾는 문제를 피하려고 먼저 재가져오기) → `godot --path .`(실제 렌더러)로
   실행 → `QA_INBOX134_CHECK_PASS` 확인 → `project.godot` 원상복구(`git diff` 빈 diff
   재확인). 캡처한 5장을 Read로 직접 열어봤다: 재질 6종이 서로 뚜렷이 구분되고
   (나무=갈색 세로 판자, 돌=회색 벽돌, 강철=은색 리벳 패널), 문/창문도 벽과 같은
   "캡+그림자" 셸을 공유하면서 손잡이/유리로 확실히 구별됐다. 나무벽 5개를 이어
   붙인 화면에서는 위쪽 캡이 끊김 없이 하나로 이어지고 바닥 그림자도 연속돼 "선 벽"
   처럼 보였다(#133이 만든 32px 격자와 완전히 겹쳐서 캐릭터가 벽 바로 위에 서면
   상반신만 보이는 자연스러운 구도가 나옴). 문 열림 상태(반투명+오프셋, `door.gd`
   로직 변경 없음)도 닫힘과 뚜렷이 구별됨. 방 회귀(`room_id=1`)도 정상.
5) `docs/feedback/INBOX.md`의 `#134`를 `[x]`로 갱신했다. **BUILD/DESIGN 완료
   카운터**: 직전 `#133`(BUILD)으로 카운터가 2였으므로, 이번 `#134`(DESIGN)로
   **3**이 됐다(전체 스윕까지 2개 남음).

**다음에 할 것**: `docs/feedback/INBOX.md` "처리 대기"에 남은 미완료 항목(번호 순)
— `#135`([BUILD], 시야/전장의 안개, #133/#134 이후 진행 권장 — 이제 벽 경계와
시각적 형태가 모두 정리됐으니 바로 착수 가능). 다음 `[BUILD]` 바퀴는 `#135`를
집으면 된다. `[DESIGN]` 하네스는 INBOX에 새 `[DESIGN]` 항목이 추가되기 전까지는
할 일이 없다 — 다음 `[DESIGN]` 바퀴는 INBOX를 다시 확인해서 새 항목이 없으면
빈 문서 규칙대로 조용히 종료할 것. 참고로 바퀴 186이 남긴 옛 관찰 후보(#132 유래,
극단적 카메라 각도에서 캐릭터 머리/몸통이 분리돼 보이는 현상, `/tmp/qa132/
01_player_beside_wall.png`)는 이번 범위 밖이라 손대지 않았다 — 여전히 재현 여부
확인 가치가 있다는 판단 유효.

## 지난 바퀴 기록 (바퀴 186, 그대로 보존)
...
- **(2026-09-05 갱신, 바퀴 192) INBOX #139([BUILD]) 완료 — INBOX.md에 미완료
  항목이 하나도 없다.** 사람이 새 지시를 추가하기 전까지 `loop.sh`는 새 세션을
  열지 않고 스스로 종료한다(정상 동작). **BUILD/DESIGN 완료 카운터는 2**(#138, #139
  — 5 도달까지 3개 남음). 다음 바퀴가 시작되면(새 INBOX 항목이 추가된 뒤) 가장 먼저
  `docs/feedback/INBOX.md`에서 번호가 가장 작은 미완료 항목을 다시 확인할 것 — 아래
  나머지 항목들은 바퀴 164 시점 기준 참고 메모라 지금은 번호가 안 맞을 수 있다(예:
  `#111`/`#113`/`#114`는 이미 오래전에 처리되어 지금 INBOX.md에는 없다).
...
- (바퀴 191) #138을 고치고 나서도(벽이 실제로는 항상 위에 정상 그려짐, 픽셀 비교로
  확인) 1280×720 축소 스크린샷을 눈으로만 보면 벽(나무 널빤지)과 가공대(나무 가구)가
  둘 다 비슷한 갈색 톤이라 경계가 잘 안 보인다 — 4배로 확대해서 보면 세로 판자 무늬로
  구분되지만, 축소된 화면에서는 여전히 "덩어리"처럼 보일 여지가 있다. 이건 그리기
  순서(BUILD 범위) 문제가 아니라 벽/가구 색 대비(DESIGN 범위) 문제라서 이번 항목에서는
  건드리지 않았다. 나중에 실제 플레이 화면에서도 방 구분이 잘 안 된다는 지적이 나오면
  벽 색상을 가구류와 더 대비되게(예: 더 밝거나 채도 높은 톤) 다시 그리는 `[DESIGN]`
  항목으로 다룰 것 — 아직 사람 지적이 없어서 새 티켓을 만들지는 않았다.
...
- (바퀴 186, INBOX #133) **충돌 크기와 시각 크기를 별도 상수로 분리하는 방식(#132)을
  ...
  같이 재계산할 것.
- (바퀴 191, INBOX #138) **"플레이어/동물/가공대류처럼 Y좌표로 자연스럽게 앞뒤가
  갈려야 하는 것들"과 "벽/문처럼 크기와 무관하게 항상 맨 위여야 하는 것들"을 같은
  `y_sort_enabled` 컨테이너 안에 함께 넣는 방식은 채택하지 않기로 확정함.** 근거:
  Godot 엔진 자체가 `y_sort_enabled`인 부모 밑의 자식들 사이에서는 `z_index`를
  완전히 무시하고 Y좌표로만 그린다(공식 이슈
  [godotengine/godot#62715](https://github.com/godotengine/godot/issues/62715)로
  확인, 벽 스프라이트 z_index를 100~105까지 올려도 전혀 반영 안 됨을 직접 재현해
  검증함). 그래서 "항상 맨 위" 규칙이 필요한 벽/문은 YSort 컨테이너 밖(월드 최상위)에
  별도로 두고 고정 `z_index`(`STRUCTURE_Z_INDEX`)를 줘야 한다 — 앞으로 "이 오브젝트는
  무조건 다른 것보다 위/아래여야 한다"는 요구가 생기면 YSort 안에 넣지 말고 이
  패턴(YSort 밖 + 고정 z_index)을 따를 것. 반대로 "서로 Y좌표로 자연스럽게 앞뒤가
  갈려야 하는 것들"(플레이어/동물/가공대류/저장 상자)은 계속 YSort 안에 두는 게 맞다.
- (바퀴 191, INBOX #138) 렌더링 순서를 코드로만 고치고 스크린샷을 축소 해상도로만
  보면 "고쳐졌는지 눈으로 판단하기 어려운" 경우가 있다는 것을 겪었다(벽과 가공대가
  같은 갈색 팔레트라 축소 화면에서 경계가 흐릿함). **다음 바퀴가 참고할 점**: 그리기
  순서/z_index를 고치는 작업을 검증할 때는 (1) 스크린샷을 확대해서 눈으로 보는 것과
  별개로, (2) "고치기 전/후 특정 좌표의 픽셀 색이 기대한 대로 바뀌었는지"를 코드로
  직접 비교하는 자동 판정을 반드시 같이 넣을 것 — 시각적으로 비슷한 색의 오브젝트가
  겹치는 상황에서는 사람 눈보다 픽셀 비교가 더 믿을 만하다(`game/qa/
  inbox138_check.gd`에 이 패턴을 남겨뒀다, 재사용 가능).
