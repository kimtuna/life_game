...
- **(2026-09-04 신규, 최우선) INBOX #79/#80 — 크레딧이 필요 없는 절차적(파이썬) 정적
  오브젝트 작업으로 전환.** #79는 나무(logging 대상) 그림을 잔디 텍스처(`grass_tile.png`)
  때와 같은 방식(Pillow 도형+그라디언트)으로 절차적으로 만드는 [DESIGN] 항목, #80은
  그 나무로 실제 벌목 포인트를 만들어 도끼에게 처음으로 사용 대상을 붙여주는 [BUILD]
  항목이다(`world.gd:62` 주석에 "도끼는 아직 벌목 대상이 없고"라고 오래전부터 적혀
  있던 갭). AI 생성 도구를 쓰지 않으므로 크레딧/잔액 확인 없이 바로 진행 가능.
