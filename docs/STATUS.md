...
바퀴 60 / 2026-09-03 (**INBOX #48 착수 보류 재확인 (4바퀴 연속 — 57/58/59/60 전부 동일
결과).** `GET https://api.pixellab.ai/v1/balance`를 다시 호출한 결과 여전히
`{"type":"usd","usd":0.0}`. `PIXELLAB_API_KEY` 환경변수 자체는 정상으로 주입돼 있음을
확인했다(길이 36자, API가 401이 아니라 정상 JSON 잔액 응답을 줬으므로 키 문제가 아니라
순수하게 잔액이 0인 것). #48은 `gun_idle_south`/`gun_fire_south`/`gun_idle_north`/
`gun_fire_north`(3색, 12장)에서 총의 몸체 형태·각도 자체를 조준 방향(위/아래)으로
다시 그려야 하는 작업으로, 바퀴 58이 판단한 대로 순수 좌표 이동/PIL 회전으로는
도트 그래픽 외곽선이 뭉개져 합격 기준(①)을 통과할 수 없고 PixelLab이 반드시 필요하다 —
이 판단을 다시 검토했지만 뒤집을 근거를 찾지 못했다. DESIGN.md/PROMPT.md 어디에도 이
상황(잔액 소진)에 대한 대안이 없어 임의로 범위를 지어내지 않았다. 게임 코드/에셋은
전혀 건드리지 않았다(변경 파일 없음) — 이 문서(STATUS.md) 갱신과 커밋만 했다. INBOX #48은
`- [ ]`로 그대로 둔다. **다음 바퀴가 참고할 것**: 잔액 확인만 반복하는 바퀴가 이미 4개
누적됐다 — 잔액이 계속 0이면 사용자의 실제 충전 전까지는 세션이 할 수 있는 일이 없으니,
매번 전체 STATUS.md/DESIGN.md를 다시 읽고 같은 결론에 도달하는 데 드는 시간만 최소화하고
(이 항목의 조사 결과는 이미 충분히 상세하니 재조사하지 말고 잔액 확인 한 번만 하고 바로
이 패턴을 반복할 것) 빠르게 세션을 마칠 것.)

바퀴 59 / 2026-09-03 (**INBOX #48 착수 보류 재확인.** `GET /v1/balance`를 다시 호출한
결과 여전히 `{"type":"usd","usd":0.0}` — 바퀴 58과 동일한 결과다. 바퀴 58이 세션
종료(커밋) 전에 죽어서 이 STATUS.md 갱신 내용이 미커밋 상태로 작업 디렉터리에 남아
있었는데, 내용을 읽어보니 이번 바퀴가 직접 재확인한 사실(잔액 $0, #48은 그림 자체를
다시 그려야 해서 PixelLab 없이는 품질 기준 통과 불가)과 정확히 일치해 새로 쓰지 않고
그대로 검수 후 재사용한다. 게임 코드/에셋은 이번 바퀴도 전혀 건드리지 않았다 — 이
문서 갱신과 커밋만 한다. INBOX #48은 `- [ ]`로 그대로 둔다.)
...
- **PixelLab 잔액이 여전히 $0.00다 (바퀴 57, 58, 59, 60 네 번 확인, API 키 자체는
  정상 — 401이 아니라 정상 JSON 응답으로 0을 반환함).** 다음 바퀴가 세션을 시작하면
  **가장 먼저 `GET https://api.pixellab.ai/v1/balance`로 잔액을 다시 확인할 것** —
  사용자가 충전했다면 값이 바뀌어 있을 것이다. 여전히 0이면 INBOX #48을 시도하지
  말고(아래 이유 참고) STATUS.md에 그 사실만 짧게 기록한 뒤 세션을 마칠 것(이미 원인
  분석은 충분히 끝났으니 매번 재조사하지 말고, 확인 한 번 + 기록만 하고 빠르게 종료 —
  불필요한 반복 확인/시행착오로 예산을 쓰지 말 것). 순수 좌표/합성 편집(PIL)만으로 고칠
  수 있는 문제는 바퀴 56(낚싯대)·57(총구 불꽃)처럼 계속 진행 가능하다.
- **INBOX #48(남/북 총 방향 결함)은 바퀴 58이 시도했으나 PixelLab 잔액 $0으로 착수하지
  못했다 — 여전히 번호가 가장 작은 미완료 항목이므로, 잔액이 채워지면 다음 바퀴는 이
  항목부터 시작할 것.** #48은 `gun_idle_south`/`gun_fire_south`/`gun_idle_north`/
  `gun_fire_north`(3색, 12장)에서 총의 몸체 형태 자체를 조준 방향(위/아래)으로 기울어지게
  다시 그려야 한다 — **좌표 이동이 아니라 실제 그림을 새로 그려야 하는 작업이라 PixelLab이
  반드시 필요하다고 판단했다**(순수 PIL 회전은 픽셀 아트 외곽선이 뭉개져 품질 기준 미달
  위험, 위 "마지막 갱신" 참고). 잔액이 채워지면: east/west 방향은 이미 자연스러우니 각도
  참고용으로 먼저 확대 비교하고, PixelLab `/inpaint`나 `/rotate`로 south/north 총 부분만
  다시 그리는 것을 우선 검토할 것(전체 캐릭터를 처음부터 다시 생성하지 않는 방향).
  그 뒤로 #49(총 QA 재확인)가 대기 중이다. 도구 애니메이션 통합(총/도끼/곡괭이낫/낚싯대,
  #41~#45)은 바퀴 56으로 전부 끝났다.
- **(바퀴 57 신규) 색상별(green/blue/red) 결함이 항상 3색 전부에 동일하게 있다고
  가정하지 말 것.** INBOX #47 원문은 "3색"이라 썼지만 실제로 확대 검수해보니 blue는
  이미 정상이었고 green/red만 결함이 있었다 — 고치기 전에 반드시 3색 전부를 개별로
  직접 눈으로 먼저 확인하고, 실제로 문제 있는 색상만 건드릴 것(불필요한 재작업/재생성
  방지).
- **(바퀴 57 신규) 캐릭터 스프라이트에서 특정 요소(불꽃 등)만 픽셀 단위로 지우거나
  옮길 때, 색상 임계값을 전체 캔버스에 적용하면 머리카락/피부톤과 혼동되고, "idle과
  다른 픽셀"을 전체 캔버스에 적용하면 반동 자세의 팔다리 이동분까지 오탐된다.** 반드시
  먼저 그리드 눈금(8px 간격) 오버레이 스크린샷으로 대상 요소가 몸통과 안 겹치는 좁은
  사각 범위를 육안으로 확정한 뒤, 그 범위 안에서만 판정 규칙(idle에 없던 새 픽셀 OR
  특정 색상)을 적용할 것. 수정 후에는 원본과의 픽셀 diff를 마젠타로 시각화해 의도치
  않은 부분이 안 바뀌었는지 확인하는 것도 유효했다(바퀴 57에서 사용).
  - **#47(동/서 총구 불꽃 위치)**: `game/assets/sprites/character/gun/{variant}_east_fire.png`/
    `{variant}_west_fire.png`(3색, 6장)만 고치면 된다 — idle 프레임과 south/north
    fire는 문제 없으니 건드리지 말 것. 불꽃을 총구(바라보는 방향 쪽 끝) 근처로 옮겨
    다시 합성/생성.
  - **#48(남/북 총 방향)**: `gun_idle_south`/`gun_fire_south`/`gun_idle_north`/
    `gun_fire_north`(3색, 12장)를 다시 그려야 한다 — 총이 조준 방향(아래/위)으로
    기울어지게. east/west는 이미 자연스러우니 참고용으로 각도를 비교해볼 것.
  - 검증은 `game/_verify_*.gd`(커밋 전 삭제) 헤드리스 스크린샷 스크립트를
    `set_physics_process(false)` → 상태 강제 설정 → 몇 프레임 대기 → 캡처 패턴으로
    재사용할 것 — 바퀴 56이 `_verify_rod_motion.gd`로 다시 확인한 최신 예시(그 전에는
    `_verify_gun_motion.gd`).
  - PixelLab로 특정 부위(불꽃 위치, 각도 등)만 살짝 고칠 때는, 전체를 처음부터 다시
    생성하지 말고 바퀴 56이 낚싯대에서 쓴 방식(PIL로 합성 좌표만 바꿔 다시 합성 →
    확대해서 눈으로 확인 → 문제없을 때만 PixelLab 인페인트/회전 호출)을 먼저 검토할
    것 — 근본 원인이 좌표 하나였던 경우 이 방식이 API 호출을 크게 아낀다.
  - 이전 바퀴들의 도끼/곡괭이낫/낚싯대 생성물은 각각 `/tmp/axe_gen/`, `/tmp/pickaxe_gen/`,
    `/tmp/rod_gen/`(폐기된 첫 시도)·`/tmp/rod_gen2/`(완성본)에 남아있다(이 머신은 세션이
    지나도 `/tmp`가 지워지지 않는다) — #47/#48 작업 시 총 관련 폴더(`/tmp/gun_gen/`)를
    참고하되 새 작업은 새 폴더를 쓸 것.
  - **총을 든 채 이동할 때 다리 애니메이션이 멈추는 문제(옵션A, "막힌 것/보류" 아래
    오래된 메모 참고)는 도끼/곡괭이낫/낚싯대 전부에 동일하게 적용되는 채로 남아있다** —
    지시가 오면 walk_<tool> 프레임(도구별 그림 추가)을 그려서 옵션B로 확장할 수 있다.
