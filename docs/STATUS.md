...
- **(바퀴 128 갱신, 최우선) INBOX 미완료 항목은 `#75`([QA] "도구 관련 모션" 좁은 범위
  재검증) 하나뿐이다 — `[QA]` 하네스 몫.** 총/도끼/곡괭이낫/낚싯대 4개 전부의 (1) idle
  크기 일관성(이번 바퀴가 고친 총 동/서 idle 포함), (2) 이동하며 실제로 사용했을 때
  모션이 나오는지, (3) idle↔맨손 걷기 전환까지 확인하는 항목이다. 문제가 남아있는 한
  QA가 재확인 루프를 스스로 계속 큐에 추가하도록 지시돼 있다(#75 원문 참고). 세션
  시작 시 SpriteCook/PixelLab 잔액 확인은 그림 작업이 있는 바퀴에서 여전히 습관적으로
  할 것(마지막 확인은 바퀴 128 — SpriteCook 0크레딧, PixelLab $0, 이번 바퀴는 AI
  호출 없이 PIL 후처리만으로 끝냈다).
