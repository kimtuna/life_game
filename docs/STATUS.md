...
**다음에 할 것**: INBOX.md에 남은 미완료 항목은 `#114`([QA] 전체 스윕, #104~#108 구간)와
`#115`([BUILD], 방금 등록한 리소스 포인트 겹침 버그)다. 번호 순서대로면 `#114`(QA)가
먼저지만, 두 항목 다 각자 하네스(QA/BUILD)가 처리한다 — 다음 [BUILD] 세션이 열리면
`#115`(포인트 간 최소 거리 확인 추가 + `set_input_as_handled()` 방어)를 먼저 맡고,
[QA] 세션이 열리면 `#114`를 맡는다. `game/qa/full_sweep.gd`에 이번에 추가한 4개 스텝
(`mining_point_sand_idle`/`_harvest`, `mining_point_copper_idle`/`_harvest`)과
`ISOLATED_SPAWN_POS = Vector2(2500, 2500)` 격리 좌표 패턴은 재사용 가능 — 앞으로
"필드에 새 오브젝트 하나만 깔끔하게 스폰해서 스크린샷 찍기"가 필요하면 이 상수를
그대로 쓰거나 참고할 것(자연 스폰 반경 1400보다 멀고 Ground 텍스처 범위 ±4000보다는
안쪽이어야 회색 빈 배경이 안 찍힌다).
