...
바퀴 55 / 2026-09-03 (**INBOX #46(QA 관찰, 총 idle/발사 모션) 완료 — 결함 2건 발견,
#47/#48로 티켓 발행, #49로 재확인 루프 등록.** 세션 시작 시 `git status`로
`world.gd`(M)/`STATUS.md`(M)/`fishing_rod/`(신규, untracked)가 미커밋 상태인 것을 먼저
확인했다 — INBOX.md는 이미 `#45 완료`로 커밋(65d303e)돼 있었지만 실제 코드는 커밋 전에
죽은 상태였다(바퀴 3, 51→52와 같은 패턴의 새 변형: 이번엔 **INBOX 체크박스 커밋과 실제
코드 커밋이 서로 다른 커밋으로 분리**돼 있었다). 재활용 전에 먼저 검수했더니, `_verify_rod_motion.gd`
헤드리스 스크린샷(4방향×idle/fishing)에서 "낚시하는" 모션 그림이 손에서 완전히
분리되어 캐릭터 발밑에 막대기가 떨어진 것처럼 보이는 합성 결함을 4방향 전부에서
확인했다(idle은 정상) — 합격 기준(①) 미달로 판단해 **커밋하지 않고 폐기**했다
(`git checkout -- game/scenes/world/world.gd`, `fishing_rod/` 폴더 삭제) — `INBOX.md`의
#45를 `- [ ]`로 재오픈하고 정황을 남긴 뒤 별도로 먼저 커밋(692b0a4)했다.
이후 실제 배정 작업인 **#46**을 클린 코드베이스(HEAD, gun/axe/pickaxe는 이미 커밋된
상태) 기준으로 진행했다. `game/_verify_gun_motion.gd`(커밋 전 삭제)로 world 씬을 실제
렌더링 드라이버로 띄우고 `set_physics_process(false)` → `_facing`/`_held_tool="gun"`/
`_tool_use_flash_timer`를 강제 설정 → `_update_player_animation()` → 4프레임 대기 →
캡처를 8가지 상태(south/east/north/west × idle/fire, green)에 대해 반복해 스크린샷을
찍고 Read 도구로 직접 확대해서 눈으로 봤다(콘솔에 `anim=gun_idle_south` 등으로 애니메이션
전환도 코드대로 동작함을 확인). 발견한 결함 2건:
1. **동/서 발사 시 총구 불꽃이 총구(바라보는 방향 쪽 끝)가 아니라 총 반대쪽(몸에 더
   가까운 쪽)에서 나타남** — 예: 동쪽을 보고 있으면 총구는 화면 오른쪽인데 불꽃은 화면
   왼쪽(몸 쪽)에서 튐. 남/북은 불꽃이 정상적으로 바깥쪽(총구 쪽)에서 나타나 동/서만의
   문제로 보인다. → INBOX #47로 등록.
2. **남/북(정면/후면)에서 총이 조준 방향(위/아래)과 무관하게 완전히 좌우 수평으로
   몸에 걸쳐 들려 있음** — 동/서(옆모습)는 총이 보고 있는 쪽으로 자연스럽게 뻗어 있어
   문제 없지만, 남/북은 옆으로 눕혀 든 것처럼 보여 조준하는 느낌이 안 든다. → INBOX
   #48로 등록.
DESIGN.md "QA 관찰 항목" 형식대로 #47/#48 뒤에 #46과 동일한 내용의 "QA 재확인" 항목
#49를 추가해 재검증 루프를 이어지게 했다. 이번 항목(#46) 자체에서는 그림/코드를 고치지
않았다(관찰·티켓 발행까지만).)

## 끝난 것 (지금까지의 스냅샷 — 바퀴별 상세 이력은 git 커밋 메시지 `[INBOX #N] ...`에 있음)

- INBOX #46 완료 (바퀴 55, QA 관찰): 위 "마지막 갱신" 참고. 총 idle/발사 모션 4방향을
...
- **다음 바퀴는 INBOX #45(낚싯대 통합, 재오픈)부터 시작할 것** (번호가 가장 작은
  미완료 항목). 큐에는 그 뒤로 #47(동/서 총구 불꽃 위치 결함) → #48(남/북 총 방향
  결함) → #49(총 QA 재확인)가 순서대로 대기 중이다.
  - **#45 재작업 시 반드시 피해야 할 것**: 이전(미커밋 상태로 죽은) 세션이 만든
    "낚시하는" 모션 합성이 손에서 완전히 분리되어 캐릭터 발밑에 낚싯대가 떨어진
    것처럼 보이는 결함이 있었다(이번 바퀴가 스크린샷으로 4방향 전부에서 확인하고
    폐기함) — south raw 합성 단계부터 이미 잘못돼 있었다(rotate 이후에 생긴 문제가
    아님). 손-도구 연결 좌표(캐릭터 손 위치 대략 x:22-41,y:33-40)에 실제로 겹치게
    합성됐는지 PIL 합성 직후(인페인트/회전 전) 단계에서부터 확대해서 눈으로 확인할
    것 — 도끼/곡괭이낫 때는 이 단계에서 문제가 없었어서 넘어갔지만, 낚싯대는 아이콘
    형태(가늘고 긴 대각선 막대)가 달라 손 좌표 계산을 다시 해야 할 수 있다.
    idle 모션(정적으로 들고 있는 자세)은 이전 시도에서 이미 자연스러웠으니 그 좌표는
    재사용 가능하고, "낚시하는" 모션 좌표만 다시 잡으면 된다.
  - 레시피 자체(기존 검증된 도구 아이콘을 PIL로 캐릭터 베이스에 합성 → 손 연결부만
    PixelLab `/inpaint` → `/rotate`로 south→east/north → west는 PIL 좌우반전)는 총
    (#42)/도끼(#43)/곡괭이낫(#44)에서 이미 검증됐으니 그대로 재사용하되, **각 단계마다
    중간 결과를 확대해서 직접 눈으로 확인하고 다음 단계로 넘어갈 것** — 이번 바퀴처럼
    최종 단계(게임 내 스크린샷)까지 가서야 결함을 발견하면 처음부터 다시 만들어야 한다.
  - 완성한 PNG는 `game/assets/sprites/character/fishing_rod/`에 저장 →
    `godot --headless --path . --import` 강제 재임포트 → `world.gd`의
    `_build_player_sprite_frames`/`_current_animation_name`/`_select_hotbar`/
    `_unhandled_input`을 총(#42)/도끼(#43) 패턴 그대로 복제 → 검증 통과 후에만
    `_held_item_sprite`/`HELD_ITEM_OFFSETS`/`HELD_ITEM_BEHIND_FACINGS`/`TOOL_USE_ICONS`/
    `_play_tool_swing()` 등을 정리(삭제)할 것(INBOX #45 원문 지시).
  - 검증은 `game/_verify_*.gd`(커밋 전 삭제) 헤드리스 스크린샷 스크립트를
    `set_physics_process(false)` → 상태 강제 설정 → 몇 프레임 대기 → 캡처 패턴으로
    재사용할 것 — 이번 바퀴가 `_verify_gun_motion.gd`/`_verify_rod_motion.gd`로 다시
    확인한 최신 예시.
  ...
  - 이전 바퀴들의 도끼/곡괭이낫 생성물은 각각 `/tmp/axe_gen/`, `/tmp/pickaxe_gen/`에
    남아있다(이 머신은 세션이 지나도 `/tmp`가 지워지지 않는다) — #45/#47/#48 작업 시
