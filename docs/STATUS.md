...
**다음 바퀴가 참고할 것**: 다음 `[BUILD]` 항목은 `#89`(가공대에 화약/탄약 레시피
추가 — 새 오브젝트 아님, `processing_table.gd`의 `RECIPES` 목록만 확장하면 됨).
`#90`(조리대+조리용 화로, #87/#88과 같은 프레임워크)이 그 다음이다. `#91`([DESIGN],
가공대/제련로/조리대/조리용 화로 4개 오브젝트 그림 + 아이템 아이콘 일괄 제작)은 여전히
`#89`~`#90`이 먼저 끝나야 의미가 있어 대기. `#92`([QA] 전체 스윕)는 이미 큐에 등록돼
대기 중(별도 QA 하네스 세션 몫). **BUILD/DESIGN 완료 카운터는 이번(#88)으로 2** —
다음 QA 스윕 자동 등록까지 3개 더 필요.
...
- **(2026-09-04 갱신, 바퀴 141) INBOX #88([BUILD] 제련로) 완료.** 위 "마지막 갱신"
  참고. **다음 `[BUILD]` 항목은 `#89`**(화약/탄약 레시피 — 새 오브젝트 아님,
  `scenes/processing_table/processing_table.gd`의 `RECIPES` 목록만 확장하면 된다.
  유황광석+숯→화약, 화약+철→탄약. 총 재장전 연동은 범위 밖, 건드리지 말 것).
  그 다음은 `#90`(조리대+조리용 화로, #87/#88과 같은 프레임워크를 `scenes/
  cooking_table/`·`scenes/cooking_stove/`로 새로 만들어야 함 — #89와 달리 새
  오브젝트 2개가 필요). **다음 `[QA]` 항목은 `#92`**(전체 스윕, 바퀴 139에서 이미
  큐에 등록돼 대기 중 — 별도 QA 하네스 세션 몫이라 이번 바퀴도 건드리지 않았다).
  **BUILD/DESIGN 완료 카운터는 바퀴 139(#86)에서 5에 도달해 0으로 리셋된 뒤 140(#87)=1
  → 이번(#88)=2가 됐다** — 다음 QA 스윕 자동 등록까지 3개 더 필요. `[DESIGN]` 큐
  `#91`(가공대/제련로/조리대/조리용 화로 4개 오브젝트 그림 + 가공물/완성품/가공식품
  아이콘 일괄 제작)은 여전히 `#89`~`#90`이 먼저 끝나야 의미가 있어 대기. **참고**:
  제련로의 월드 그림도 가공대와 마찬가지로 아직 임시 사각형(`Polygon2D`, 회색 계열로
  가공대의 갈색과만 구별)이고, 철/숯 아이템도 아직 전용 아이콘이 없어 핫바/인벤토리에
  텍스트로만 표시된다 — 둘 다 결함이 아니라 `#91`이 처리할 범위(INBOX 원문이 명시).
