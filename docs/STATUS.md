...
- **PixelLab 잔액이 $0.00다 (바퀴 57에서 확인).** 다음 바퀴가 새 그림 생성/인페인트/회전이
  필요한 항목을 만나면 API 호출이 실패할 것이다 — 먼저 잔액을 확인하고, 0이면 무리하게
  호출을 반복하지 말고 STATUS.md(이 절)에 기록한 뒤 INBOX 항목을 미완료로 남겨둘 것.
  순수 좌표/합성 편집(PIL)만으로 고칠 수 있는 문제는 바퀴 56(낚싯대)·57(총구 불꽃)처럼
  계속 진행 가능하다.
- **다음 바퀴는 INBOX #48(남/북 총 방향 결함)부터 시작할 것** (번호가 가장 작은 미완료
  항목). #48은 `gun_idle_south`/`gun_fire_south`/`gun_idle_north`/`gun_fire_north`(3색,
  12장)를 총이 조준 방향으로 기울어지게 다시 그려야 하는데, **이건 좌표 이동이 아니라
  실제 그림을 다시 그려야 하는 작업이라 PixelLab이 필요할 가능성이 높다** — 잔액 $0
  상태이므로 이 항목은 이번 바퀴 방식(PIL 픽셀 편집)으로 해결하기 어려울 수 있다. 먼저
  잔액을 재확인하고, 여전히 0이면 STATUS.md에 막힘을 기록하고 INBOX #48을 완료 처리하지
  말 것. 그 뒤로 #49(총 QA 재확인)가 대기 중이다. 도구 애니메이션 통합(총/도끼/곡괭이낫/
  낚싯대, #41~#45)은 바퀴 56으로 전부 끝났다.
