...
3) **연결**: `mining_point_sand.tscn`/`mining_point_copper.tscn`의 `texture_path`를
   ...
   딕셔너리와 `_ready()`의 관련 분기를 완전히 삭제했다(#84가 도입한 임시 처리
   패턴이었는데, #86 때 stone/sulfur_ore에서 이미 한 번 폐기됐다가 #103이 sand/
   copper_ore에 재도입한 것 — 이제 그 두 항목마저 전용 그림이 생겨서 이 메커니즘을
   쓰는 곳이 완전히 없어져 죽은 코드가 됐으므로 규칙대로 삭제).
4) **검증**: 새 QA 스크립트 `game/qa/inbox104_check.gd`(커밋, 재사용 가능,
   `wall_door_icon_check.gd`(#101)+`mining_variety5_check.gd`(#103)를 합친 구성)를
   `project.godot [autoload]`에 임시 등록하고 세이브 파일을 지운 뒤 `godot --path .`
   (실제 렌더링)로 실행: (a) 필드에 모래/구리광석 채광 포인트를 나란히 놓고 찍은
   스크린샷(`/tmp/qa104/00_field_sand_copper.png`)을 Read로 직접 봐서 베이지색
   납작한 더미와 붉은 오렌지색 크리스탈 뭉치가 뚜렷이 다른 실루엣·색으로 보이고
   주변 밭/잡초와도 안 겹침을 확인, (b) 두 아이템을 바닥에 드롭해서 4배 확대
   크롭(`/tmp/qa104_drop_zoom.png`)으로 본 결과 둘 다 화면에서 32px를 넉넉히 넘고
   서로 구별됨을 확인, (c) 인벤토리 창에 채워 넣은 스크린샷(`/tmp/qa104/02_inventory.png`)
   에서 "모래 x3"/"구리광석 x3" 라벨이 기존 아이템과 같은 스타일로 정상 표시됨을
   확인 — 콘솔에 `QA_INBOX104_CHECK_PASS` 출력. 검증 후 `project.godot`의 임시
   autoload 등록을 원상복구하고 `diff /tmp/project.godot.bak104 project.godot`로
   완전히 동일함을 확인했다(커밋 대상은 새 PNG 4개 + `.import` 4개 + `dropped_item.gd`/
   두 `.tscn` 수정 + 새 QA 스크립트 1개뿐).
5) `docs/feedback/INBOX.md`의 `#104`를 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#103`(BUILD)이 5에 도달해 `#113`([QA] 전체 스윕)을 큐에 추가하며 0으로 리셋된
   뒤, 이번 `#104`(DESIGN)로 **1**이 됐다(5 도달까지 4개 남음 — `#113`은 카운터와
   무관하게 이미 큐에 있으므로 그대로 대기).

## 스스로 판단해서 고친 부분 (INBOX #104)
- 구리광석 1차 시도(어두운 암석 위에 둥근 주황색 반점 3개)를 Read로 직접 보고
  "크리스탈이 아니라 뭉툭한 반점"이라고 스스로 판정해 폐기하고, 유황광석 스타일의
  다이아몬드형 크리스탈 스파이크(다만 위쪽만이 아니라 사방으로, 색은 적갈색)로
  다시 그렸다. 근거: INBOX #104 원문이 "광택 있는 결정형"을 명시했는데 1차 시도는
  실루엣이 각지지 않아 그 표현에 못 미쳤다고 판단(사용자 지적 아님, 자체 판정).

## 어려움/에러 (INBOX #104)
- 없음 — 절차적 생성만 사용했고(PixelLab/SpriteCook 호출 자체를 시도하지 않음),
  기존 `.tscn`/`dropped_item.gd` 구조가 #84/#103이 이미 만들어둔 modulate 임시
  처리 패턴을 그대로 걷어내는 정도라 API/엔진 관련 시행착오는 없었다.

**다음에 할 것**: **다음 BUILD 세션은 INBOX #105**(제련로 레시피 확장 — 철 2개→강철,
모래 2개→유리, 구리광석 2개→구리)를 처리한다. 참고할 것: (1) #99가 만든 수량 지정+
타이머+수동 수령 프레임워크(`CraftingStation` 베이스)를 그대로 타므로 `RECIPES` 상수만
확장하면 된다(#105 원문). (2) 이어서 `#106`(가공대 부재료 5종), 그다음 `#107`
([DESIGN], 이번 #104와 같은 절차적 생성 기법 재사용 — 강철=은회색, 유리=반투명 청록,
구리=붉은 금속광택 등, `#105`/`#106` 완료 후 진행). (3) `#113`([QA] 전체 스윕)은 이미
큐에 있음 — `full_sweep.gd`에 모래/구리광석 채광 스텝을 추가하는 항목인데, 이번 #104로
"임시 modulate 색 처리 상태"라는 전제가 이미 바뀌었으니(전용 그림으로 교체됨) 그
QA 바퀴가 이 사실을 참고할 것. (4) BUILD/DESIGN 완료 카운터는 이번(#104)으로 **1**.
