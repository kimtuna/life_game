...
- INBOX #37 완료: 총의 "들고 있는 모션"과 "발사하는 모션"을 서로 다른 그림으로 분리.
  `game/assets/sprites/tools/gun_firing.png`(총구에 발사열이 추가된 그림)를 새로 추가하고,
  `world.gd`에 `TOOL_FIRING_ICONS` 딕셔너리 + `_firing_flash_timer`(0.12초)를 둬서
  `_fire()` 호출 시 잠깐 발사 텍스처로 바뀌었다가 자동으로 들고 있는 텍스처(`gun.png`)로
  되돌아가게 했다. 세션 시작 시 이 코드/이미지가 이미 미커밋 상태로 존재했다(이전 세션이
  중간에 끊긴 것으로 보임, 아래 결정 로그 참고) — 내용을 검수만 하고 다시 만들지 않았다.
  - 검증: `game/_verify_gun_motion.gd`(커밋 전 삭제)로 world 씬을 실제 렌더링 드라이버로
    띄운 뒤 south/east/north/west 4방향 각각 holding/firing 상태로 강제 전환하며
    스크린샷 8장을 찍어 직접 확인했다. 4방향 모두 총이 자연스러운 위치(옆구리/등 뒤)에
    들려 있고, 발사 순간에는 총구의 노란 발사열 + 총알 궤적이 뚜렷이 구분돼 보였다 —
    합격 기준(①) 통과로 판단.
  - 변경 파일: `game/scenes/world/world.gd`, `game/assets/sprites/tools/gun_firing.png`(+`.import`).

## 다음에 할 것

- **다음은 INBOX #38**(도끼의 "들고 있는 모션"/"패는 모션"을 PixelLab로 서로 다른
  그림으로 만들기 — #37과 같은 패턴, DESIGN.md "도구 동작 표현" 규칙). 이어서
  #39(곡괭이낫: 들기/채광/채집 3종)/#40(낚싯대: 들기/낚시)이 같은 패턴으로 대기 중이다.
- #37에서 검증한 방식(스크린샷 검증 스크립트로 `_facing`을 강제 전환하고 `_physics_process`를
  꺼서 4방향을 순회하며 holding/사용 상태를 캡처)을 #38~#40에도 그대로 재사용할 수 있다.
  단, holding 단계를 캡처하기 전에는 이전 단계가 남긴 "사용 중" 텍스처를 직접 원래
  텍스처로 되돌려줘야 한다(아래 결정 로그 참고 — `_physics_process`를 꺼둔 검증
  스크립트에서는 자동 복귀 로직이 안 돈다).
- #38~#40은 전부 PixelLab로 실제 그림을 새로 그려야 하는 작업이라 이전 바퀴들보다
