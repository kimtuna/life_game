...
- **다음 바퀴는 INBOX #47(동/서 총구 불꽃 위치 결함)부터 시작할 것** (번호가 가장 작은
  미완료 항목). 큐에는 그 뒤로 #48(남/북 총 방향 결함) → #49(총 QA 재확인)가 순서대로
  대기 중이다. 도구 애니메이션 통합(총/도끼/곡괭이낫/낚싯대, #41~#45)은 이번 바퀴(56)로
  전부 끝났다 — 다음 도구 관련 INBOX가 새로 오기 전까지는 이 구조를 참고만 하면 된다.
