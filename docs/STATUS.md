...
- INBOX #34 완료: 목장에 동물을 풀어놓는 방식을 "빈손 좌클릭"에서 "포획한 동물 아이템
  (captured_deer)을 핫바에서 손에 든 채 좌클릭"으로 바꿨다(DESIGN.md "목장" 절, #27
  씨앗 심기와 같은 "손에 든 아이템으로 좌클릭" 패턴).
  - `ranch_zone.gd`: `REQUIRED_TOOL := ""` 상수를 지우고, `_unhandled_input()`의 발동
    조건을 `world_ref.get_held_tool() == REQUIRED_TOOL`(빈손 확인)에서
    `world_ref.get_held_item() == CAPTURED_ITEM`(손에 든 아이템이 captured_deer인지
    확인)로 바꿨다. `get_held_item()`은 #27이 이미 만들어둔 함수를 그대로 재사용했다 —
    새 함수를 추가할 필요가 없었다.
  - `ranch_zone.tscn`의 `Prompt` 라벨 텍스트를 "빈손으로 좌클릭: 사슴 풀어놓기"에서
    "포획한 사슴을 들고 좌클릭: 풀어놓기"로 바꿨다.
  - 검증: `godot --headless --path . --script <임시스크립트>.gd`로 ranch_zone.tscn을
    실제로 인스턴스화하고, `world_ref`를 `get_held_item()`을 오버라이드할 수 있는 가짜
    Node2D로 교체한 뒤 (1) 빈손 상태에서 좌클릭 이벤트를 보내도 사슴이 스폰되지 않는 것
    (자식 노드 수 불변), (2) `captured_deer`를 인벤토리에 2개 넣고 손에 든 것으로
    설정한 뒤 좌클릭하면 사슴이 스폰되고(자식 노드 +1) 인벤토리 개수가 1개 소비되는 것 —
    2개 케이스 모두 콘솔에 `PASS`로 확인했다. (주의: 오토로드 `InventoryData`도 다른
    오토로드처럼 `_ready()`가 첫 프레임 이후에 실행되므로, `_initialize()` 직후 바로
    `add_item()`을 호출하면 슬롯 배열이 아직 `resize`되지 않아 아이템이 씹힌다 — 1프레임
    이상 지난 뒤 호출해야 한다. 이번 바퀴에 새로 발견해서 아래 "헤드리스 CLI 검증" 절에
    기록함.) 검증 전후로 `inventory.save`를 지워 깨끗한 상태를 유지했고, 임시 스크립트는
    커밋 전 삭제했다. 텍스트 라벨 하나와 조건식 한 줄 수준의 작은 변경이라 실제 렌더링
    스크린샷은 별도로 찍지 않았다(예산 절약) — 다음 바퀴가 목장 화면을 다룰 일이 있으면
    그때 프롬프트 문구가 잘리지 않는지 눈으로 한 번 확인할 것.
  - 변경 파일: `game/scenes/ranch_zone/ranch_zone.gd`, `game/scenes/ranch_zone/ranch_zone.tscn`.

## 다음에 할 것

- `docs/feedback/INBOX.md` "처리 대기"에 다음 미완료 항목은 **#35(도구 든 자세 4방향
  품질)**이다. `_held_item_sprite`가 `player_sprite`의 자식이라 항상 위에 그려지는 것이
  의심되는 원인이라고 INBOX 원문에 적혀 있다 — north(후면)를 볼 때 도구가 몸 앞에 뜬
  것처럼 보이는지 실제로 4방향(정면/좌/우/후면) 스크린샷을 찍어서 눈으로 확인하고,
  방향별로 z_index나 위치/각도를 다르게 조정할 것. 스크린샷 비교가 핵심이므로 예산을
  충분히 남겨두고 시작할 것(지난 바퀴들 기준 스크린샷 촬영+재확인에 예산이 꽤 든다).
- **INBOX #34(목장 풀어놓기 방식 정정)는 이번 바퀴(36)에 완료했다.** 고친 파일은
  `game/scenes/ranch_zone/ranch_zone.gd`, `game/scenes/ranch_zone/ranch_zone.tscn` 둘뿐이다.
