...
- **(바퀴 127 갱신, 최우선) INBOX #75까지 전부 완료돼서 미완료 항목이 하나도 없다.**
  다음 `loop.sh` 실행은 세션을 새로 열지 않고 스스로 종료한다(정상 동작) — 사람이
  새 지시를 `docs/feedback/INBOX.md`에 추가해야 다음 바퀴가 돈다. 세션 시작 시
  SpriteCook/PixelLab 잔액 확인은 그림 작업이 있는 바퀴에서 여전히 습관적으로 할 것
  (마지막 확인은 바퀴 124 — SpriteCook 0크레딧, PixelLab은 바퀴 123에 $0).
...
- **BUILD/DESIGN 완료 카운터: 바퀴 119(#67 QA 완료)에서 0으로 리셋된 뒤 바퀴
  ...
  #74(QA 전체 스윕)를 큐에 추가하고 다시 0으로 리셋됐다. 바퀴 125(#73)로 1, 바퀴
  127(#75)로 2가 됐다(바퀴 126은 QA라 카운터에 반영 안 됨).** 5에 도달하려면 3개 더
