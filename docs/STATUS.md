...
- INBOX #35 완료: 도구를 든 채 4방향(정면/동/서/후면)을 볼 때 부자연스러운 문제를
  고쳤다. 먼저 원인을 실제로 스크린샷으로 확인했다(플레이어를 마우스 방향으로 자동
  회전시키는 `world.gd`의 조준 로직이 있어서, 헤드리스에 가까운 실행 환경에서는 OS
  마우스 커서를 못 옮기니 `world.set_physics_process(false)`로 그 로직을 꺼두고
  `_facing`/`_update_texture()`/`_update_held_item_transform()`을 직접 호출해 4방향을
  강제로 재현하는 임시 스크립트로 캡처했다, 커밋 전 삭제함):
  - south(정면): 총이 몸통 정중앙(offset x=3)에 겹쳐서 배에서 튀어나온 것처럼 보였다.
  - north(후면): 총이 `_held_item_sprite`가 `player_sprite`의 자식이라 항상 위에
    그려져서 머리/어깨 위에 떠 있는 것처럼 보였다(INBOX #35 원문이 의심한 원인이 실제
    원인이었음을 확인).
  - east/west(측면): 이미 자연스러웠다(옆으로 충분히 떨어진 offset).
  - 고친 내용(`game/scenes/world/world.gd`): `HELD_ITEM_OFFSETS`의 south를
    `(3,15)→(16,11)`, north를 `(3,-15)→(14,-2)`로 바꿔 몸통 중앙 대신 옆구리/엉덩이
    쪽으로 옮겼다. 그리고 `HELD_ITEM_BEHIND_FACINGS := ["north"]` 상수를 추가하고
    `_update_held_item_transform()`에서 `_held_item_sprite.show_behind_parent = _facing
    in HELD_ITEM_BEHIND_FACINGS`를 매 방향 전환마다 갱신하게 했다 — north일 때만 도구가
    캐릭터 스프라이트보다 뒤에 그려져(등에 걸친 것처럼) 더 이상 얼굴/어깨 위에 뜨지
    않는다. east/west/south는 기존처럼 앞에 그려진다.
  - 재검증: 같은 임시 스크립트로 수정 후 4방향을 다시 캡처해서 4장 전부 직접 눈으로
    확인했다 — south는 총이 이제 오른쪽 옆구리에 자연스럽게 걸려 있고, north는 총이
    몸 뒤로 반쯤 가려진 채 어깨 옆으로만 살짝 보여 "등에 멘" 것처럼 보인다. east/west는
    변경 없음을 확인.
  - 도구 그림 자체(총 하나짜리 대각선 원근 아트)는 방향별로 다시 그리지 않았다 — 그림을
    새로 만드는 건 이번 지시(위치/z_index 조정)의 범위를 넘고, 위치/앞뒤 순서 조정만으로
    합격 기준(①)을 통과할 만큼 자연스러워졌다고 판단했다.
  - 변경 파일: `game/scenes/world/world.gd` 하나뿐(오프셋 상수 2개 값 + 상수 1개 추가 +
    함수 한 줄 추가).

## 다음에 할 것

- **`docs/feedback/INBOX.md` "처리 대기"에 미완료 항목이 하나도 없다 (#1~#35 전부
  `[x]`).** PROMPT.md에 따르면 이 상태에서는 loop.sh가 다음 세션을 열지 않고 스스로
  종료한다 — 다음에 할 일은 사람이 INBOX.md에 새 지시(`#36`부터)를 추가하는 것이다.
- **INBOX #35(도구 든 자세 4방향 품질)는 이번 바퀴(37)에 완료했다.** 고친 파일은
  `game/scenes/world/world.gd` 하나뿐이다(위 "끝난 것" 참고).
- 다음 지시가 들어오면 참고할 만한 백로그(강제 사항 아님, 아래 "막힌 것/보류"·"오래된
  ...
  - 도끼/낚싯대는 최소 스윙 동작만 있고 실제 결과물(벌목/낚시)이 없음(DESIGN.md 범위 밖,
    의도된 상태).
  - 도구를 든 채 4방향을 볼 때의 그림 자체(총 하나짜리 대각선 아트를 방향별로 새로
    그리는 것)는 이번 바퀴에서 다루지 않았다 — 위치 조정만으로 합격선을 넘었다고
    판단했지만, 더 정교한 요구(예: 방향별 총 그림을 따로 그려달라)가 오면 그때 PixelLab로
    만들 것.
