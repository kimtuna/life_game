...
- `docs/feedback/INBOX.md` "처리 대기"의 다음 항목은 **#11(농사)**이다. #10에서
  얻은 벼 씨앗을 밭(정해진 구역)에 심으면 시간이 지나 벼로 자라고 수확할 수 있어야
  한다. 지금까지 인벤토리(`InventoryData`)와 아이템 이름("rice_seed")까지는 이미
  있으니, 그 위에 밭 구역(필드에 고정 배치된 Area2D 등)과 "심기 → 성장 타이머 →
  수확 가능" 상태 전이만 새로 설계하면 된다. 수확물(벼)을 인벤토리에 어떤 이름으로
  넣을지(`rice`?)는 DESIGN.md에 정해져 있지 않으니 임의로 정하되 STATUS.md
  결정 로그에 남길 것. 성장 시간(초 단위 등 구체적 숫자)도 DESIGN.md에 없으므로
  임의로 정하돼 짧게(수 분 내)를 추천 — 낮/밤 시스템(#13)이 아직 없어서 "며칠" 단위
  성장은 지금 구현할 방법이 없다.
- 채집/채광 포인트는 "20초 쿨다운 후 재생성"(무한 자원) 방식으로 만들었다. 밭도
  비슷하게 "수확 후 다시 심을 수 있는 빈 상태로 돌아간다" 정도로 충분해 보이지만,
  #11을 시작하기 전에 이 판단이 여전히 맞는지 다시 확인할 것.
