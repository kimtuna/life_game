...
## 스스로 판단해서 고친 부분 (INBOX #106)
- 없음 — INBOX 원문이 재료/산출량/카테고리까지 전부 구체적으로 지정해뒀고, 코드
  프레임워크(`CraftingStation`)도 #105와 동일하게 그대로 재사용 가능해 임의 판단이
  필요한 지점이 없었다.

## 어려움/에러 (INBOX #106)
- 없음 — `processing_table.gd`의 `RECIPES` 배열 확장 + `world.gd` 아이템 등록만으로
  끝났고, #105의 QA 스크립트 패턴을 그대로 재사용할 수 있어 API/엔진 관련 시행착오는
  없었다.

**다음에 할 것**: **다음 [DESIGN] 세션은 INBOX #107**(#105/#106이 추가한 8개 가공물
— 강철/유리/구리/못/경첩/톱니바퀴/구리선/유리병 — 아이콘을 파이썬 절차적 생성으로
일괄 제작, AI 생성 크레딧 미사용)을 처리한다. 참고할 것: (1) `#107` 완료 후
`#108`(강철 도구 3종, [BUILD])과 `#109`(강철 건축+갑옷, [BUILD])가 진행 가능해진다
(#105/#106 둘 다 이미 완료됨). (2) `#110`(전기 계열+사료/급수, [BUILD])도 #105/#106
완료로 이미 진행 가능. (3) `#111`([DESIGN], #108~#110의 완성품 아이콘)은 그 셋이
끝난 뒤 진행. (4) `#113`([QA] 전체 스윕)은 이미 큐에 있고 번호 순서상 `#107`~`#111`
보다 뒤에 자연스럽게 대기한다. (5) BUILD/DESIGN 완료 카운터는 이번(#106)으로
**3**(5 도달까지 2개 남음 — `#107`/`#108`이 각각 완료되면 5에 도달해 새 QA 스윕이
자동으로 큐에 추가될 것).
