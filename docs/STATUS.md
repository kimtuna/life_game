...
**다음에 할 것**: 다음 미완료 항목은 `#126`([BUILD], 방(Room) 오버레이 UI —
화면 우상단 토글 버튼 + 산소미포함 스타일 반투명 격자 색칠). 그 뒤로 `#127`(HUD
정리)/`#128`(건설 해제 모드)/`#129`(건축 겹침 검사 확장)/`#130`(#128 확장)이
큐에 대기 중이다. BUILD/DESIGN 완료 카운터는 1(전체 스윕까지 4개 남음).
