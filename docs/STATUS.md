...
- `docs/feedback/INBOX.md` "처리 대기"의 다음 미완료 항목은 `#28`(총알이 마우스
  커서/조준점보다 위에서 나가는 것처럼 보이는 버그 — 총알 스폰 위치/방향 계산 확인 및
  수정)이다.
- **INBOX #27(농사 심기 방식 변경)은 이번 바퀴(29)에 완료했다.** `world.gd`에 새로
  추가한 `get_held_item()`(선택된 핫바 슬롯의 아이템 키를 도구 여부와 상관없이 반환)을
  다음 바퀴가 "이 아이템을 손에 들고 있어야 한다"는 유사한 요구(도구가 아닌 소모품
  기준 상호작용)를 또 만들게 되면 재사용할 것 — `get_held_tool()`은 여전히 도구
  전용이니 혼동하지 말 것.
- INBOX #28(총알 스폰 위치 버그)을 조사할 때는 `world.gd`의 `_fire_bullet()`(또는
  이름이 바뀌었을 수 있으니 총알 생성/발사 관련 함수)에서 총알 시작 위치가 플레이어
  스프라이트의 어느 자식/오프셋 기준인지, 그리고 마우스 조준 방향 계산에 카메라
  줌/오프셋이 반영되고 있는지부터 확인할 것 — INBOX #6의 카메라/해상도 규칙(해상도가
  달라도 시야는 고정)과 얽힐 수 있으니 해상도를 바꿔가며(#5/#7에서 만든 해상도 옵션)
  재현되는지도 함께 확인하면 원인 범위를 좁히기 쉽다.
