...
- INBOX #36 완료: 기본탄/마취탄 탄창 분리. `game/scenes/world/world.gd`의
  `_ammo_in_magazine`을 단일 `int`에서 `{"normal": 8, "tranq": 8}` 형태의 `Dictionary`로
  바꿨다. `_fire()`/`_start_reload()`/발사 조건/라벨 표시가 전부 `_ammo_in_magazine[_ammo_type]`을
  읽고 쓰도록 고쳤다. 재장전 도중 우클릭으로 탄종을 바꾸는 엣지 케이스(재장전이 끝나는
  시점에 `_ammo_type`이 이미 다른 값으로 바뀌어 있으면 엉뚱한 탄창이 채워지는 문제)를
  막기 위해 `_reloading_ammo_type`(재장전을 시작한 시점의 탄종)을 별도로 기억해뒀다가
  타이머가 끝나면 그 탄종의 탄창만 채우도록 했다 — 지시문에는 없었지만 Dictionary로
  바꾸면서 자연스럽게 드러난 버그라 같이 고쳤다.
  - 검증: `game/_verify_ammo.gd`(커밋 전 삭제)로 world 씬을 헤드리스로 띄워서
    ① 기본탄 8발을 다 쏘고 → 0 확인, ② 마취탄으로 전환 → 마취탄 탄창은 그대로 8 확인
    (분리 확인), ③ 마취탄 3발 쏘고 기본탄으로 복귀 → 기본탄 여전히 0 확인, ④ R
    재장전 → 기본탄만 8로 차고 마취탄은 그대로 5로 유지되는 것 확인. 전부 PASS.
  - 시행착오: 검증 스크립트 최상단에서 `const WorldScene := preload(...)`로 world.tscn을
    미리 로드하면 그 파싱이 `--script` 진입 스크립트 자체의 컴파일 단계에서 일어나서
    오토로드(`CharacterData` 등)가 아직 전역 식별자로 등록되기 전이라 "Identifier not
    found" 컴파일 에러가 났다. `preload()` 대신 `_process()` 안에서 몇 프레임 지난 뒤
    `load("res://scenes/world/world.tscn")`로 지연 로드하니 해결됐다 — 기존
    STATUS.md의 "`--script` 모드는 오토로드를 정적 식별자로 못 씀" 메모와 같은 뿌리의
    문제이지만, 이번엔 테스트 스크립트 자체가 아니라 그 스크립트가 최상단에서
    `preload`하는 씬의 의존 스크립트들이 문제였다는 점이 새로운 발견이다(아래 "헤드리스
    CLI 검증" 절에 일반화해서 추가함).
  - 변경 파일: `game/scenes/world/world.gd` 하나뿐.

## 다음에 할 것

- **다음은 INBOX #37**(총의 "들고 있는 모션"/"발사 모션"을 PixelLab로 서로 다른
  그림/애니메이션으로 새로 만들기 — DESIGN.md "도구 동작 표현" 규칙, 단순 오프셋
  조정으로는 부족하다고 사용자가 재지적함). 이어서 #38(도끼)/#39(곡괭이낫)/#40(낚싯대)이
  같은 패턴으로 대기 중이다.
- #37~#40은 전부 PixelLab로 실제 그림/애니메이션을 새로 그려야 하는 작업이라(코드
  조정만으로는 불충분하다고 이미 확정됨, DESIGN.md "도구 동작 표현" 참고) 이전
  바퀴들(#36 이하)보다 시간/토큰을 더 많이 쓸 가능성이 높다 — 세션 예산이 빠듯하면
  PROMPT.md ⑤대로 그림 생성이 끝나는 대로, 또는 방향 하나가 검증되는 대로 더 일찍
  커밋하는 것을 고려할 것.
- 다음 지시가 들어오면 참고할 만한 백로그(강제 사항 아님, 아래 "막힌 것/보류"·"오래된
  ...
  - 도끼/낚싯대는 최소 스윙 동작만 있고 실제 결과물(벌목/낚시)이 없음(DESIGN.md 범위 밖,
    의도된 상태) — #38/#40에서 "드는/쓰는 모션"만 만들고 실제 벌목/낚시 결과물은 여전히
    범위 밖이다.
