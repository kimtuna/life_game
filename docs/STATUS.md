...
- **(2026-09-04 갱신, 바퀴 143) INBOX #90([BUILD] 조리대+조리용 화로) 완료.** 위
  "마지막 갱신" 참고. **다음 `[DESIGN]` 항목은 `#91`**(가공대/제련로/조리대/조리용
  화로 4개 오브젝트 그림 + 가공물/완성품/가공식품 아이콘 일괄 제작 — 지금 4개
  오브젝트 전부 임시 단색 사각형 상태). **다음 `[QA]` 항목은 `#92`**(전체 스윕,
  바퀴 139에서 이미 큐에 등록돼 대기 중 — 별도 QA 하네스 세션 몫이라 이번 바퀴도
  건드리지 않았다). **BUILD/DESIGN 완료 카운터는 바퀴 139(#86)에서 5에 도달해 0으로
  리셋된 뒤 140(#87)=1 → 141(#88)=2 → 142(#89)=3 → 이번(#90)=4가 됐다** — 다음 QA
  스윕 자동 등록까지 1개 더 필요(`#91` 완료 시 5). **참고**: 밥/익힌고기도 화약/탄약과
  마찬가지로 아직 전용 아이콘이 없어 핫바/인벤토리에 텍스트로만 표시된다 — 결함이
  아니라 `#91`이 처리할 범위(INBOX 원문이 명시). **조리대(`cooking_table.gd`)는
  `RECIPES`가 빈 배열이라 좌클릭하면 레시피 없는 빈 창만 열린다 — 결함이 아니라
  채소 아이템 추가 후 별도 지시로 조합 요리 레시피를 채울 자리다(위 "마지막 갱신"
  2번 참고).**
