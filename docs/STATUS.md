...
- **(2026-09-04 갱신, 바퀴 155) INBOX #102([QA] 전체 스윕) 완료 — 결함 1건 발견,
  `#112`([BUILD])로 티켓 발행.** 위 "마지막 갱신" 참고. **INBOX에 남은 미완료
  항목(번호 순): `#103`([BUILD] 모래/구리광석 채광 포인트, 다음 BUILD 차례) →
  `#104`([DESIGN], #103 완료 후) → `#105`/`#106`(BUILD, 제련로·가공대 레시피 확장)
  → `#107`(DESIGN) → `#108`~`#110`(BUILD, 강철 도구/건축/전기 계열) → `#111`(DESIGN)
  → **`#112`([BUILD] 저장 상자/가공대/제련로/조리대/조리용 화로 5개 창을 열면 플레이어가
  창 위에 겹쳐 그려지는 버그 — 다음 BUILD 세션이 `#103`보다 먼저 처리할지, 순서대로
  갈지는 번호 순서를 따르면 `#103`이 먼저다).** **BUILD/DESIGN 완료 카운터는 #98에서
  5 도달로 0 리셋된 뒤 #99=1→#100=2→#101=3이었고, 이번 바퀴(#102)는 QA라 카운터에
  반영되지 않는다 — 여전히 3, 5 도달까지 2개 남음.**
