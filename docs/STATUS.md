...
- **PixelLab 잔액이 여전히 $0.00다 (바퀴 57, 58, 59 세 번 확인).** 다음 바퀴가 세션을
  시작하면 **가장 먼저 `GET https://api.pixellab.ai/v1/balance`로 잔액을 다시 확인할
  것** — 사용자가 충전했다면 값이 바뀌어 있을 것이다. 여전히 0이면 INBOX #48을 시도하지
  말고(아래 이유 참고) STATUS.md에 그 사실만 기록한 뒤 세션을 마칠 것(불필요한 반복
  확인/시행착오로 예산을 쓰지 말 것). 순수 좌표/합성 편집(PIL)만으로 고칠 수 있는
  문제는 바퀴 56(낚싯대)·57(총구 불꽃)처럼 계속 진행 가능하다.
