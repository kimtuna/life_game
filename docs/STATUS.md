...
- **(바퀴 89 갱신, 최우선) 다음 미완료 항목은 INBOX #62(`[BUILD]` 핫바 UI 텍스트
  겹침)다 — `[BUILD]` 하네스 몫.** 이어서 #63(`[QA]` 전체 스윕, BUILD/DESIGN 카운터
  5 도달로 자동 등록됨)이 대기 중이다. `[DESIGN]` 하네스가 다시 열릴 차례가 오면
  #62/#63 이후 새 `[DESIGN]` 태그 항목이 등록될 때까지 처리할 것이 없다. 세션 시작 시
  잔액 확인은 여전히 습관적으로 할 것 — 바퀴 89 종료 시점 SpriteCook 16크레딧(그대로,
  이번엔 SpriteCook 호출 안 함), PixelLab $0.0(그대로, 바퀴 89가 재확인함).
...
- **BUILD/DESIGN 완료 카운터: 바퀴 83(#55)으로 5에 도달해 INBOX #56(QA 전체 스윕)을
  ...
  #63(QA 전체 스윕)을 큐에 추가하고 0으로 리셋했다.** 다음 BUILD/DESIGN 5개 완료 시
  다시 QA 스윕을 큐에 추가할 것(PROMPT_BUILD.md ③ / PROMPT_DESIGN.md ③과 동일 규칙).
