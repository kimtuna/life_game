...
바퀴 186 / 2026-09-05 ([BUILD] **INBOX #133 완료 — BUILD_GRID_SIZE(충돌/방 감지)와
벽/문/창문 스프라이트 표시 크기를 16.0/32.0(분리) → 32.0(단일) 하나로 다시 합쳐,
인접 배치 시 화면상 빈틈/겹침 착시를 없앰.**)
1) 원인 재확인: #132가 도입한 `BUILD_STRUCTURE_VISUAL_SIZE`(32) 방식은 충돌 격자
   (`BUILD_GRID_SIZE`=16)와 시각 크기(32)가 2배 차이나서, 격자 기준으로는 딱 붙게
   설치한(비겹침) 벽도 화면(32px)에서는 서로 겹쳐 보이고, 반대로 화면상 안 겹치게
   보이도록 한 칸 띄워 설치하면 실제 격자에는 빈 칸이 생겨 플레이어가 지나가고 방도
   안 막혔다. 사용자 스크린샷으로 재확인된 내용 그대로였다.
2) `world.gd`의 `BUILD_GRID_SIZE`를 16.0 → **32.0**으로 되돌리고, 별도였던
   `BUILD_STRUCTURE_VISUAL_SIZE` 상수는 완전히 제거했다 — 벽/문/창문 원본 아이콘이
   전부 32×32(Python PIL로 재확인)라서 32.0 하나로 두면 `sprite.scale`이 정확히
   1.0이 되는 동시에 `CollisionShape2D.shape.size`와도 1:1로 맞아떨어진다(잔디 타일
   64×64의 약수라 격자 정렬도 유지됨). `_update_build_ghost()`/`_spawn_structure()`의
   scale 계산 두 곳(각각 `BUILD_STRUCTURE_VISUAL_SIZE` 참조하던 자리)을
   `BUILD_GRID_SIZE`로 교체했다.
3) `door.gd`의 `setup(col, sprite, visual_size)` 세 번째 인자에 이제 `BUILD_GRID_SIZE`를
   그대로 넘긴다(값의 의미가 다시 격자 크기와 같아졌으므로) — 파라미터/필드 이름
   (`visual_size`/`_visual_size`)은 "문이 보이는 크기에 비례해 슬라이드"라는 의도를
   그대로 표현하므로 되돌리지 않고 유지, 주석만 갱신했다.
4) `ROOM_FLOOD_CELL_CAP`을 6400 → **1600**으로 낮췄다 — 격자 한 칸의 물리적 면적이
   4배(16→32, 한 변 2배) 커졌으므로 기존과 같은 최대 방 면적(칸수×칸면적 불변,
   400×64² = 6400×16² = 1,638,400)을 유지하려면 칸 수 상한을 1/4로 줄여야 한다
   (6400/4=1600, 32²×1600=1,638,400로 검산 일치).
5) `game/qa/inbox133_check.gd`를 새로 만들어 검증: (a) `BUILD_GRID_SIZE==32` 및 벽의
   시각 크기(`texture.get_size()*scale`)와 `CollisionShape2D.shape.size`가 정확히
   같은 값인지 코드로 확인, (b) 격리된 좌표(230000,230000)에 벽 5개를 격자상 인접
   (비겹침) 배치하고 이웃 벽끼리 `global_position`+half-width로 화면 픽셀 간격을
   계산해 gap이 정확히 0인지 4쌍 전부 확인, (c) 그 벽 라인 왼쪽에서 오른쪽으로
   `_is_position_blocked()`를 40스텝 반복 호출해 플레이어가 실제로 통과 못 하는지
   확인, (d) 별도 좌표(240000,240000)에 벽 4x4 둘레로 방을 만들어 `room_id != -1`
   회귀 확인, (e) 벽 라인 옆 스크린샷 1장 + 방 내부 스크린샷 1장. `project.godot`의
   `[autoload]`에 임시 등록 → `godot --path .`(실제 렌더러)로 실행 →
   `QA_INBOX133_CHECK_PASS` 확인(콘솔에 `PROBE seam gap[...]=0.0` 4줄,
   `player_x_after_push`가 벽 라인 오른쪽 경계를 못 넘음, `room_id=1 판정=잡실`을
   전부 확인) → `project.godot` 원상복구(`git diff`로 빈 diff 재확인). 캐릭터/인벤토리
   세이브도 실행 전 `/tmp/save_backup_133/`에 백업했다가 실행 후 그대로 복원했다.
6) 스크린샷을 Read로 직접 열어 확인 — 벽 5개가 이음매 없이 하나의 연속된 벽처럼
   붙어 보이고(#132 때처럼 겹쳐 보이거나 틈이 벌어지는 착시 없음), 3x3 벽 링 내부에
   플레이어를 세운 스크린샷도 사방이 자연스러운 비율(캐릭터 키 대비)로 막혀 있는
   방처럼 보였다.
7) `docs/feedback/INBOX.md`의 `#133`을 `[x]`로 갱신했다. **BUILD/DESIGN 완료
   카운터**: 직전 `#132`(BUILD)로 카운터가 1이었으므로, 이번 `#133`(BUILD)으로
   **2**가 됐다(전체 스윕까지 3개 남음).

**다음에 할 것**: `docs/feedback/INBOX.md` "처리 대기"에 남은 미완료 항목(번호 순)
— `#134`([DESIGN], 벽/문/창문을 "서 있는 벽"처럼 다시 그리기, #133 이후 진행 지시됨,
[BUILD] 하네스는 건드리지 않음) → `#135`([BUILD], 시야/전장의 안개, #133 이후 진행
권장 — 이번에 벽 경계가 정확해졌으니 바로 착수 가능). 다음 `[BUILD]` 바퀴는 `#135`를
집도록, `[DESIGN]` 바퀴는 `#134`를 집도록 순서가 자연스럽게 잡혀 있다. 참고로 아직
남아있는 옛 관찰 후보(#132가 남긴 것, 이번 바퀴 범위 밖이라 손대지 않음): 격리된 먼
좌표에서 극단적인 조준 각도로 스크린샷을 찍을 때 캐릭터 머리가 몸통과 분리돼 보이는
현상(`/tmp/qa132/01_player_beside_wall.png`, 이번 `/tmp/qa133/` 캡처들은 정상이었음
— 마우스가 원점 근처에 고정된 채 캐릭터만 아주 먼 좌표에 있는 극단 케이스에서만
재현되는 것으로 추정) — 다음 QA/DESIGN이 원점 근처 정상 좌표에서 재현되는지 확인해볼
가치가 있다는 판단은 여전히 유효.

## 지난 바퀴 기록 (바퀴 185, 그대로 보존)

바퀴 185 / 2026-09-05 ([BUILD] **INBOX #132 완료 — 벽/문/창문 스프라이트의 시각적
크기를 BUILD_GRID_SIZE(충돌/방 감지 해상도, 16)에서 분리해 별도 상수로 32px에
고정.**)
1) `world.gd`에 `BUILD_STRUCTURE_VISUAL_SIZE := 32.0` 상수를 새로 추가했다 — 벽/문/
   창문 아이콘 원본이 전부 32×32라서(Python PIL로 직접 확인) 이 값을 32로 두면
   scale이 정확히 1.0이 되어 확대/축소 없이 가장 선명하게 그려진다(임의 비율을
   새로 짓지 않고 이미 검증된 아이콘 해상도를 그대로 채택). `_update_build_ghost()`
   (world.gd:860 근처)와 `_spawn_structure()`(world.gd:984 근처)의 `sprite.scale`
   계산에서 `BUILD_GRID_SIZE`를 이 새 상수로 교체했다 — `BUILD_GRID_SIZE`(16)는
   `_grid_occupancy`/방 감지(flood-fill)/`CollisionShape2D.shape.size`에는 그대로
   남아있어 충돌·방 인식 해상도는 전혀 건드리지 않았다(원문이 요구한 대로).
2) `door.gd`의 `setup(col, sprite, grid_size)` 세 번째 인자가 실제로는 문이 열릴 때
   슬라이드하는 거리(`_sprite.position.x = _grid_size * 0.35`)를 정하는 값이라,
   격자 크기 대신 이제 문의 실제 시각 크기(`BUILD_STRUCTURE_VISUAL_SIZE`)를 받도록
   바꿨다 — 인자/필드 이름도 `grid_size`/`_grid_size`에서 `visual_size`/`_visual_size`로
   바꿔 의미를 명확히 했다(스스로 판단해서 고침, 원문에 없던 리네임이지만 값의 실제
   의미가 격자 크기가 아니라 시각 크기이므로 이름이 그대로면 다음 바퀴가 헷갈릴
   위험이 있었음).
3) `game/qa/inbox132_check.gd`를 새로 만들어 검증: (a) `_spawn_structure()`가 만든
   벽의 `texture.get_size() * scale`이 정확히 (32,32)인지 코드로 확인, (b) 격리된
   좌표(220000,220000)에 벽 4x4 둘레+문 1개로 작은 방을 만들고 플레이어를 옆에 세워
   스크린샷 2장을 찍음, (c) 배치 모드 고스트 미리보기도 같은 32px로 계산되는지 확인
   후 스크린샷 1장, (d) 벽으로 둘러싼 공간이 여전히 방(`room_id != -1`)으로 인식되는지
   회귀 확인. `project.godot`의 `[autoload]`에 임시 등록 → `godot --path .`(실제
   렌더러)로 실행 → `QA_INBOX132_CHECK_PASS` 확인 → `project.godot` 원상복구(diff
   없음 재확인, 백업 파일과 `diff`로 직접 대조). 캐릭터/인벤토리 세이브도 실행 전
   `/tmp/save_backup_132/`에 백업했다가 실행 후 그대로 복원했다.
4) 스크린샷을 Read로 직접 열어 확인 — 벽이 이제 나무판자 결이 보이는 정사각형
   타일로 또렷하게 그려지고(16px 시절의 "손톱만 한 갈색 점"과 다름), 캐릭터 옆에서
   비교했을 때 벽 하나가 캐릭터 키의 대략 1/3 정도로 자연스러운 비율로 보였다.
   고스트 미리보기도 실제 설치물과 동일한 32px로 반투명하게 표시됨을 확인했다(원문
   요구사항 "미리보기와 실제 결과가 다르게 보이지 않게").
5) **범위 밖 관찰(고치지 않음, 기록만)**: 검증 스크린샷에서 캐릭터의 머리가 몸통과
   분리된 채 벽 위쪽 한참 위에 떠 있고 가느다란 살구색 선(목?)이 몸통까지 이어지는
   기이한 렌더링이 보였다(`/tmp/qa132/01_player_beside_wall.png`, 확대 크롭으로
   확인). 이번 코드 변경 때문인지 확인하려고 **기존(수정 전) `inbox125_check.gd`를
   그대로 재실행**했더니 똑같은 현상이 나왔다 — 즉 이번 #132 변경과 무관한 기존
   현상이다. 격리된 원점에서 아주 먼 좌표(22만 단위)에 캐릭터를 두고 마우스 좌표는
   원점 근처로 고정된 채라 조준 각도가 극단적으로 치우치는 테스트 전용 상황이라서
   생기는 것으로 추정되며, 실제 플레이(원점 근처 정상 좌표)에서도 재현되는지는
   확인하지 못했다. #132 범위가 아니므로 고치지 않고 다음 QA/DESIGN 바퀴가 원점
   근처 정상 좌표에서 같은 방향(캐릭터가 화면 왼쪽 위, 즉 마우스가 플레이어 기준
   좌상단 방향을 조준할 때)으로 재현되는지 확인해볼 가치가 있다.
6) `docs/feedback/INBOX.md`의 `#132`를 `[x]`로 갱신했다. **BUILD/DESIGN 완료
   카운터**: 직전 전체 스윕(`#131`)이 5개 도달로 카운터를 0으로 리셋했으므로, 이번
   `#132`로 카운터는 1이 됐다(전체 스윕까지 4개 남음).

**다음에 할 것**: `docs/feedback/INBOX.md` "처리 대기"에 미완료(`- [ ]`) 항목이
**하나도 없다**(`#132`가 마지막 남은 항목이었음). `loop.sh` 규칙대로 다음 바퀴는
세션을 새로 열지 않고 스스로 종료할 것으로 예상된다 — 사람이 새 지시를 INBOX에
추가해야 루프가 다시 돈다. 위 5) "범위 밖 관찰"(캐릭터 머리/몸통 분리 렌더링,
`/tmp/qa132/01_player_beside_wall.png`)은 다음에 사람이 새 QA/DESIGN 항목을 넣을 때
참고할 후보로 남겨둔다.

## 지난 바퀴 기록 (바퀴 184, 그대로 보존)

바퀴 184 / 2026-09-05 ([QA] **INBOX #131 완료 — 전체 스윕, 결함 1건 발견(#132로
티켓 발행).**)
1) `game/qa/full_sweep.gd`를 재사용하되, 직전 5개(#125~#130 — 건축 격자 크기 축소/
   방 오버레이 UI/HUD 정리/건설 해제 모드+확장)가 실제로 화면에 나타난 적이
   한 번도 없어서 새 스텝 5개(`hud_no_tool`/`room_overlay_view`/
   `deconstruct_wall_highlight`/`deconstruct_wall_removed`/`deconstruct_chest_blocked`)
   를 추가했다. 캐릭터/저장 세이브(`{characters,inventory}.save`)를 지우고 처음부터
   `godot --path .`(헤드리스 아님, 실제 렌더러)로 56장을 캡처해 전부 Read로 직접 봤다.
2) **결함 1건 발견 → #132로 티켓 발행**: INBOX #125가 `BUILD_GRID_SIZE`를 64→16으로
   줄이면서, 벽/문/창문 스프라이트 scale(`world.gd:984`, `_spawn_structure()`의
   `sprite.scale = Vector2(BUILD_GRID_SIZE, BUILD_GRID_SIZE) / tex_size`)이 그 값에
   그대로 1:1로 묶여 있어서 실제 화면에 **16×16px**로만 그려진다 — 캐릭터 키(약
   102px)의 1/6도 안 되고 DESIGN.md "최소 32px" 기준에도 못 미친다. `build_wall_ghost`/
   `build_wall_placed`/`build_door_ghost`/`deconstruct_wall_highlight` 캡처(`/tmp/qa67/
   40~44`, `50`)에서 캐릭터 옆에 손톱만 한 갈색 사각형 하나만 겨우 보이는 것을, 그리고
   `room_with_table`(45번)에서는 방을 이루는 벽 4개가 가공대 스프라이트에 완전히 가려
   화면에서 아예 안 보이는 것을 눈으로 직접 확인했다. 새 그림이 필요한 문제가 아니라
   격자(충돌/방 감지) 해상도와 스프라이트 시각 크기가 한 상수에 묶여 있는 게 원인이라
   `[BUILD]`로 등록했다(#132).
3) **QA 스크립트 자체의 버그도 하나 발견해서 고쳤다(게임 결함 아님)**: 새로 추가한
   `room_chest_craft_start` 검증에서 상자를 `_build_room_center_world + Vector2(10, 10)`
   에 놓던 기존(#124) 오프셋이, #125로 격자 반칸이 64→8로 줄어들면서 방(1칸) 경계를
   넘어 이웃 칸(방 밖)에 상자를 놓아버려 재료 소모가 조용히 실패하는 것처럼 보였다
   (`wood in chest(after click)`가 그대로 10, `batch_active=false`). 오프셋을 `(2, 2)`로
   줄여 같은 칸 안에 확실히 들어오게 고치자 정상 동작(10→8, `batch_active=true`,
   `plank` 획득)을 확인했다 — **다음 바퀴가 참고할 점**: `BUILD_GRID_SIZE`가 또 바뀌면
   `full_sweep.gd`의 격자 관련 좌표 오프셋(`BUILD_WALL_TEST_POS`/`BUILD_ROOM_TEST_POS`
   자체는 셀 단위 변환 함수를 거치므로 안전하지만, `Vector2(2, 2)` 같은 절대 오프셋은
   반칸(`BUILD_GRID_SIZE/2`)보다 작아야 한다는 걸 다시 확인할 것.
4) 그 외 새 스텝은 전부 정상 확인됨: HUD 정리(#127)는 총을 안 들면 탄약 패널이
   숨고(`48_hud_no_tool.png`) 총을 들면 다시 보이며(`07_tool_gun_idle.png`, "탄약:
   기본탄 8/8"), 인벤토리 요약 패널은 화면 어디에도 없다. 방 오버레이(#126)는 육안으로는
   거의 안 보였지만(알파 0.3~0.35로 의도된 은은함) Python/PIL로 오버레이 켜기 전/후
   스크린샷을 픽셀 단위로 비교해서 화면 전체에 회색(잡실/방없음) 틴트가, 방-상자
   테스트 방 중앙에는 파란("제작소") 틴트가 정확히 실제로 그려지고 있음을 확인했다 —
   결함 아님, 의도대로 동작. 건설 해제(#128~#130)는 벽 위에서 X 모드로 빨간 하이라이트가
   뜨고(`50`) 클릭하면 `wood_wall`이 100% 환급되며(`deconstruct_wall_removed:
   wood_wall before=0 after=1`), 내용물이 있는 상자는 하이라이트만 뜨고 클릭해도
   철거되지 않는 것(`deconstruct_chest_blocked: chest still valid=true is_empty=false`)
   까지 확인했다.
5) `project.godot`의 `[autoload]` 임시 추가분은 실행 후 되돌렸다(diff 없음 확인).
   `game/qa/full_sweep.gd`는 새 스텝 5개를 포함해 커밋했다. `docs/feedback/INBOX.md`의
   `#131`을 `[x]`로 갱신하고 `#132`(BUILD 티켓)를 새로 추가했다 — 전체 스윕은 좁은
   범위 QA와 달리 결함을 찾아도 "QA 재확인" 루프를 다시 추가하지 않는다
   (`loop/PROMPT_QA.md` ③ 규칙, 5개마다 자동으로 다시 도는 스윕이 대신함).

## 지난 바퀴 기록 (바퀴 183, 그대로 보존)

바퀴 183 / 2026-09-05 ([BUILD] **INBOX #130 완료 — 건설 해제 모드(#128)를 상태를
가진 다른 설치물(상자/제작대류/밭/목장)까지 확장.**)
1) `world.gd`의 `_try_deconstruct_structure()`를 두 갈래로 나눴다: 벽/문(`_grid_occupancy`
   에 있으면)은 기존 로직 그대로 `_deconstruct_wall_or_door(cell)`로 옮기고, 없으면
   새 `_find_fixture_at_cell(cell)`로 상자/제작대류(가공대/제련로/조리대/조리용
   화로)/밭/목장 중 그 칸을 차지하는 게 있는지 찾아 `_deconstruct_fixture(node)`로
   철거한다. `_find_fixture_at_cell()`은 #129가 만든 `_occupancy_footprint()`(각
   오브젝트의 실제 시각적 범위)를 그대로 재사용하되, 채집/채광 포인트(ResourcePoint)는
   이번 항목 범위가 아니므로 타입 필터에서 제외했다.
2) 철거 가능 조건은 INBOX #130 원문대로 종류별로 다르게 판정하는 `_can_deconstruct_fixture()`
   를 추가했다: 상자=슬롯 전부 비어있을 때만(새로 추가한 `storage_chest.gd`의
   `is_empty()`), 제작대류=진행 중인 배치가 없을 때만(기존 `is_batch_active()`
   재사용), 목장=사육 중인 동물이 없을 때만(새로 추가한 `ranch_zone.gd`의
   `has_animal()` — Fence(StaticBody2D)/Prompt(Label)는 CharacterBody2D가 아니라서
   자연스럽게 제외되고 `_release_one()`이 붙인 사슴만 걸린다), 밭=언제든 가능(자라는
   중/수확 가능한 작물도 함께 사라짐 — 원문이 명시한 의도된 단순화, 되돌려주지 않음).
   철거가 막힌 경우에도 원문 요구대로 빨간 하이라이트 자체는 그대로 유지되고
   (`_update_deconstruct_highlight()`가 실제 철거 가능 여부와 무관하게 항상 대상
   노드를 하이라이트함), 좌클릭만 조용히 무시된다.
3) 하이라이트가 씌울 시각 노드를 찾는 부분도 확장했다 — 기존 `_get_structure_sprite()`
   (벽/문의 자식 Sprite2D만 찾음)는 그대로 두고, 새 `_fixture_visual()`이 상자
   (`Placeholder` ColorRect)/제작대류(`Sprite`)/밭(`Soil`)/목장(`Pasture`)별로 다른
   자식 노드 이름을 찾아 반환하게 하고, `_deconstruct_visual()`이 이 둘을 하나로
   묶어 위임한다(어느 쪽 타입이든 같은 코드로 하이라이트/원복 처리).
4) **철거 성공 시 인벤토리로 돌아오는 아이템은 없다 — 스스로 판단.** INBOX #130 원문은
   "#128과 동일(100% 환급)"이라고 적었지만, 실제로 확인해보니 상자/제작대류/밭/목장은
   벽/문(크래프팅해서 손에 들고 설치하는 아이템)과 달리 `world.gd`가 스폰 시점에
   고정 배치하는 오브젝트라 대응하는 인벤토리 아이템 자체가 애초에 없다(`ITEM_LABELS`
   에 "storage_chest" 같은 키가 없음, grep으로 확인). DESIGN.md에 없는 새 아이템을
   이 항목에서 임의로 지어내는 대신(PROMPT_BUILD.md ④ 규칙 — 지어내지 않고 여기에
   기록), 철거하면 그냥 사라지고 환급되는 아이템 없이 처리했다. 다음에 이 부분을
   다르게 만들고 싶으면(예: 실제로 상자를 재설치 가능한 아이템으로 만들기) 별도
   지시가 필요하다.
5) **QA 검증 중 실제 버그를 하나 발견해서 같이 고쳤다.** `_deconstruct_fixture()`가
   `node.queue_free()` 직후 바로 `_recompute_rooms()`를 불렀는데, `queue_free()`는
   트리에서 즉시 빠지는 게 아니라 그 프레임 끝에 지연되기 때문에, `_classify_room()`
   의 `get_children()` 스캔이 막 지우기로 예약된 가공대를 여전히 카운트해서 방
   카테고리가 "제작소"에서 안 바뀌는 걸 QA 스크립트로 직접 재현했다 — `remove_child(node)`
   로 먼저 트리에서 동기적으로 떼어낸 뒤 `queue_free()`하도록 고쳐서 해결했다(벽/문
   철거는 `_grid_occupancy`만 보고 판정해서 이 문제가 없었음 — `_classify_room()`이
   벽/문 자체를 스캔하지 않기 때문).
6) 새 QA 스크립트 `game/qa/inbox130_check.gd`로 검증: 격리된 좌표(700000,700000
   근방)에서 벽 4개로 작은 방을 만들고 가공대를 넣어 "제작소" 판정을 먼저 확인한 뒤,
   (a) 배치 진행 중(`_batch_remaining=3`)인 가공대는 철거가 막히고 방 카테고리도
   그대로인지, (b) 배치가 끝나면 철거되고 방이 "잡실"로 재계산되는지, (c) 내용물이
   있는 상자는 철거가 막히고 비우면 철거되며 인벤토리에 아이템이 새로 생기지
   않는지, (d) 동물이 있는 목장은 철거가 막히고 빼면 철거되는지, (e) 자라는 중인
   밭도 항상 철거되는지, (f) X로 모드 켜기/끄기가 정상 동작하는지까지 9개 체크
   전부 통과했다. `project.godot`의 `[autoload]`에 `QAInbox130Check`를 임시 등록 →
   `godot --path .`(실제 렌더러)로 실행 → `QA_INBOX130_CHECK_PASS`까지 확인 →
   `project.godot` 원상복구(diff 없음 확인). 스크린샷 두 장
   (`/tmp/qa130/00_crafting_station_blocked_highlight.png`,
   `01_chest_blocked_highlight.png`)을 Read 도구로 직접 열어, 배치 진행 중인 가공대와
   내용물 있는 상자가 실제로 선명한 빨간색으로 덧칠된 것을 눈으로 확인했다.
7) 회귀 확인: `inbox128_check.gd`(9개 체크)/`inbox129_check.gd`(4개 체크)/
   `inbox122_check.gd`(방 감지 4단계)/`inbox123_check.gd`(방-상자 자동 연동 4단계)를
   전부 다시 실행해 이번 리팩터링(`_get_structure_sprite`→`_deconstruct_visual` 위임
   구조 변경, `_try_deconstruct_structure` 분리)이 기존 동작을 깨지 않았음을 확인했다.
8) 이번 항목은 새 아이템 종류를 추가하지 않았으므로(4)에서 설명한 대로 애초에 대응
   아이템이 없음) 테스트 상자 갱신은 필요 없다. `docs/feedback/INBOX.md`의 `#130`을
   `[x]`로 갱신, `game/qa/inbox130_check.gd`와 그 짝 `.uid`를 커밋에 포함시켰다.
   **BUILD/DESIGN 완료 카운터**: 직전 전체 스윕(`#124`) 이후 카운터는 `#125`~`#129`로
   5에 도달해 이미 `#131`(QA 전체 스윕)이 큐에 추가되고 0으로 리셋된 상태였다
   (바퀴 182 기록 참고) — 이번 `#130`으로 카운터는 1이 됐다(전체 스윕까지 4개 남음).

**다음에 할 것**: 다음 미완료 항목은 `#132`([BUILD] 벽/문/창문 스프라이트가
`BUILD_GRID_SIZE`(16)에 1:1로 묶여 16×32px밖에 안 되는 문제 — 위 "마지막 갱신" 2)
참고, `world.gd:984`/`860` 근처). 격자(충돌/방 감지) 해상도 16은 유지한 채 시각적
스프라이트 크기만 최소 32px 이상으로 키울 것. BUILD/DESIGN 완료 카운터는 `#131`이
QA라 카운터에 포함 안 됨(직전 `#125`~`#130`으로 5에 도달해 이미 리셋된 상태 —
`#132`가 이번 사이클의 첫 항목).

## 지난 바퀴 기록 (바퀴 182, 그대로 보존)
...
1) **시작할 때 이전 바퀴가 이미 이 항목 대부분을 코드로 다 짜놓고 커밋 전에 죽어있는
   상태를 발견했다** — `game/scenes/world/world.gd`/`world.tscn`/`game/scripts/door.gd`에
   미커밋 변경(`_deconstruct_mode` 상태 + X키 토글 + `_update_deconstruct_highlight()`
   + `_try_deconstruct_structure()` + `DeconstructModePanel` HUD)이 있었고, 검증용
   `game/qa/inbox128_check.gd`(untracked)까지 이미 작성돼 있었다. 새로 짜는 대신
   내용을 읽고 검증부터 시작했다(중복 작업 방지).
2) `project.godot`의 `[autoload]`에 `QAInbox128Check`를 임시 등록 →
   `godot --path .`(실제 렌더러)로 실행 → `QA_INBOX128_CHECK_PASS`까지 9개 체크
   전부 통과(모드 켜기/빈 칸 무하이라이트/벽 하이라이트/벽 철거+환급/문 철거/모드
   끄기/평소 문 토글 회귀 없음/배치 모드와의 상호 배제/철거 후 방 재계산) → `project.godot`
   원상복구(diff 없음 확인).
3) **스크린샷을 눈으로 봤을 때 벽이 안 보이는 문제를 발견해서 QA 스크립트 자체를
   고쳤다.** 기존 `_point_mouse_at`(#119~#121의 마우스 고정 오프셋 트릭)은 카메라를
   `목표 칸 - offset`에 두는데, `offset`이 테스트 시작 시점의 실제 마우스 화면 위치에
   좌우돼서 이번엔 그 값이 커 목표 칸이 뷰포트 밖으로 밀려나 스크린샷이 완전히
   빈 배경만 보였다(프로그램적 modulate 값 검사는 그래도 통과했었음 — 코드 로직은
   맞았지만 "눈으로 확인"이 안 되는 상태였다). `Input.warp_mouse(뷰포트 중앙)`으로
   마우스를 화면 정중앙에 고정해 offset을 0에 가깝게 만들어서, 목표 칸이 항상 화면
   중앙에 렌더링되도록 고쳤다. 재실행 후 Read 도구로 `/tmp/qa128/00_wall_highlighted.png`를
   직접 열어 확인 — 벽 아이콘이 선명한 빨간색(픽셀 `(163,40,24)`)으로, 같은 화면의
   문 아이콘은 평소 갈색(`(108,75,44)`)으로 뚜렷이 구분됨을 확인했다(Python/Pillow로
   `wood_wall.png` 원본 텍스처 색 `(160,112,68)`과도 대조). `01_wall_removed.png`에서는
   철거 후 벽이 화면에서 사라지고 핫바에 `나무벽 x1`이 환급된 것도 확인했다.
4) **QA 검증 중 이 항목 자체와 무관한 실제 결함을 하나 더 찾아 같이 고쳤다.**
   `farm_plot.gd`/`resource_point.gd`/`ranch_zone.gd`/`storage_chest.gd`/
   `crafting_station.gd`(가공대/제련로/조리대/조리용 화로 공용 베이스) 5개 스크립트가
   전부 `world.gd`와 별개의 독립적인 `_unhandled_input`으로 좌클릭을 직접 받고
   있는데, 그중 어느 것도 건설 해제 모드를 확인하지 않았다 — 즉 건설 해제 모드를
   켠 채로 밭/채집포인트/목장/상자/제작대 근처에서 좌클릭하면(마침 손에 맞는
   도구/아이템을 들고 있었다면) 벽 철거 시도와 **동시에** 심기/수확/채집/채광/동물
   풀어놓기/상자 열기/제작 창 열기까지 같이 실행돼 버린다(INBOX #128 원문의 "이
   모드가 켜져 있는 동안은 손에 무엇을 들었든 좌클릭이 항상 철거로만 동작해야
   한다"는 의도와 어긋남). `door.gd`가 이미 쓰고 있던 패턴(`if world_ref.
   is_deconstruct_mode_active(): return`)을 5곳 모두에 똑같이 추가해서 막았다.
   수정 후 `inbox128_check.gd`를 다시 돌려 여전히 전부 통과함을 확인했고,
   회귀 확인용으로 `qa/storage_chest_check.gd`도 별도로 재실행해서(건설 해제
   모드가 꺼진 평소 상태) 상자 이전/스택 로직이 그대로 정상 동작함을 확인했다.
5) `qa/processing_table_check.gd`를 같은 김에 돌려봤더니 `world.gd`에 없는
   `_on_craft_pressed()`를 호출하다 실패하는데, 이 스크립트는 이번 바퀴에 건드리지
   않았고 세부 확인 결과 제작 UI가 이후 다른 방식으로 개편되면서 남은 **이전부터
   있던 결함(고아가 된 낡은 QA 스크립트)**으로 보인다 — 이번 항목 범위 밖이라
   고치지 않고 아래 "막힌 것/보류"에 기록만 해서 다음에 정리할 수 있게 남긴다.
6) 이번 항목은 새 아이템 종류를 추가하지 않았으므로(벽/문 자체는 #100/#109에서 이미
   만들어진 기존 아이템, 새로 만든 건 코드 동작뿐) 테스트 상자 갱신은 필요 없다.
   `docs/feedback/INBOX.md`의 `#128`을 `[x]`로 갱신, `game/qa/inbox128_check.gd`와
   그 짝 `.uid`, 그리고 이전 바퀴가 커밋을 놓쳤던 `game/qa/inbox127_check.gd.uid`도
   함께 커밋에 포함시켰다(이미 커밋된 `inbox127_check.gd` 본체와 짝을 맞춤).
   **BUILD/DESIGN 완료 카운터**: 직전 전체 스윕(`#124`) 이후 `#125`=1→`#126`=2→
   `#127`=3 → 이번 `#128`로 **4**가 됐다 — 5 도달까지 1개 남음(다음 BUILD/DESIGN
   완료 시 QA 전체 스윕 항목을 큐에 추가해야 함).

## 어려움 / 에러와 해결 (INBOX #128)

- 마우스 오프셋 트릭으로 카메라를 옮겨 목표 칸에 마우스를 맞췄는데도 스크린샷에는
  아무것도 안 보이는 문제 — 오프셋 값 자체가 테스트 시작 시점 실제 마우스 화면
  좌표에 좌우되어 클 수 있다는 걸 놓쳤다. `Input.warp_mouse()`로 마우스를 뷰포트
  정중앙에 먼저 고정해 오프셋을 0으로 만드는 방식으로 해결(위 3) 참고). **다음
  바퀴가 이 마우스 고정 오프셋 트릭으로 스크린샷까지 찍어야 하면, 처음부터
  `Input.warp_mouse(get_viewport().get_visible_rect().size / 2.0)`을 오프셋 계산
  전에 넣어서 목표 칸이 항상 화면 중앙에 오게 할 것** — 프로그램적 값 검사만으로는
  "눈으로 확인"이 안 되는 사각지대가 생긴다는 걸 이번에 실제로 겪었다.
- 이번 바퀴 시작 시점에 이전 바퀴의 미커밋 작업(코드+QA 스크립트)이 이미 완성도
  높게 남아있었다 — 처음부터 다시 짜지 않고 읽고 검증하는 방향으로 진행해 시간을
  아꼈다. **다음 바퀴도 세션 시작 시 `git status`/`git diff`를 먼저 확인해서 이런
  이어받을 미완료 작업이 있는지 확인할 것** (이번엔 우연히 발견했지만, 매번 확인
  절차로 삼아도 좋다).

**다음에 할 것**: 다음 미완료 항목은 `#129`([BUILD], 건축 겹침 검사를 벽/문뿐 아니라
다른 점유 오브젝트·플레이어 칸까지 확장). 그 뒤로 `#130`(#128 건설 해제를 상자·
제작대류·밭·목장까지 확장)이 큐에 대기 중이다. BUILD/DESIGN 완료 카운터는
4(전체 스윕까지 1개 남음 — 다음 항목 완료 시 QA 전체 스윕 추가 필요).

## 지난 바퀴 기록 (바퀴 180, 그대로 보존)
...
1) 새 스크립트 `game/scripts/room_overlay.gd`(`class_name RoomOverlay`, `Node2D`
   확장, door.gd와 같은 `setup(world, grid_size)` 패턴)를 만들어 `game/scenes/world/
   world.tscn`에 `RoomOverlay` 자식 노드(z_index=10, Ground(-2)/구조물(-1)/플레이어(0)
   보다 위, UI CanvasLayer(layer=10)보다는 아래)로 추가했다. `_process()`에서
   `active`일 때만 매 프레임 `queue_redraw()`를 호출하고(카메라가 플레이어를 따라
   계속 움직이므로 화면에 보이는 칸 범위 자체가 매 프레임 바뀜 — "방 배치가 바뀔 때만
   갱신"은 방 판정 재계산(#122)에 해당하는 얘기지 카메라 추종과는 별개라고 판단),
   `_draw()`에서 카메라 뷰포트(`get_viewport_rect().size`, 프로젝트 기준 해상도
   1280×720 고정 — `settings_data.gd` 주석이 이미 확인해준 대로 `canvas_items` 스트레치
   모드에서는 창 크기와 무관하게 항상 이 값) 범위의 격자 칸을 순회하며 `world_ref.
   get_room_id_at()`/`get_room_category()`(#122/#123이 이미 만든 공개 API 재사용,
   판정 로직 중복 없음)로 방 종류를 물어 `draw_rect()`로 반투명 색을 칠한다. 방
   종류별 색(주방 주황/제작소 파랑/대장간 빨강/연구소 보라/농장 초록/목장 노랑, 전부
   알파 0.35) + 잡실·방없음 회색(알파 0.3) — DESIGN.md 규칙표(2026-09-05 확정)의
   6개 방 종류를 그대로 반영.
2) `world.tscn`의 `UI/HUD`에 `RoomOverlayToggle` 버튼(`toggle_mode=true`)을 우상단
   `TimePanel`/`NetPanel` 아래(offset_top 108~146, 기존 패널들과 8px 간격 패턴을
   그대로 따름 — NetPanel이 멀티플레이 세션에서 보이게 되어도 겹치지 않음)에 추가하고,
   `world.gd`의 `_ready()`에서 `room_overlay.setup(self, BUILD_GRID_SIZE)` +
   `room_overlay_toggle.toggled.connect(_on_room_overlay_toggle_toggled)`로 연결,
   핸들러는 `room_overlay.set_active(pressed)`만 호출하는 얇은 래퍼.
3) 새 QA 스크립트 `game/qa/inbox126_check.gd`로 검증: 격리된 좌표(-200000,-200000
   근방)에 벽 4×4 둘레 + 안쪽 2×2 트인 공간에 가공대를 넣어 "제작소" 방을 만들고,
   토글 버튼의 `button_pressed`를 프로그램적으로 켜서(실제 클릭과 동일하게 `toggled`
   신호가 발생함, `set_pressed_no_signal()`이 아님) `room_overlay.active/visible`이
   true가 되는지 확인 → 방 한가운데/방 바깥(트인 들판)/토글을 다시 끈 상태 세
   스크린샷을 `/tmp/qa126/`에 저장. **주의**: `godot --headless --path .`로 실행하면
   `await RenderingServer.frame_post_draw`가 끝없이 대기하며 타임아웃난다(헤드리스는
   실제 프레임을 그리지 않음) — 기존 결정 로그가 이미 경고한 대로 `godot --path .`
   (실제 렌더러, `--headless` 없이)로 실행해야 스크린샷이 나온다. 이번에 처음으로
   `--headless`로 먼저 시도했다가 60초 타임아웃을 두 번 겪은 뒤에야 이 사실을
   재확인했다 — 다음 바퀴는 스크린샷이 필요한 QA 스크립트를 짤 때 처음부터
   `--headless` 없이 실행할 것.
4) **스크린샷을 눈으로만 봤을 때는 오버레이가 안 보이는 것처럼 착각했다** — 알파
   0.3~0.35가 실제로는 매우 은은해서, 1280×720 축소 이미지를 그냥 보면 배경(회색
   지면)과 거의 구분이 안 갔다. Python(Pillow)으로 같은 좌표의 픽셀 RGB 값을
   직접 비교해서야(오버레이 끄면 (76,76,76), 야외 킨 상태는 (92,92,92) — 회색 오버레이
   계산과 정확히 일치, 방 내부는 (76,99,135) — 파란색으로 뚜렷이 이동) 실제로 정상
   동작함을 확인했고, 이어서 200% 확대 크롭 이미지로도 파란 방 내부와 회색 배경이
   육안으로 뚜렷이 구분되는 것을 재확인했다. **교훈**: 알파값이 낮은(반투명) UI를
   스크린샷으로 QA할 때는 축소된 원본 이미지를 눈으로만 훑고 "안 보인다"고 성급히
   결론 내리지 말고, 픽셀 값 비교나 확대 크롭으로 한 번 더 검증할 것 — 특히 이
   기능은 DESIGN.md가 "완전 불투명이 아니라 알파값을 낮춰서"라고 명시적으로 요구해서
   낮은 알파가 의도된 사양이었다.
5) 이번 항목은 새 아이템 종류를 추가하지 않았으므로 테스트 상자 갱신은 필요 없다.
   `docs/feedback/INBOX.md`의 `#126`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   직전 전체 스윕(`#124`) 이후 `#125`(BUILD)로 1, 이번 `#126`(BUILD)로 **2**가 됐다 —
   5 도달까지 3개 남음.

## 어려움 / 에러와 해결 (INBOX #126)

- 새 스크립트 파일(`room_overlay.gd`, `class_name RoomOverlay`)을 만들고 바로
  `godot --headless --path .`로 실행했더니 `world.gd`에서 "Could not find type
  RoomOverlay in the current scope" 파싱 에러가 났다 — 새 전역 클래스가
  `.godot/global_script_class_cache.cfg`에 아직 등록되지 않아서였다. 기존 결정
  로그들이 이미 여러 번 남긴 교훈과 동일: 새 `class_name`을 추가한 바로 다음 실행
  전에는 항상 먼저 `godot --headless --editor --quit`(또는 `--quit-after 30`)을
  한 번 돌려 전역 클래스 캐시를 갱신해야 한다.
- 위 4)에서 다룬 "헤드리스로는 스크린샷 캡처가 영원히 멈춘다" 문제 — `--headless`
  없이 실제 렌더러로 실행해야 한다는 규칙은 STATUS.md에 이미 10회 넘게 기록돼
  있었는데도 이번에 처음 습관대로 `--headless`를 먼저 썼다가 시간을 낭비했다.
  다음부터는 스크린샷이 필요한 QA 스크립트는 처음부터 `godot --path .`로 실행할 것.

**다음에 할 것**: 다음 미완료 항목은 `#127`([BUILD], HUD 정리 — `AmmoPanel`을 총을
들었을 때만 보이게, `InventoryPanel` 완전 제거). 그 뒤로 `#128`(건설 해제 모드)/
`#129`(건축 겹침 검사 확장)/`#130`(#128 확장)이 큐에 대기 중이다. BUILD/DESIGN
완료 카운터는 2(전체 스윕까지 3개 남음).

## 지난 바퀴 기록 (바퀴 178, 그대로 보존)

바퀴 178 / 2026-09-05 ([BUILD] **INBOX #125 완료 — 건축 격자 크기를 64→16(1/4)으로
축소하고, 그 과정에서 발견된 플레이어-건축물 충돌 판정의 실제 회귀를 함께 고쳤다.**)
1) `game/scenes/world/world.gd`의 `BUILD_GRID_SIZE`를 `64.0` → `16.0`으로 변경.
   근거: 벽 아이콘(32×32)이 격자 크기(64)에 맞춰 2배 확대되면서 플레이어(화면 기준
   약 102px)에 비해 벽이 지나치게 커 보인다는 사용자 스크린샷 지적(INBOX #125 원문).
   고스트 미리보기/벽·문 스프라이트 스케일링/`door.gd`의 `_grid_size`는 전부
   `BUILD_GRID_SIZE`를 그대로 참조하고 있어서 상수 하나만 바꿔도 자동으로 맞춰졌다
   (별도 코드 수정 불필요, 이미 좋은 설계였음).
2) `ROOM_FLOOD_CELL_CAP`을 `400` → `6400`(16배)으로 재조정. 근거: 그리드 한 변이
   1/4이 되면 같은 물리적 면적이 4×4=16배 많은 칸으로 나뉘므로, 기존과 같은 최대
   방 면적을 유지하려면 탐색 상한도 16배로 올려야 한다(INBOX #125 원문이 명시적으로
   요구).
3) **실제 회귀를 발견해서 고쳤다**: `_is_position_blocked()`가 플레이어 위치 주변
   4개 대각선 모서리(`±PLAYER_COLLISION_RADIUS`)만 표본으로 확인하던 방식이었는데,
   `PLAYER_COLLISION_RADIUS`(16, 원래 이 값이었음)와 새 `BUILD_GRID_SIZE`(16)가
   우연히 같아지면서 대각선 모서리가 항상 옆 행/열로 벗어나 버려 **정면으로 다가오는
   벽/문을 아예 감지 못 하는** 실제 버그가 됐다 — 직접 재현: 닫힌 문을 정면으로
   완전히 통과해버림(`inbox120_check.gd` 재실행으로 처음 발견). 표본 점 방식 대신
   플레이어 충돌 사각형이 걸치는 격자 칸 범위 전체를 스캔하는 방식으로 고쳐서,
   격자 크기와 충돌 반경의 비율에 관계없이 항상 정확하게 판정하도록 만들었다.
4) `inbox119_check.gd`의 "벽이 플레이어를 막는지" 검사가 한 번의 500유닛 단일
   이동으로 목적지만 확인하던 방식이라, 그리드가 작아지며 벽 1칸을 통째로 건너뛰어도
   통과 실패로 못 잡을 위험이 커졌다(STATUS.md 바퀴173 결정 로그가 이미 경고했던
   함정과 같은 종류) — `inbox120_check.gd`가 쓰던 검증된 패턴(8유닛씩 80회 나눠
   이동, 실제 물리 프레임 흉내)으로 교체했다.
5) 새 QA 스크립트 `game/qa/inbox125_check.gd`를 만들어 벽 4면+문 1개로 작은 방을
   짓고 플레이어를 옆에 세운 스크린샷(`/tmp/qa125/00_player_inside_room.png`,
   `01_player_beside_wall.png`)으로 직접 확인 — 벽이 캐릭터의 허리~가슴 높이 정도로
   작아져 이전(플레이어와 거의 맞먹던 크기)보다 훨씬 자연스러운 비율이 됐다. 이어서
   기존 `inbox119_check.gd`/`inbox120_check.gd`/`inbox121_check.gd`/`inbox122_check.gd`/
   `inbox123_check.gd`를 전부 다시 실행해(project.godot [autoload]에 하나씩 임시
   등록 → `godot --path .` 실행 → 원상복구 반복) 회귀가 없는지 확인, 전부
   `_PASS`로 통과했다(단 #119는 위 4)의 수정을 반영한 뒤에 통과).
6) 이번 항목은 새 아이템 종류를 추가하지 않았으므로 테스트 상자 갱신은 필요 없다.
   `docs/feedback/INBOX.md`의 `#125`를 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   직전 전체 스윕(`#124`)이 리셋한 뒤 이번 `#125`(BUILD)로 **1**이 됐다 — 5 도달까지
   4개 남음.

## 스스로 판단해서 고친 부분 (INBOX #125)

- `_is_position_blocked()`의 대각선 표본 검사를 격자 범위 전체 스캔 방식으로 바꾼 것과
  `inbox119_check.gd`의 단일 점프 이동 검사를 스텝 이동으로 바꾼 것, 둘 다 INBOX #125
  원문이 직접 요구한 내용은 아니다(원문은 크기 조정과 `ROOM_FLOOD_CELL_CAP`만 명시).
  하지만 그리드를 줄이는 과정에서 실제로 재현되는 충돌 회귀를 발견했으므로, 고치지
  않고 넘기면 "벽을 설치해도 플레이어가 그냥 통과하는" 훨씬 심각한 결함을 새로
  만드는 셈이라 이번 항목 범위에 포함해 함께 고쳤다.

## 어려움 / 에러와 해결 (INBOX #125)

- 위 "스스로 판단해서 고친 부분" 참고 — `PLAYER_COLLISION_RADIUS`와
  `BUILD_GRID_SIZE`가 같은 값(16)이 되면서 기존의 "4개 대각선 모서리만 표본 검사"
  방식이 실패하는 경계 조건에 우연히 정확히 걸렸다. 앞으로 격자/충돌 관련 상수를
  바꿀 때는 두 상수의 비율이 바뀌면서 생기는 이런 표본 검사의 사각지대를 항상
  의심하고, 가능하면 표본 점 대신 범위 스캔 방식을 기본으로 쓸 것.

## 지난 바퀴 기록 (바퀴 177, 그대로 보존)
...
1) `game/scenes/storage_chest/storage_chest.gd`에 `class_name StorageChest`를
   추가하고(월드가 `node is StorageChest`로 타입 판정할 수 있어야 해서, #122가
   `FarmPlot`/`RanchZone`에 했던 것과 같은 패턴), `InventoryData.get_count()`/
   `remove_item()`과 이름·동작을 그대로 맞춘 `get_count(item_name)`/
   `remove_item(item_name, amount)`를 추가했다(기존 `_slots` 배열을 그대로 스캔/차감 —
   새 자료구조 없음).
2) `game/scenes/world/world.gd`에 `get_room_chests(world_pos: Vector2) -> Array`를
   추가했다: `get_room_id_at()`으로 방 ID를 구하고 `get_room_category()`가 "잡실"이면
   즉시 빈 배열(자동 연동 없음), 아니면 `get_children()`을 돌며 `StorageChest`이고
   같은 방 ID인 것만 모아 반환한다. 방 ID는 벽/문이 바뀔 때마다 다시 부여되므로
   캐싱하지 않고 호출될 때마다 새로 계산한다(#122가 남긴 경고 그대로 따름).
3) `game/scripts/crafting_station.gd`의 `start_batch(recipe, qty)`를 확장했다:
   `_room_chests()`(= `world_ref.get_room_chests(global_position)`)로 같은 방 상자
   목록을 구하고, `_available_count()`(인벤토리+상자 합산)로 재료가 충분한지 먼저
   전부 확인한 뒤(하나라도 부족하면 아무것도 소모하지 않고 false — 기존 안전 패턴
   유지), `_consume()`으로 실제 차감한다. **소모 순서는 플레이어 인벤토리를 먼저
   비우고 남는 만큼만 상자에서 뺀다** — DESIGN.md/INBOX 원문에 순서 지정이 없어서
   스스로 판단해 정함(아래 "스스로 판단해서 고친 부분" 참고). `world_ref`가 이미
   모든 `CraftingStation` 서브클래스에 스폰 시점에 주입돼 있어(가공대/제련로/조리대/
   조리용 화로 전부) 이 확장이 4개 모두에 자동으로 적용된다 — 서브클래스 코드는
   전혀 건드리지 않았다.
4) **검증**: 새 QA 스크립트 `game/qa/inbox123_check.gd`(커밋, 재사용 가능)를
   `project.godot [autoload]`에 임시 등록하고(`godot --headless --editor
   --quit-after 30`을 먼저 돌려 `StorageChest` 전역 클래스를 인식시킨 뒤, #120 이후
   반복되는 교훈), `godot --path .`(실제 렌더러)로 실행. (1) 벽 4개로 방을 만들고
   가공대를 넣어 "제작소"로 인식시킨 뒤, 같은 방에 상자를 놓고 `wood` 10개만 채우고
   플레이어 인벤토리는 비운 채로 `start_batch({"wood":2}, 1)`을 호출 — 성공하고
   상자 wood가 10→8로 줄고 인벤토리는 그대로 0 유지됨을 확인. (2) 배치를
   `_advance_batch(CRAFT_SECONDS_PER_UNIT)`으로 강제 완료시키고 `collect_output()`
   호출 → 인벤토리에 plank 1개 확인(기존 타이머+수동 수령 흐름이 그대로 동작). (3)
   상자+인벤토리를 합쳐도 재료가 부족한 레시피(`wood: 999`)는 여전히 실패하는지
   확인(상자가 있다고 무한정 되는 게 아님). (4) **회귀**: 벽으로 안 둘러싼 트인
   자리에 가공대+상자(재료 10개)를 나란히 놓고 `start_batch()` 호출 → 실패하고
   상자 재료도 그대로 10개 유지됨을 확인(거리가 아니라 방 ID 기준이라는 DESIGN.md
   규칙대로, 방이 없으면 자동 연동이 전혀 안 켜짐). 4개 전부
   `QA_INBOX123_CHECK_PASS`로 통과. 검증 후 `project.godot`의 임시 autoload 등록을
   원상복구(diff 없음 확인)하고, 테스트 중 생성된 세이브 파일을 삭제했다. 이번
   항목은 새 아이템/오브젝트 종류를 추가하지 않았으므로 테스트 상자 갱신은
   필요 없다.
5) `docs/feedback/INBOX.md`의 `#123`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**가
   이번 `#123`(BUILD)으로 **5**에 도달해, `loop/PROMPT_BUILD.md` ③ 규칙대로
   `#124 [QA] 전체 스윕`을 새로 추가하고 카운터를 **0**으로 리셋했다.

## 스스로 판단해서 고친 부분 (INBOX #123)

- 재료 소모 순서(플레이어 인벤토리 우선 → 부족분만 상자)를 스스로 정했다.
  DESIGN.md "방-상자 자동 연동" 절과 INBOX #123 원문 둘 다 "상자 내용물도 재료로
  함께 인식해서 쓸 수 있게 한다"까지만 말하고 인벤토리/상자 중 어느 쪽을 먼저
  비우는지는 명시하지 않았다. 인벤토리를 먼저 비우는 쪽을 택한 이유: (1) 상자는
  "여분 비축"이라는 직관에 맞고, (2) 플레이어가 들고 다니는 소량 재료부터 자연스럽게
  줄어드는 편이 상자 슬롯 수가 뒤죽박죽 남는 것보다 상자 UI에서 이해하기 쉽다.
  나중에 사용자가 반대 순서(상자 우선, "손에 든 건 아끼고 상자부터 쓴다")를 원하면
  `crafting_station.gd`의 `_consume()` 안 순서만 뒤집으면 된다(로직이 이미 인벤토리
  블록과 상자 루프로 분리돼 있어 변경 범위가 작음).

## 어려움 / 에러와 해결 (INBOX #123)

- 없음 — #122가 이미 만들어둔 `get_room_id_at()`/`get_room_category()`/
  `_grid_occupancy` 기반 QA 패턴(마우스 오프셋 트릭, `_place_wall()` 헬퍼, 임시
  autoload 등록→해제)을 그대로 재사용할 수 있어서 새로운 함정 없이 한 번에 통과했다.

**다음에 할 것**: 다음 세션은 `#124`([QA] 전체 스윕, `loop/PROMPT_QA.md` 하네스)를
처리한다. 범위: 메인 메뉴부터 시작해 캐릭터 생성/월드 입장/이동/사냥/채집/채광/농사/
목장/제작(가공대·제련로·조리대·조리용 화로)/인벤토리/저장 상자까지 실제로 플레이해
보며 문제를 찾는다. 특히 신경 써서 볼 것: (1) 격자 배치 시스템(#119~#121) — 나무벽/
나무문/석제벽/강철벽/강철문/창문을 실제로 설치해보고 충돌/문 열고 닫기가 자연스러운지.
(2) 방 감지(#122) — 벽 4개로 작은 방을 실제로 지어보고 (가공대만 넣었을 때 "제작소",
두 카테고리를 섞었을 때 "잡실" 등) 판정이 화면상 행동과 일치하는지, 지금은 방 이름이
화면에 안 보이는데 그게 실제 플레이 경험에서 혼란을 주는지도 관찰만 해서 기록(고치는
건 QA 하네스 범위 밖). (3) 방-상자 자동 연동(#123) — 방 안에 가공대+상자를 두고
상자에만 재료를 넣은 채 실제로 UI에서 "제작 시작" 버튼을 눌러보며(이번 바퀴는
`start_batch()`를 코드로 직접 호출해서 검증했지, 실제 제작 UI 클릭까지는 확인하지
않았다 — QA가 이 경로를 마저 확인할 것) 정상 동작하는지, 그리고 방 밖에서는 여전히
상자 재료를 손으로 옮겨야 하는지. 문제를 찾으면 각각 새 INBOX 항목(`[BUILD]`/
`[DESIGN]` 태그)으로 등록하고 뒤에 같은 범위의 QA 재확인 항목을 추가한다(자기
재검증 루프, DESIGN.md "QA 관찰 항목" 절 참고). 문제가 없으면 새 항목을 추가하지
않고 DESIGN.md/STATUS.md에 스윕 완료를 기록한다.

## 지난 바퀴 기록 (바퀴 175, 그대로 보존)
...
1) `scenes/world/world.gd`의 `BUILDABLE_STRUCTURES`에 `"wood_door": {"texture": ...,
   "is_door": true}`를 추가했다. `_spawn_structure()`가 `is_door`를 보고 평범한
   `StaticBody2D` 대신 새로 만든 `scripts/door.gd`(`class_name Door`)를 붙인 노드를
   생성하도록 분기했다 — 그 외 격자 스냅/고스트 미리보기/겹침 검사/인벤토리 소모/
   우클릭·핫바 전환 취소는 #119 프레임워크를 그대로 재사용(새 코드 없음).
2) `scripts/door.gd` 신규 작성. resource_point.gd와 같은 "근처에서 좌클릭" 패턴
   (`_unhandled_input()`이 플레이어와의 거리(`INTERACT_RADIUS := 70.0`)를 확인)으로
   열림/닫힘을 토글하되, 특정 도구를 요구하지 않고 **배치 모드가 아닐 때만** 반응한다
   (`world.is_build_placement_active()`라는 새 공개 접근자로 판정 — INBOX #120 원문의
   "도구가 없거나 배치 모드가 아닐 때" 조건). 토글 시 `CollisionShape2D.disabled`를
   바꾸고(닫힘=활성=못 지나감, 기본값), 새 그림 없이 기존 스프라이트를 반투명(alpha
   0.35)+옆으로 밀린 것처럼(격자 크기의 0.35배 오프셋) 만들어 열림 상태를 표현했다.
3) **문은 열림/닫힘과 무관하게 `_grid_occupancy`에 계속 남아있는다**(INBOX #120/
   DESIGN.md "건축/방 시스템" 2026-09-05 확정 — 나중에 #122 방 감지가 "이 칸에 문이
   있다"는 사실만 보면 되도록). 대신 `_is_position_blocked()`를 고쳐서, 점유된 칸의
   노드가 `Door`이고 `is_open`이면 그 칸은 건너뛰도록(통과 가능) 만들었다 — 격자 점유
   자료구조 자체는 문의 개폐 상태와 분리되고, 실제 통과 가능 여부만 `is_open`을 따로
   확인하는 구조.
4) **검증**: 새 QA 스크립트 `game/qa/inbox120_check.gd`(커밋, 재사용 가능,
   `inbox119_check.gd`와 같은 방법 — 마우스 고정 오프셋 트릭으로 원하는 격자 칸에
   설치)를 `project.godot [autoload]`에 임시 등록하고, 새 스크립트의 `class_name Door`가
   전역 클래스로 인식되게 먼저 `godot --headless --editor --quit-after 30`을 한 번
   돌린(안 돌리면 "Could not find type Door in the current scope" 파싱 에러 — 아래
   "어려움/에러" 참고) 뒤 `godot --path .`(실제 렌더러)로 실행. (a) 좌클릭 설치 시
   인벤토리 정확히 1개 감소 + `_grid_occupancy`에 `Door` 노드 등록, (b) 설치 직후
   기본 상태는 닫힘(`is_open == false`, 충돌 활성), (c) 닫힌 문은 플레이어를 막음,
   (d) 빈손으로 문 근처에서 좌클릭(`Input.parse_input_event()`로 실제 엔진 입력 경로
   흘림)하면 열림으로 토글되고 충돌이 비활성화됨, (e) 열린 뒤에도 `_grid_occupancy`에
   `Door` 노드가 그대로 남아있음(방 감지용 자료구조 보존 확인), (f) 열린 문은 플레이어가
   통과 가능, (g) 다시 좌클릭하면 닫힘으로 복귀, (h) `wood_door`를 다시 손에 든(배치
   모드) 상태에서는 근처 좌클릭이 토글을 일으키지 않음(가드 확인) — 전부
   `QA_INBOX120_CHECK_PASS`로 통과. 닫힌/열린 상태 스크린샷(`/tmp/qa120/01_door_open.png`,
   `02_door_closed.png`)을 Read 도구로 직접 확인 — 닫힘은 불투명한 갈색 문, 열림은
   반투명하게 흐려지고 오른쪽으로 살짝 밀린 모습이 뚜렷이 구별됨. 검증 후
   `project.godot`의 임시 autoload 등록을 원상복구(diff로 원래와 완전히 동일함 확인),
   세이브 파일 없음(애초에 생성 안 됐음, `app_userdata/life_game/`에 `.save` 파일이
   원래 존재하지 않았음을 확인).
5) `docs/feedback/INBOX.md`의 `#120`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#119`(BUILD) 완료 시 1이었음 → 이번 `#120`(BUILD)으로 **2**가 됐다(5 도달까지 3개
   남음). 다음은 `#121`([BUILD], 나머지 벽/문 5종을 #119/#120 프레임워크에 아이템 키만
   추가해서 연결).

**다음에 할 것**: 다음 `[BUILD]` 세션은 `#121`(`stone_wall`/`steel_wall`/`steel_door`/
`window` 4종을 `BUILDABLE_STRUCTURES`에 항목만 추가 — 벽류는 `is_door` 없이, 문류는
`"is_door": true`로. 새 코드 작성은 필요 없어야 정상). 참고할 것: (1) `is_build_placement_active()`
(공개 접근자, world.gd 803번째 줄 부근)와 `Door`(class_name, `scripts/door.gd`)를
그대로 재사용할 것 — 문 종류가 늘어나도 door.gd 자체는 아이템에 무관한 범용 로직이라
수정이 필요 없다. (2) **QA 스크립트에서 플레이어 이동/충돌을 검증할 때
`_move_player_with_grid_collision()`에 큰 이동량(예: 500)을 한 번에 넘기지 말 것** —
이 함수는 이동 시작~끝 사이 "목적지 지점"만 막혔는지 검사하고 중간 경로는 보지
않는다(실제 게임에서는 `_physics_process`가 프레임당 아주 작은 이동만 넘겨서 문제가
없지만, QA에서 한 번에 큰 이동을 흉내내면 벽/문 칸을 그냥 건너뛰어버려 "통과 안 됨"이
거짓으로 통과 처리될 수 있다 — 이번 바퀴가 실제로 처음엔 이 실수로 열린 문 통과 검증이
실패로 나왔다가 원인을 찾아 고쳤다). `_move_player_with_grid_collision()`을 여러 번
작은 간격(예: 8유닛씩 80회)으로 반복 호출하는 이번 `inbox120_check.gd`의 패턴을
재사용할 것 — `inbox119_check.gd`의 500유닛 단일 호출 패턴은 우연히 통과했을 뿐 이
문제를 안고 있었다는 것도 알아두면 좋다(그 스크립트를 그대로 복붙하지 말 것). (3)
`#121` 완료 후 BUILD/DESIGN 카운터는 3이 되어 5 도달까지 2개 남는다.

## 지난 바퀴 기록 (바퀴 172, 그대로 보존)
...
클릭→인벤토리 이전, 인벤토리 공간 부족 시 실패 메시지, 슬롯 많아진 상태의 스크롤)이
그대로 동작하는지 별도로 재검증.)
1) `game/qa/storage_chest_check.gd`(#96)와 `game/qa/inventory_safety_check.gd`(#98)를
   `project.godot [autoload]`에 각각 임시 등록하고 세이브 파일을 지운 뒤 `godot --path .`
   (실제 렌더러)로 재실행해서, #116/#117 이전에 통과했던 상자 상호작용 검증이 지금도
   유효한지 독립적으로 다시 확인했다(자기 채점 방지 — DESIGN.md "QA 관찰 항목" 절 참고).
2) **`storage_chest_check.gd`가 첫 실행에서 FAIL로 나왔다** — 원인을 추적한 결과 게임
   결함이 아니라 **스크립트 자체가 낡은 가정을 갖고 있었다**: 이 스크립트는 "상자에
   wood 120/stone 5를 채운 뒤 정확히 120/5여야 한다"처럼 **절대 수량**을 기대하는데,
   #117 이후 이 테스트 상자는 이미 41종을 999개씩 채운 채로 스폰되므로 add_item()이
   기존 999개 위에 합산돼(예: wood 999+120=1119) 항상 어긋난다. 실제 로그를 보면
   기능 자체(클릭 한 번에 최대 99개 이전, 인벤토리 꽉 찼을 때 거부+실패 메시지 표시)는
   전부 정상 동작하고 있었다 — 스크립트의 하드코딩된 기대값만 틀렸다. `game/qa/
   storage_chest_check.gd`를 수정해서 절대값 대신 "채우기 전/후 델타"로 검증하도록
   바꿨다(`_chest_item_count()`/`_chest_find_item_index()` 헬퍼 추가) — 이제 상자에
   사전 채움 목록이 앞으로 더 늘어나도 이 스크립트가 계속 유효하다. 수정 후 재실행해서
   `QA_STORAGE_CHEST_CHECK_PASS` 확인.
3) `inventory_safety_check.gd`는 처음부터 상대(before/after) 값으로만 비교하고 있어서
   수정 없이 그대로 `QA_INVENTORY_SAFETY_CHECK_PASS` 통과 — 바닥 드롭 생존, 배치 제작
   출력 버퍼 안전, 상자 이전 거부/성공, 제작 수령 거부/성공 6가지 경로 전부 정상.
4) `game/qa/full_sweep.gd`(수정 없이 그대로 재사용)를 실행해 43장 캡처, 전부 Read로
   직접 확인. 캐릭터/도구 4종 idle·사용·이동 방향별 크기 일관성, 저장 상자 UI(41종
   실제 사전 채움 상태로 스크린샷 — 스크롤바 렌더링 확인, 이전 #116 검증은 가짜
   아이템 60종으로만 확인했었는데 이번엔 실제 아이템으로 재확인), 가공대/제련로/
   조리대/조리용 화로 4개 창, 농사, 채집/채광(철광석·모래·구리광석)/벌목/목장/사냥,
   낮/밤/비 — 전부 이상 없음.
5) 스윕 도중 목장(ranch_zone) 스텝 스크린샷에서 캐릭터가 총을 발사하는 모습으로
   찍힌 것을 발견해 처음엔 결함으로 의심했으나, 원인을 추적한 결과 **게임 결함이
   아니라 `full_sweep.gd` 자신의 스크립트 한계**였다: 이 스텝이
   `InventoryData.add_item("captured_deer", 1)` 후 그 슬롯을 찾아 `_select_hotbar()`로
   손에 들려고 시도하는데, 그 시점까지 스윕이 누적해서 넣어둔 다른 테스트 아이템들
   때문에 `captured_deer`가 핫바 범위(인덱스 0~8) 밖의 일반 슬롯에 들어가 버려서
   코드의 폴백(`i if i < HOTBAR_SIZE else 0`)이 슬롯 0(총)을 대신 선택한 것 —
   `zone._release_one()`은 손에 든 아이템과 무관하게 직접 호출되므로 방출 자체(펜
   안에 사슴이 보임)는 정상 동작했다. 실제 플레이에서는 사슴을 처음 포획하는 시점에
   핫바가 이 정도로 꽉 차 있을 상황이 아니라 재현되지 않는, 순수 스크립트 누적 상태
   artefact로 판단 — 티켓 등록하지 않음.
6) `docs/feedback/INBOX.md`의 `#118`을 `[x]`로 갱신. 결함 0건이라 "전체 스윕" 규칙대로
   재확인 루프(QA 재확인 항목)를 추가하지 않는다 — 다음 스윕은 BUILD/DESIGN 5개
   완료 시 자동으로 다시 등록된다.

**다음에 할 것**: INBOX.md에 미완료 항목이 하나도 없다 — `loop.sh`가 새 `[BUILD]`/
`[DESIGN]`/`[QA]` 항목이 추가될 때까지 세션을 새로 열지 않고 스스로 종료한다. 참고할
것: (1) `game/qa/storage_chest_check.gd`가 이제 델타 기반으로 바뀌었으니, 앞으로 이
스크립트를 참고해 새 QA 스크립트를 짤 때도 절대값 대신 "테스트 상자에 이미 뭐가
있든 상관없이 성립하는 값"을 기준으로 검증하는 패턴을 재사용할 것. (2) `full_sweep.gd`
같은 누적형 스크립트에서 이번 ranch_zone처럼 "핫바가 꽉 차서 의도한 아이템을 못
드는" 폴백 artefact가 또 나올 수 있다 — 스크린샷에서 이상해 보여도 먼저 스크립트의
상태 누적 문제인지 실제 게임 결함인지 구분할 것.

## 지난 바퀴 기록 (바퀴 170, 그대로 보존)
...
톱니바퀴/구리선/유리병 레시피+8종 아이콘, 가공대 강철 도구 3종 레시피+#111 아이콘).)
1) `game/qa/full_sweep.gd`(바퀴 166이 모래/구리광석 채광 스텝까지 갖춘 버전, 수정
   없이 그대로 재사용)를 `project.godot [autoload]`에 임시 등록하고 세이브 파일을
   지운 뒤 `godot --path .`(실제 렌더러)로 실행 — 43장 전부 정상 캡처, 실행 후
   `project.godot`를 diff로 원상복구 확인.
2) **#114 지시문이 특정한 #104~#108 구간을 자체 판단만으로 넘기지 않기 위해(자기
   채점 방지), 그 기능들을 만들 때 각 BUILD/DESIGN 세션이 이미 커밋해둔 전용 검증
   스크립트(`steel_glass_copper_check.gd`(#105)/`processing_bymaterials_check.gd`
   (#106)/`inbox107_check.gd`(#107)/`steel_tools_check.gd`(#108))를 이번 QA
   세션에서 독립적으로 다시 실행**해서 재검증했다 — 각각 project.godot autoload
   교체 → 세이브 삭제 → `godot --path .` 실행 → 원상복구를 반복(총 5회 godot 실행).
   전부 `QA_..._CHECK_PASS`로 통과(제련로 강철/유리/구리 3종, 가공대 못×4/경첩/
   톱니바퀴/구리선×2/유리병 5종, 강철곡괭이낫/강철도끼/강철낚싯대 3종 모두 실제
   제작→수령까지 확인).
3) 스크린샷 43장 + 검증 스크립트가 남긴 아이콘/인벤토리 스크린샷을 전부 Read로
   직접 열어 확인. 모래/구리광석 채광 프롬프트 텍스트 정상(바퀴 166이 격리한
   `Vector2(2500,2500)` 좌표 덕에 겹침 없음), 제련로/가공대 레시피 목록에 강철/
   유리/구리/못/경첩/톱니바퀴/구리선/유리병/강철 도구 3종이 전부 정확한 재료
   수량으로 표기됨, `#111`이 만든 강철 도구 아이콘까지 인벤토리에 정상 반영.
   `28_farm_empty.png`/`30_farm_ready.png`가 썸네일상 똑같아 보여 의심했으나
   확대 크롭(`Image.crop`+`NEAREST` 리사이즈)으로 대조한 결과 새싹→황금 벼
   그림으로 명확히 다름을 확인(오탐 방지). `43_world_rain.png`도 육안상 비
   내리는 티가 거의 안 나 의심했으나 픽셀 RGB를 직접 비교(`getpixel`)해서
   `RainOverlay`(파란빛 22% 알파)가 실제로 얹혀 있음을 확인(DESIGN.md가 이미
   "비주얼 최소한으로 충분"이라 규정한 부분이라 결함 아님).
4) 조리대(가공식품 전용) 창이 레시피 0개인 것도 확인했으나, `cooking_table.gd`
   주석에 "채소 아이템이 아직 정해지지 않아 범위 밖"이라고 명시돼 있어 의도된
   빈 상태 — 결함 아님.
5) `#115`(바퀴 166이 발행한 리소스 포인트 겹침 BUILD 티켓)는 이번 스윕에서
   자연 발생으로는 재현되지 않았다(43스텝 전부 격리/고정 좌표 기반이라 우연한
   근접 스폰을 지나칠 가능성 있음) — 이미 티켓이 있으므로 새로 등록하지 않는다.
...
바퀴 166 / 2026-09-04 ([QA] **INBOX #113 완료 — 전체 스윕, 결함 1건 발견(#115로 티켓
발행).** 범위: 메인 메뉴 → 캐릭터 슬롯/생성 → 로비 → 월드 스폰 → 도구 4종(총/도끼/
곡괭이낫/낚싯대) idle·사용·이동 → 인벤토리/저장 상자/가공대·제련로·조리대·조리용
화로 5개 창 → 농사 → 채집/채광(+ #103이 새로 추가한 모래/구리광석 채광 포인트를
실제로 캐보는 스텝 신규 추가) → 벌목 → 목장 → 사냥 → 낮밤/날씨, 총 43장.)
1) `game/qa/full_sweep.gd`(바퀴 155/156 버전, 이미 저장 상자/가공대/제련로/조리대/
   조리용 화로 5개 창 스텝까지 갖춰져 있었음)를 그대로 재사용하되, INBOX #113 원문
   지시대로 모래/구리광석 채광 포인트를 실제로 캐보는 스텝 4개(`mining_point_sand_idle`/
   `_harvest`, `mining_point_copper_idle`/`_harvest`)를 새로 추가했다. 기존
   `mining_point` 스텝은 필드에 무작위로 스폰된 mining류 포인트 중 처음 찾은 것 하나만
   봤을 뿐이라(가중치가 낮은 모래(10%)/구리광석(7%)는 안 뽑힐 수 있음), `#103`이
   실제로 만든 두 씬(`mining_point_sand.tscn`/`mining_point_copper.tscn`)을
   `mining_variety5_check.gd`(#103)와 같은 방식으로 직접 인스턴스화해서 확실히
   확인하게 만들었다.
2) 새 스크립트를 `project.godot [autoload]`에 임시 등록하고 세이브 파일(`characters.
   save`/`inventory.save`)을 지운 뒤 `godot --path .`(헤드리스 아님, 실제 렌더러)로
   실행 — 43장 전부 정상 캡처됨을 확인, 실행 후 `project.godot`를 diff로 원상복구 확인.
3) **1차 시도에서 모래/구리광석 프롬프트 텍스트가 심하게 깨져 보이는 것을 발견**(예:
   "채광 (모래)"가 "채광 (묻래)강석)"처럼 두 텍스트가 겹친 것처럼 보임) — 처음엔
   한글 폰트 렌더링 버그로 의심했으나, `Image.getbbox()`로 idle/harvest 스크린샷을
   픽셀 diff해서 실제로 두 개의 서로 다른 `Prompt` 라벨이 같은 화면 위치에 겹쳐
   그려지고 있음을 확인했다. 원인을 추적한 결과 **내 새 스텝이 직전 스텝이 플레이어를
   옮겨둔 위치 바로 옆(상대 오프셋)에 새 포인트를 스폰**해서, 근처에 이미 무작위로
   스폰돼 있던 다른 포인트(예: 자연 스폰된 유황광석/벼 씨앗 포인트)와 겹친 것이었다 —
   `game/qa/full_sweep.gd`의 스텝을 절대좌표 `Vector2(2500, 2500)`(자연 스폰 반경
   1400보다 확실히 멀지만 `Ground` 텍스처 범위(대략 ±4000, region_rect 8000×8000
   centered) 안쪽)로 격리해서 재실행 — 재확인 결과 텍스트("모래"/"구리광석")가
   완전히 정상이고, 아이템 그림도 화면에서 짧은 변 기준 45~70px로 32px 최소 기준을
   넉넉히 통과함을 확인했다. **즉 모래/구리광석 자체(#103/#104)는 결함 없음.**
4) 위 과정에서 우연히 발견한 진짜 게임 결함: **`world.gd`의 `_spawn_one_resource_point()`
   가 새 리소스 포인트 위치를 정할 때 플레이어 스폰 위치와의 최소 거리만 확인하고,
   이미 배치된 다른 포인트와의 거리는 전혀 확인하지 않는다.** 별도 디버그 스크립트로
   새 게임을 5회 반복 스폰해본 결과 3회에서 두 포인트가 서로의 `INTERACT_RADIUS`(70)
   합보다 가깝게 스폰됐다(우연이 아니라 흔한 케이스). 겹친 두 포인트에 동시에 접근하면
   두 `Prompt` 라벨이 겹쳐 읽을 수 없게 되는 것은 물론, 더 중요하게는 `resource_point.gd`
   의 `_unhandled_input()`이 `set_input_as_handled()`를 호출하지 않아서 **좌클릭 한
   번이 겹친 포인트 전부를 동시에 채집/채광한다는 것을 직접 재현해서 확인했다**(모래·
   구리광석 포인트를 40유닛 간격에 배치하고 좌클릭 이벤트 1회를 시뮬레이션 —
   `sprite.visible=false`+쿨다운 시작+드롭 아이템 생성이 두 포인트 모두에서 동시에
   일어남, 로그로 확인). INBOX #115로 티켓 발행(BUILD).
5) 그 외 나머지 39장(메인 메뉴~로비, 도구 4종 idle/사용/이동, 인벤토리, 저장 상자,
   가공대/제련로/조리대/조리용 화로 5개 창(#112가 고친 불투명 배경 유지 확인), 농사,
   채집(벼)/채광(철광석), 벌목, 목장, 사냥, 낮/밤/비)은 전부 이상 없음 — 창 뒤 월드
   투과 없음, 도구 idle/사용 모션 구별 정상, 배치 제작(#99) 수령까지 정상 동작.
6) `docs/feedback/INBOX.md`의 `#113`을 `[x]`로, `#115`([BUILD] 신규 티켓)를 추가했다.
   `#113`은 "전체 스윕"이라 DESIGN.md 규칙대로 재확인 루프(QA 재확인 항목)를 별도로
   추가하지 않는다 — 다음 전체 스윕(`#114`, 이미 큐에 있음, 이번과 별개 사이클)이나
   이후 자동 등록되는 스윕이 `#115` 수정 여부를 자연히 다시 훑는다.

## 지난 바퀴 기록 (바퀴 165, 그대로 보존)

바퀴 165 / 2026-09-04 ([DESIGN] **INBOX #111 완료 — #108~#110이 추가한 완성품 11종
(강철곡괭이낫/강철도끼/강철낚싯대/강철벽/강철문/강철 상자/창문/강철 갑옷/배터리/조명/
발전기) + 가공식품 1종(사료)의 인벤토리 아이템 아이콘을 파이썬 절차적 생성(Pillow)으로
일괄 제작했다.** AI 생성 도구 크레딧은 쓰지 않음(#107의 `/tmp/gen_107.py` 헬퍼 패턴
재사용). 급수 장치는 지시문이 허용한 대로 새로 그리지 않고 구리선+강철 아이콘을
합성해서 만들었다(총 13개 PNG 생성, 그중 12개가 이번 INBOX 항목이 요구한 대상).)
1) 새 스크립트 `/tmp/gen_111.py`(재사용 가능) 작성. **강철 도구 3종**(`steel_pickaxe`/
   `steel_axe`/`steel_fishing_rod`)은 지시문대로 새로 그리지 않고, 기존
   `pickaxe.png`/`axe.png`/`fishing_rod.png`를 픽셀 단위 루미넌스 기반으로 은회색
   톤(어두운 쪽 `(78,82,94)` ~ 밝은 쪽 `(232,236,244)`)으로 재도색해서
   `game/assets/sprites/tools/`에 저장했다 — 원본과 같은 구도(실루엣/음영 구조)를
   그대로 유지하면서 색만 강철 톤으로 바뀐다. **강철벽/강철문/강철 상자/창문**은
   `/tmp/gen_101.py`의 `vgrad_rect`/`outline_rect` 패턴을 재사용해 `game/assets/
   sprites/items/`에 제작 — 강철벽(리벳 3단 패널), 강철문(리벳 인셋 2칸+대각 보강대+
   청록 손잡이), 강철 상자(뚜껑/몸통 이음매+세로 밴드+자물쇠), 창문(회색 프레임+
   십자 창살+청록 반투명 유리, glass.png와 톤 통일). **강철 갑옷**은 어깨판(원 2개)+
   가슴판(사다리꼴) 실루엣으로 도구/건축물과 확실히 구별되게 새로 그렸다. **배터리/
   조명/발전기**는 지시문대로 작은 기계 부품 느낌(배터리=케이스+단자+노란 띠, 조명=
   유리 전구+필라멘트+소켓, 발전기=환기구 그릴+계기판+배기관)으로 서로 다른 실루엣/
   색으로 제작. **사료**는 마대자루(삼베색) 실루엣으로 rice.png와 형태로 구별.
   **급수 장치**는 `copper_wire.png`+`steel.png`를 20x20으로 축소해 겹쳐 합성한
   것(새 그림 없음, 지시문이 허용한 선택지).
2) 8배 확대 시트(`/tmp/inbox111_sheet.png`)로 13개 전체를 먼저 육안 검토 — 실루엣이
   서로 뚜렷이 구별됨을 확인. 강철 상자가 처음엔 뚜껑/몸통 구분이 흐릿해서 이음매를
   굵고 어둡게(`(34,36,42,255)`, 폭 3px) 강조하는 수정을 한 번 거쳤다.
3) `dropped_item.gd`의 `ITEM_ICONS`에 13개 항목을 추가했다(강철 도구 3종은
   `res://assets/sprites/tools/`, 나머지 10개는 `res://assets/sprites/items/`).
4) **검증**: 새 QA 스크립트 `game/qa/inbox111_check.gd`(커밋, 재사용 가능,
   `wall_door_icon_check.gd`(#101)와 같은 패턴, 슬롯0 인벤토리 초기화 포함)를
   `project.godot [autoload]`에 임시 등록하고, 먼저 `timeout 90 godot --headless
   --editor --quit-after 30`으로 새 PNG 13개의 `.import`를 생성한 뒤 `godot --path .`
   (실제 렌더링)로 실행 — 13개를 5열 그리드로 바닥에 드롭해 스크린샷
   (`/tmp/qa111/00_dropped_icons.png`)으로 확인. **첫 시도에서 드롭 지점이 슬롯0의
   기존 저장된 나무 벽 구조물과 겹쳐 강철문 아이콘이 가려 보였다** — 드롭 좌표를
   플레이어 기준 `(-3000,-3000)`으로 멀리 옮겨 재실행해서 깨끗한 화면으로 재확인(13개
   전부 서로 다른 실루엣/색으로 32px 이상 뚜렷이 보임, 3배 확대 크롭으로 재검증).
   인벤토리에 각 3개씩 채운 뒤 인벤토리 창 스크린샷(`/tmp/qa111/01_inventory.png`)에서
   13개 라벨("강철곡괭이낫 x3" 등)이 정상 표시됨을 확인(`QA_INBOX111_CHECK_PASS` 출력).
   **이 게임의 인벤토리 그리드 UI는 원래 아이콘 그림 없이 텍스트+수량만 표시하는
   방식**임을 `#107`의 과거 스크린샷과 비교해 확인했다 — 새 아이템도 기존 패턴과
   동일하게 동작하는 것이라 결함이 아니다. 검증 후 `project.godot`의 임시 autoload
   등록을 원상복구하고 `diff /tmp/project.godot.bak111 project.godot`로 완전히
   동일함을 확인했다.
5) `docs/feedback/INBOX.md`의 `#111`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#110`(BUILD) 완료로 2였음 → 이번 `#111`(DESIGN)로 **3**이 됐다(5 도달까지 2개
   남음). INBOX에 남은 미완료 항목은 `#113`/`#114`([QA] 전체 스윕) 둘뿐이라, 다음
   [BUILD]/[DESIGN] 세션이 열려도 처리할 항목이 없다 — 새 지시가 추가되지 않으면
   `loop.sh`가 [QA] 하네스로 넘어갈 것이다.

## 스스로 판단해서 고친 부분 (INBOX #111)
- 강철 상자 아이콘의 뚜껑/몸통 이음매 선을 지시문에 없던 굵기(3px)로 강조했다. 근거:
  1차 렌더링에서 상자가 강철문과 실루엣이 너무 비슷해 보여 ①의 "서로 구별되게
  만든다" 기준에 못 미친다고 판단했고, 이음매 하나만 진하게 해도 "뚜껑 있는 상자"로
  바로 읽혀서 최소 수정으로 해결했다.
- QA 드롭 좌표를 `(300,-200)`(#101 스크립트 기본값)에서 `(-3000,-3000)`으로 바꿨다.
  근거: 슬롯0에 이전 바퀴들이 지어둔 나무 벽 구조물과 겹쳐 스크린샷 판독이 어려웠던
  것을 직접 보고 수정했다 — 다음 바퀴가 같은 슬롯으로 드롭 스크린샷 QA를 짤 때
  플레이어 근처가 아니라 확실히 빈 좌표를 쓰는 것을 권장.

## 지시받지 않았지만 추가한 개선 (INBOX #111)
- 없음.

## 어려움/에러 (INBOX #111)
- 위 "검증" 항목대로, 새 PNG 13개를 만든 직후 바로 QA를 돌리려 하면 `.import`가 없어
  `preload()`가 실패한다는 것을 `#107`의 기록으로 미리 알고 있어서, 이번엔 처음부터
  `godot --headless --editor --quit-after 30`을 먼저 돌려 시행착오 없이 넘어갔다.
- 강철 도구 3종을 픽셀 루미넌스로 재도색하는 방식이 처음 시도라 결과가 걱정됐지만,
  손잡이(원래 갈색)와 도구머리(원래 회색)의 밝기 차이가 그대로 유지되면서 색만
  은회색 계열로 바뀌어 재도색 후에도 손잡이/머리 구분이 자연스럽게 남았다 — **다음
  바퀴가 "기존 도구와 같은 구도, 톤만 다르게" 요구를 받으면 이 루미넌스 재도색
  기법(`dark`~`light` 그라디언트로 원본 RGB를 밝기 기준 매핑)을 그대로 재사용할 것**,
  손으로 각 부위를 다시 그리는 것보다 훨씬 빠르고 구도가 100% 보존된다.

**다음에 할 것**: INBOX.md에 남은 미완료 항목은 `#113`/`#114`([QA] 전체 스윕) 둘뿐이다
— 다음 [BUILD]/[DESIGN] 세션이 열려도 처리할 대상이 없으므로 `loop.sh`가 [QA] 하네스로
넘어가 `#113`부터 처리할 것이다. 참고할 것: (1) `#113`은 `full_sweep.gd`(바퀴 155
버전)를 재사용하고 모래/구리광석 채광 포인트를 캐보는 스텝을 추가해야 한다. (2) `#114`는
`#104`~`#108` 구간(모래/구리광석 채광 포인트 그림, 제련로 강철/유리/구리, 가공대
못/경첩/톱니바퀴/구리선/유리병+8개 아이콘, 강철 도구 3종 — **이번 `#111`로 강철 도구
아이콘도 이제 다 갖춰졌으니 "정상"으로 재확인만 하면 됨**)을 특히 확인해야 한다. (3)
BUILD/DESIGN 완료 카운터는 3(5 도달까지 2개 남음) — `#113`/`#114`는 이미 큐에 있는
스윕이라 이번 카운터와 무관하게 처리되고, 카운터는 사용자가 새 `[BUILD]`/`[DESIGN]`
항목을 추가할 때 계속 누적된다.

## 지난 바퀴 기록 (바퀴 164, 그대로 보존)

바퀴 164 / 2026-09-04 ([BUILD] **INBOX #110 완료 — 가공대(`processing_table.gd`)
`RECIPES`에 전기 계열 완성품 3종 + 사료 + 급수 장치 5종(배터리 구리2+유황광석1→1,
조명 구리선1+유리1→1, 발전기 톱니바퀴2+구리선3→1, 사료 벼3→2, 급수 장치
구리선1+강철1→1) 레시피를 추가했다.** DESIGN.md "생산 라인"(2026-09-04) 규칙대로
레시피 재료는 최대 2종까지만 썼다. **부가로 가공대 창의 스크롤 누락 버그를 함께
고쳤다(아래 "스스로 판단해서 고친 부분" 참고).**)
1) `processing_table.gd`의 `RECIPES` 배열에 5개 항목(`battery`/`lamp`/`generator`/
   `feed`/`water_pump`)을 추가했다 — 기존 `CraftingStation` 공용 베이스를 그대로
   타므로 이 파일 외 다른 로직 변경은 필요 없었다. 재료(구리/유황광석/구리선/유리/
   톱니바퀴/벼/강철)는 전부 이미 존재하는 아이템이라 새 원재료/가공물을 만들 필요는
   없었다.
2) `world.gd`의 `ITEM_LABELS`("배터리"/"조명"/"발전기"/"사료"/"급수 장치")/
   `ITEM_CATEGORIES`(`battery`/`lamp`/`generator`/`water_pump`는 "완성품", `feed`는
   "가공식품")에 등록했다.
3) **검증**: 새 QA 스크립트 `game/qa/electric_check.gd`(커밋, 재사용 가능,
   `steel_construction_check.gd`(#109)와 같은 패턴, 슬롯0 인벤토리 초기화 포함)를
   `project.godot [autoload]`에 임시 등록하고 `godot --path .`(실제 렌더링)로 실행 —
   5개 레시피 모두 재료가 정확히 소모되고(`start_batch()`) 결과물이 정확한 수량으로
   수령됨(`collect_output()`, `QA_ELECTRIC_CHECK_PASS` 출력, 사료만 amount=2로 2개
   수령 확인). 창 열림 스크린샷(`/tmp/qa110/01_window_open.png`)을 Read로 직접 확인.
4) 검증 후 `project.godot`의 임시 autoload 등록을 원상복구하고
   `diff /tmp/project.godot.bak110 project.godot`로 완전히 동일함을 확인했다.
5) `docs/feedback/INBOX.md`의 `#110`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#109`(BUILD) 완료로 1이었음 → 이번 `#110`(BUILD)로 **2**가 됐다(5 도달까지 3개
   남음).

## 스스로 판단해서 고친 부분 (INBOX #110)
- **가공대 제작 창(`_build_crafting_window()`)에 `ScrollContainer`가 없어서 레시피가
  20개로 늘어난 지금 화면(720px) 밖으로 넘쳐 이번에 추가한 5개(특히 마지막 급수
  장치)를 포함한 아래쪽 레시피들을 실제 플레이어가 마우스로 클릭할 수 없는 상태였다.**
  QA 스크립트는 `start_batch()`/`collect_output()`을 직접 호출해서 로직 자체는
  통과했지만, INBOX #110 원문의 "가공대에서 다섯 레시피 모두 실제로 동작하는지
  확인한다"를 "실제 플레이어가 UI로 접근 가능한가"까지 포함하는 것으로 판단해서
  `storage_list`(#96)와 같은 패턴으로 `_crafting_list`를 `ScrollContainer`(높이
  420px)로 감쌌다. 이 창은 가공대/제련로/조리대/조리용 화로가 모두 공유하는 범용
  창(`open_crafting_window()`)이라 네 작업대 전부에 적용된다. 스크롤을 맨 아래로
  내린 스크린샷(`/tmp/qa110/01b_window_scrolled_bottom.png`)으로 5개 신규 레시피가
  전부 보이고 "제작 시작" 버튼이 눌리는 위치에 있는 것을 직접 확인했다. 근거: 이번
  항목의 결과물(가공대 레시피)이 실제로 "동작"하려면 코드 로직뿐 아니라 UI로 도달
  가능해야 하고, 합격 기준(①, "다른 사람이 봤을 때도 퀄리티가 떨어지지 않는가")에
  스크롤 없이 절반의 레시피가 클릭 불가능한 상태는 명백히 못 미친다고 판단했다.

## 지시받지 않았지만 추가한 개선 (INBOX #110)
- 위 스크롤 수정과 함께, QA 스크립트에 스크롤을 맨 아래로 내려 스크린샷을 찍는
  단계(`crafting_scroll.scroll_vertical = 100000`)를 추가했다 — 앞으로 레시피가 더
  늘어날 때도 이 패턴을 재사용해 "새로 추가한 레시피가 실제로 화면에 나타나고
  클릭 가능한가"를 스크린샷으로 확인할 수 있다.

## 어려움/에러 (INBOX #110)
- 없음 — 레시피 자체는 기존 재료로만 구성돼 막힘 없이 진행됐다. 스크롤 버그는 QA
  스크린샷을 찍는 과정에서 레시피 목록이 화면 아래로 잘려 있는 것을 직접 보고
  발견했다(코드만 봐서는 몰랐을 것 — 스크린샷을 실제로 Read해서 눈으로 본 덕분에
  잡은 문제).

**다음에 할 것**: **다음 [BUILD] 세션은 INBOX #111**은 [DESIGN] 태그라 BUILD
하네스가 처리하지 않는다 — INBOX에 남은 `[BUILD]` 태그 미완료 항목이 현재 없다(#111은
DESIGN, #113/#114는 QA). 다음 `[BUILD]` 세션이 열릴 때는 INBOX.md를 다시 확인해서
그 시점에 번호가 가장 작은 미완료 `[BUILD]` 항목을 찾을 것 — 사용자가 새 `[BUILD]`
항목을 추가하지 않았다면 `loop.sh`가 다음 태그(`[DESIGN]`/`[QA]`) 하네스로 넘어갈
것이다. 참고할 것: (1) **가공대(및 제련로/조리대/조리용 화로 공용) 제작 창의
`ScrollContainer` 수정이 이번에 들어갔다 — 앞으로 레시피를 추가하는 BUILD 항목은
더 이상 이 스크롤 문제를 걱정하지 않아도 된다.** (2) `#111`([DESIGN], `#108`~`#110`의
완성품 아이콘 11개+가공식품 1개)이 다음 DESIGN 차례 — `#107`이 만든 `/tmp/gen_107.py`의
헬퍼 함수를 재사용할 것(강철 도구 3종은 기존 도구 아이콘과 같은 구도에 은회색 톤만
다르게 하라는 지시가 있으니 그 생성 방식도 먼저 조사할 것). (3) `#113`/`#114`([QA]
전체 스윕)는 번호 순서상 `#111`보다 뒤에 자연스럽게 대기한다. (4) **새 QA 스크립트를
짤 때 슬롯 0 캐릭터의 인벤토리가 이전 바퀴들의 QA로 누적돼 가득 찼을 수 있다는 것을
항상 의심할 것** — `InventoryData._general_slots.fill(null)` + `_save()` 초기화
패턴(`electric_check.gd`/`steel_construction_check.gd`에 이미 적용)을 검증 시작 전에
넣어두는 것을 권장.

## 지난 바퀴 기록 (바퀴 163, 그대로 보존)

바퀴 163 / 2026-09-04 ([BUILD] **INBOX #109 완료 — 가공대(`processing_table.gd`)
`RECIPES`에 강철 건축 완성품 + 창문 + 강철 갑옷 5종(강철벽 강철3+못1→1, 강철문
강철2+경첩1→1, 강철 상자 강철4+못2→1, 창문 유리2+못1→1, 강철 갑옷 강철5→1) 레시피를
추가했다.** DESIGN.md "생산 라인"(2026-09-04) 규칙대로 레시피 재료는 최대 2종까지만
썼다(강철 갑옷은 원단/가죽 출처가 없어 1종만).)
1) `processing_table.gd`의 `RECIPES` 배열에 5개 항목(`steel_wall`/`steel_door`/
   `steel_chest`/`window`/`steel_armor`)을 추가했다 — `CraftingStation` 공용 베이스를
   그대로 타므로 이 파일 외 다른 로직 변경은 필요 없었다(#108 원문 예상대로).
2) `world.gd`의 `ITEM_LABELS`("강철벽"/"강철문"/"강철 상자"/"창문"/"강철 갑옷")/
   `ITEM_CATEGORIES`(다섯 다 "완성품")에 등록했다. `steel_chest`는 INBOX 원문 지시대로
   #96 저장 상자와는 별개의 "완성품" 아이템일 뿐, 새 저장 오브젝트로 놓을 수 있게
   만들지는 않았다.
3) **검증**: 새 QA 스크립트 `game/qa/steel_construction_check.gd`(커밋, 재사용 가능,
   `steel_tools_check.gd`(#108)와 같은 패턴, 2종 재료 레시피용으로 소비 검증 부분만
   `inputs` 딕셔너리 순회로 일반화)를 `project.godot [autoload]`에 임시 등록하고
   `godot --path .`(실제 렌더링)로 실행. **첫 실행에서 `steel_wall`만 성공하고 나머지
   4개는 실패**했다 — 원인 조사 결과 코드 버그가 아니라, 캐릭터 슬롯 0의 저장된
   인벤토리가 `#105`~`#108`이 반복해서 같은 슬롯으로 QA를 실행해온 탓에 일반 슬롯
   18칸이 이미 전부 차 있었던 것(`InventoryData.get_general_slots()`로 직접 확인).
   DESIGN.md "인벤토리 안전" 규칙대로 공간이 없으면 아이템이 사라지지 않고
   출력 버퍼에 남아있었을 뿐 — 정상 동작이었다. QA 스크립트에 검증 시작 전
   `InventoryData._general_slots.fill(null)` + `InventoryData._save()`로 일반 슬롯을
   비우는 처리를 추가해 재실행 → 5개 레시피 모두 재료가 정확히 소모되고
   (`start_batch()`) 결과물이 1개씩 정확히 수령됨(`collect_output()`,
   `QA_STEEL_CONSTRUCTION_CHECK_PASS` 출력). 창 열림 스크린샷(`/tmp/qa109/01_window_open.png`)
   과 제작 후 스크린샷(`/tmp/qa109/02_after_craft_all.png`)을 Read로 직접 확인 —
   20개 레시피 목록에 "강철 x3, 못 x1 → 강철벽 x1" 등 2종 재료 표기가 기존 1종 재료
   레시피와 같은 형식으로 정상 표시됨, 제작 후 강철 20→6/유리 5→3/못 10→6개로 정확히
   줄어듦을 확인.
4) 검증 후 `project.godot`의 임시 autoload 등록을 원상복구하고
   `diff /tmp/project.godot.bak109 project.godot`로 완전히 동일함을 확인했다.
5) `docs/feedback/INBOX.md`의 `#109`를 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#108`(BUILD) 완료로 5에 도달해 0으로 리셋됐었음(`#114` 추가) → 이번 `#109`(BUILD)로
   **1**이 됐다.

## 스스로 판단해서 고친 부분 (INBOX #109)
- 없음 — INBOX 원문이 재료/개수/카테고리까지 전부 구체적으로 지정해뒀고, 기존
  `CraftingStation` 베이스와 `steel_tools_check.gd`(#108) QA 패턴을 그대로 재사용할 수
  있어 임의 판단이 필요한 지점이 없었다.

## 지시받지 않았지만 추가한 개선 (INBOX #109)
- QA 스크립트 시작 시 캐릭터 슬롯 0의 일반 인벤토리 슬롯을 비우는 처리
  (`InventoryData._general_slots.fill(null)` + `_save()`)를 추가했다. **다음 바퀴가
  참고할 것**: 슬롯 0으로 매번 같은 캐릭터를 재사용하는 QA 스크립트 패턴 자체가
  누적되면 18칸이 가득 차서 이번처럼 "재료 부족이 아닌 인벤토리 꽉 참"으로 검증이
  실패할 수 있다 — 새 QA 스크립트를 짤 때 이 초기화 한 줄을 기본으로 넣는 것을 권장.

## 어려움/에러 (INBOX #109)
- 위 "검증" 항목 3)에 적은 대로, 처음엔 `steel_wall`만 성공(그나마 이전 바퀴 잔여
  1개와 합쳐져 카운트가 2로 왜곡)하고 나머지 4개가 전부 "수령 실패"로 나타나 코드
  버그처럼 보였다. `table.output_buffer`를 직접 출력해서 배치 자체는 정상 완료(값이
  버퍼에 정확히 쌓임)되는데 `collect_output()`이 인벤토리로 옮기지 못하고 있음을
  확인했고, `InventoryData.get_general_slots()`로 18칸이 전부 다른 아이템(이전 바퀴들의
  강철/유리/못/경첩/톱니바퀴/구리선/유리병/강철도구 3종/`steel_wall` 잔여분)으로
  채워져 있는 것을 발견해 원인을 확정했다.

## 지난 바퀴 기록 (바퀴 162, 그대로 보존)

바퀴 162 / 2026-09-04 ([BUILD] **INBOX #108 완료 — 가공대(`processing_table.gd`)
`RECIPES`에 강철 도구 3종(강철곡괭이낫 강철3→1, 강철도끼 강철3→1, 강철낚싯대 강철2→1)
레시피를 추가했다.** DESIGN.md "생산 라인"(2026-09-04) 규칙대로 레시피 재료는 각각
1종(강철)만 쓴다. 이번 항목은 지시대로 "인벤토리에 아이템으로 만들 수 있게만" 함 —
기존 곡괭이낫/도끼/낚싯대 대비 실제 성능 차이는 만들지 않음(도구 등급 시스템 자체가
아직 없음). 새 그림은 만들지 않음(아이콘은 `#111`[DESIGN]이 나중에 일괄 제작).)
1) `processing_table.gd`의 `RECIPES` 배열에 3개 항목(`steel_pickaxe`/`steel_axe`/
   `steel_fishing_rod`)을 추가했다 — `CraftingStation` 공용 베이스를 그대로 타므로
   이 파일 외 다른 로직 변경은 필요 없었다(#105~#107 원문 예상대로).
2) `world.gd`의 `ITEM_LABELS`("강철곡괭이낫"/"강철도끼"/"강철낚싯대")/`ITEM_CATEGORIES`
   (셋 다 "도구")에 등록했다.
3) **검증**: 새 QA 스크립트 `game/qa/steel_tools_check.gd`(커밋, 재사용 가능,
   `processing_bymaterials_check.gd`(#106)와 같은 패턴)를 `project.godot [autoload]`에
   임시 등록하고 `godot --path .`(실제 렌더링)로 실행: 가공대 근처에서 좌클릭으로 창을
   열어 15개 레시피(기존 12개+신규 3개)가 모두 목록에 뜨는지 스크린샷
   (`/tmp/qa108/01_window_open.png`)으로 확인(직접 Read로 눈으로도 확인 — "강철 x3 →
   강철곡괭이낫 x1"/"강철 x3 → 강철도끼 x1"/"강철 x2 → 강철낚싯대 x1" 세 줄이 정상
   표시됨), 강철 8개를 지급한 뒤 세 레시피를 순서대로(한 번에 배치 1개만 가능하므로
   하나씩) `start_batch()`→시간 진행→`collect_output()`까지 실행해 각각 재료가 레시피대로
   정확히 소모되고 결과물이 1개씩 정확히 인벤토리에 들어옴을 확인
   (`QA_STEEL_TOOLS_CHECK_PASS` 출력), 제작 후 스크린샷(`/tmp/qa108/02_after_craft_all.png`)
   도 확보. 검증 후 `project.godot`의 임시 autoload 등록을 원상복구하고
   `diff /tmp/project.godot.bak108 project.godot`로 완전히 동일함을 확인했다.
4) `docs/feedback/INBOX.md`의 `#108`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#104`(DESIGN)=1 → `#105`(BUILD)=2 → `#106`(BUILD)=3 → `#107`(DESIGN)=4 → 이번
   `#108`(BUILD)으로 **5에 도달** — 규칙대로 새 `[QA] 전체 스윕` 항목(`#114`)을
   `docs/feedback/INBOX.md` 큐 끝에 추가하고 카운터를 0으로 리셋했다(`#113`은 별개
   사이클의 스윕으로 이미 큐에 있었고, 이번 트리거로 생략하지 않았다).

## 스스로 판단해서 고친 부분 (INBOX #108)
- 없음 — INBOX 원문이 재료/개수/카테고리까지 전부 구체적으로 지정해뒀고, 기존
  `CraftingStation` 베이스와 `processing_bymaterials_check.gd`(#106) QA 패턴을 그대로
  재사용할 수 있어 임의 판단이 필요한 지점이 없었다.

## 어려움/에러 (INBOX #108)
- 없음 — 새 이미지 자산을 만들지 않아 `#107`이 기록해둔 `.import` 재임포트 이슈도 겪지
  않았다(레시피 데이터 추가 + QA 스크립트뿐).

**다음에 할 것**: **다음 [BUILD] 세션은 INBOX #109**(강철 건축 완성품 5종 — 강철벽/
강철문/강철 상자/창문/강철 갑옷을 가공대 레시피로 추가, #105/#106 완료로 이미 진행
가능)를 처리한다. 참고할 것: (1) `#109` 완료 후 `#110`(전기 계열+사료/급수, [BUILD])
순으로 진행 가능. (2) `#111`([DESIGN], `#108`~`#110`의 완성품 아이콘 11개+가공식품
1개)은 그 셋이 끝난 뒤 진행 — `#107`이 만든 `/tmp/gen_107.py`의 헬퍼 함수를 재사용할
것(강철 도구 3종은 기존 도구 아이콘(`game/assets/sprites/tools/*.png`)과 같은 구도에
은회색 톤만 다르게 하라는 지시가 있으니 그 생성 방식도 먼저 조사할 것). (3) 이번에
새로 추가한 `#114`([QA] 전체 스윕, `#104`~`#108` 구간)는 번호 순서상 `#109`~`#111`보다
뒤에 자연스럽게 대기한다 — `#113`(별도 사이클의 스윕)도 마찬가지로 그 사이 어딘가에서
처리될 것, 둘 다 결국 처리되며 순서만 늦어질 뿐이다.

## 지난 바퀴 기록 (바퀴 161, 그대로 보존)

바퀴 161 / 2026-09-04 ([DESIGN] **INBOX #107 완료 — #105/#106이 추가한 8개 가공물
(강철/유리/구리/못/경첩/톱니바퀴/구리선/유리병)의 인벤토리 아이템 아이콘을 파이썬
절차적 생성(Pillow)으로 일괄 제작했다.** AI 생성 도구 크레딧은 쓰지 않음(#91/#86과
같은 기법 재사용 — `new_canvas`/`down`/`outline_and_save` + `shaded_blob` 패턴).)
1) 새 스크립트 `/tmp/gen_107.py`(재사용 가능, `/tmp/gen_91.py` 스타일 그대로) 작성 —
   8개 아이콘 모두 32×32, 배경 투명, 서로 겹치지 않는 실루엣으로 설계: 강철=은회색
   크로스(X)로 겹친 바 2개(iron.png의 단일 사다리꼴 잉곳과 실루엣 구별), 유리=반투명
   청록빛 각진 파편 조각(알파 블렌딩으로 투명감), 구리=붉은 금속광택 둥근 잉곳(퍽
   모양), 못=얇은 축+납작 머리+뾰족 끝, 경첩=사각 금속판 2장+가운데 원통 핀, 톱니바퀴=
   원형 몸체+둘레 톱니+가운데 구멍, 구리선=붉은 금속색 꼬인 코일(스프링 형태),
   유리병=목+몸통 병 실루엣(반투명 청록빛). `python3 /tmp/gen_107.py`로
   `game/assets/sprites/items/{steel,glass,copper,nail,hinge,gear,copper_wire,
   glass_bottle}.png` 8개를 생성했다.
2) 8×확대 시트(`/tmp/inbox107_sheet.png`)로 먼저 육안 검토(Read 도구) — 8개 모두
   실루엣이 뚜렷이 구별되고 소재 느낌(강철 광택/유리 투명/구리 금속광/못·경첩·톱니바퀴
   부품 형태/구리선 꼬인 선/유리병 병 모양)이 INBOX 원문 지시대로 드러남을 확인.
3) `dropped_item.gd`의 `ITEM_ICONS`에 8개 항목을 추가해 연결했다.
4) **검증**: 새 QA 스크립트 `game/qa/inbox107_check.gd`(커밋, 재사용 가능,
   `wall_door_icon_check.gd`(#101)와 같은 패턴)를 `project.godot [autoload]`에 임시
   등록하고 `godot --path .`(실제 렌더링, 사전에 `godot --headless --editor
   --quit-after 30`으로 새 PNG 8개의 `.import` 파일부터 생성해둠 — 임포트 전에는
   `preload()`가 "has no resource loaders" 파싱 에러를 냄, 아래 "어려움/에러" 참고)로
   실행: 8개를 바닥에 두 줄로 드롭해 한 화면에 모아 스크린샷
   (`/tmp/qa107/00_dropped_icons.png`)으로 확인 — 8개 모두 서로 다른 실루엣/색으로
   32px 이상 크기로 뚜렷이 보임(3배 확대 크롭으로 직접 재확인), 인벤토리에 각 3개씩
   채운 뒤 인벤토리 창 스크린샷(`/tmp/qa107/01_inventory.png`)에서도 "강철 x7"/
   "유리 x7"/"구리 x7"/"경첩 x7"/"못 x10"/"톱니바퀴 x7"/"구리선 x8"/"유리병 x7"
   라벨이 정상 표시됨을 확인(`QA_INBOX107_CHECK_PASS` 출력). 검증 후 `project.godot`의
   임시 autoload 등록을 원상복구하고 `diff /tmp/project.godot.bak107 project.godot`로
   완전히 동일함을 확인했다.
5) `docs/feedback/INBOX.md`의 `#107`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#104`(DESIGN)=1 → `#105`(BUILD)=2 → `#106`(BUILD)=3 → 이번 `#107`(DESIGN)으로
   **4**가 됐다(5 도달까지 1개 남음 — 다음 `#108`(BUILD) 완료 시 새 QA 스윕이 큐에
   추가될 것).

## 스스로 판단해서 고친 부분 (INBOX #107)
- 없음 — INBOX 원문이 색/형태(강철=은회색 광택, 유리=반투명 청록빛, 구리=붉은
  금속광택, 못/경첩/톱니바퀴=서로 구별되는 작은 금속 부품, 구리선=꼬인 선, 유리병=투명
  병 모양)까지 구체적으로 지정해뒀고, `/tmp/gen_91.py`/`/tmp/gen_104.py`의 기존
  헬퍼(`new_canvas`/`shaded_blob`/`outline_and_save`)를 그대로 재사용할 수 있어 임의
  판단이 필요한 지점이 없었다.

## 어려움/에러 (INBOX #107)
- 새 PNG를 `game/assets/sprites/items/`에 저장한 직후 바로 QA용 임시 autoload를 걸고
  `godot --path .`를 실행했더니, `dropped_item.gd`의 새 `preload()` 8줄에서 전부
  `Parse Error: Preload file "..." has no resource loaders (unrecognized file
  extension)`로 실패했다(스크립트 자체가 로드되지 않아 `spawn_dropped_item`의
  `item_name` 대입까지 "Invalid assignment... on a base object of type 'Node2D'"로
  연쇄 실패). **원인**: Godot 에디터가 새로 추가된 이미지 파일의 `.import` 리소스
  메타데이터를 아직 생성하지 않은 상태였다(기존 아이템 아이콘들은 이전 바퀴에서 이미
  에디터를 거쳐 `.import`가 생겼지만, 이번에 새로 만든 8개는 없었음). **해결**:
  `timeout 90 godot --headless --editor --quit-after 30`로 에디터를 한 번 짧게 띄워
  프로젝트 파일시스템을 스캔·재임포트시키면 `.import` 파일 8개가 생성되고, 그 뒤
  `godot --path .`(게임 실행 모드)로 정상적으로 `preload()`가 통과한다. **다음
  바퀴가 참고할 것**: 새 이미지 자산을 만들고 바로 같은 바퀴 안에서 QA 스크린샷을
  찍으려면, `.import` 파일 생성 여부를 먼저 확인하고(`ls *.png.import`) 없으면
  `godot --headless --editor --quit-after 30`을 먼저 한 번 돌려야 한다 — 이전
  바퀴들(#86/#91/#101/#104~#106)은 아마 이 단계를 겪었어도 기록해두지 않았을 뿐일
  가능성이 높다.

**다음에 할 것**: **다음 [BUILD] 세션은 INBOX #108**(강철 도구 3종 — 강철곡괭이낫/
강철도끼/강철낚싯대를 가공대 레시피로 추가, #105/#106 완료로 이미 진행 가능)을
처리한다. 참고할 것: (1) `#108` 완료로 BUILD/DESIGN 완료 카운터가 **5에 도달** —
이번 사이클(#104~#108)은 `#113`(#103 완료 시점 사이클이 낳은 것)과는 **별개의
사이클**이므로, 규칙대로 `#108` 완료 시 **새 `[QA] 전체 스윕` 항목을 새 번호로 큐에
추가하고 카운터를 0으로 리셋**해야 한다(바퀴 157의 STATUS.md 기록, "다음에 할 것"
줄 3663 "자동으로 `#113`과 별개의 새 QA 스윕이 큐에 추가될 것" 참고 — `#113`이 이미
큐에 있다고 이번 트리거를 생략하지 말 것). (2) `#108` 다음은 `#109`(강철 건축+갑옷,
[BUILD]), `#110`(전기 계열+사료/급수, [BUILD]) 순으로 진행 가능. (3) `#111`([DESIGN],
#108~#110의 완성품 아이콘)은 그 셋이 끝난 뒤 진행 — 이번(#107)에 만든
`/tmp/gen_107.py`의 헬퍼 함수를 그대로 재사용할 것(강철 도구 3종은 기존 도구 아이콘과
같은 구도에 은회색 톤만 다르게 하라는 지시가 있으니 `game/assets/sprites/tools/*.png`
생성 스크립트도 먼저 찾아볼 것). (4) `#113`은 번호 순서상 `#108`~`#111`보다 뒤에
자연스럽게 대기하며, `#108` 완료 시 새로 추가될 QA 스윕 항목은 그보다도 더 뒤에
쌓인다(둘 다 결국 처리됨, 순서만 늦어질 뿐).

## 지난 바퀴 기록 (바퀴 160, 그대로 보존)

바퀴 160 / 2026-09-04 ([BUILD] **INBOX #106 완료 — 가공대(`processing_table.gd`)
`RECIPES`에 못(철 1→4)/경첩(철 2→1)/톱니바퀴(강철 2→1)/구리선(구리 1→2)/유리병
(유리 1→1) 5개 부재료 레시피를 추가했다.** DESIGN.md "생산 라인"(2026-09-04) 규칙대로
레시피 재료는 각각 1종만 쓴다(못 예외 없이 전부 1종). 새 그림은 만들지 않음(이번
항목은 [BUILD], 후속 `#107`[DESIGN]이 아이콘을 만든다).)
1) `processing_table.gd`의 `RECIPES` 배열에 5개 항목을 추가했다 — #99가 만든
   `CraftingStation` 공용 베이스를 그대로 타므로 이 파일 외에 다른 로직 변경은
   필요 없었다(#105/#106 원문 예상대로).
2) `world.gd`의 `ITEM_LABELS`/`ITEM_CATEGORIES`에 `nail`("못")/`hinge`("경첩")/
   `gear`("톱니바퀴")/`copper_wire`("구리선")/`glass_bottle`("유리병")을 "가공물"로
   추가했다.
3) **검증**: 새 QA 스크립트 `game/qa/processing_bymaterials_check.gd`(커밋, 재사용
   가능, `steel_glass_copper_check.gd`(#105)와 같은 패턴)를 `project.godot
   [autoload]`에 임시 등록하고 `godot --path .`(실제 렌더링)로 실행: 가공대 근처에서
   좌클릭으로 창을 열어 12개 레시피(기존 7개+신규 5개)가 모두 목록에 뜨는지
   스크린샷(`/tmp/qa106/01_window_open.png`)으로 확인, 철 3개/강철 2개/구리 1개/
   유리 1개를 지급한 뒤 다섯 레시피를 순서대로(한 번에 배치 1개만 가능하므로 하나씩)
   `start_batch()`→시간 진행→`collect_output()`까지 실행해 각각 재료가 레시피대로
   정확히 소모되고 결과물이 정확한 개수(못4/경첩1/톱니바퀴1/구리선2/유리병1)로
   인벤토리에 들어옴을 확인(`QA_PROCESSING_BYMATERIALS_CHECK_PASS` 출력), 제작 후
   인벤토리 스크린샷(`/tmp/qa106/02_after_craft_all.png`)에서도 "못 x4"/"경첩 x1"/
   "톱니바퀴 x1"/"구리선 x2"/"유리병 x1"이 정상 표시됨을 직접 확인. 검증 후
   `project.godot`의 임시 autoload 등록을 원상복구하고 `diff /tmp/project.godot.bak106
   project.godot`로 완전히 동일함을 확인했다.
4) `docs/feedback/INBOX.md`의 `#106`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#104`(DESIGN)=1 → `#105`(BUILD)=2 → 이번 `#106`(BUILD)으로 **3**이 됐다(5 도달까지
   2개 남음 — `#107`/`#108`... 순).

## 지난 바퀴 기록 (바퀴 159, 그대로 보존)

바퀴 159 / 2026-09-04 ([BUILD] **INBOX #105 완료 — 제련로(`smelting_furnace.gd`)
`RECIPES`에 철 2개→강철 1개, 모래 2개→유리 1개, 구리광석 2개→구리 1개 세 레시피를
추가했다.** DESIGN.md "생산 라인"(2026-09-04) 규칙대로 레시피 재료는 각각 1종만 쓴다.
새 그림은 만들지 않음(이번 항목은 [BUILD], 후속 `#107`[DESIGN]이 아이콘을 만든다).)
1) `smelting_furnace.gd`의 `RECIPES` 배열에 세 항목을 추가했다 — #99가 만든
   `CraftingStation` 공용 베이스(수량 지정 → 재료 선소모 → 타이머 → 출력 버퍼 →
   수동 수령)를 그대로 타므로 이 파일 외에 다른 로직 변경은 필요 없었다(#105 원문
   예상대로).
2) `world.gd`의 `ITEM_LABELS`/`ITEM_CATEGORIES`에 `steel`("강철")/`glass`("유리")/
   `copper`("구리")를 "가공물"로 추가했다.
3) **검증**: 새 QA 스크립트 `game/qa/steel_glass_copper_check.gd`(커밋, 재사용 가능,
   `batch_crafting_check.gd`(#99)와 같은 패턴 — `station._process(delta)`를 직접
   여러 번 호출해 5초 타이머를 실시간 대기 없이 진행시킴)를 `project.godot
   [autoload]`에 임시 등록하고 `godot --path .`(실제 렌더링)로 실행: 제련로 근처에서
   좌클릭으로 창을 열어 5개 레시피(기존 2개+신규 3개)가 모두 목록에 뜨는지
   스크린샷(`/tmp/qa105/01_window_open.png`)으로 확인, 철/모래/구리광석을 각 2개씩
   지급한 뒤 세 레시피를 순서대로(한 번에 배치 1개만 가능하므로 하나씩)
   `start_batch()`→시간 진행→`collect_output()`까지 실행해 각각 재료가 정확히
   2개 소모되고 결과물이 1개씩 정확히 인벤토리에 들어옴을 확인
   (`QA_STEEL_GLASS_COPPER_CHECK_PASS` 출력), 제작 후 인벤토리 스크린샷
   (`/tmp/qa105/02_after_craft_all.png`)에서도 "강철 x1"/"유리 x1"/"구리 x1"이 정상
   표시됨을 직접 확인. 검증 후 `project.godot`의 임시 autoload 등록을 원상복구하고
   `diff /tmp/project.godot.bak105 project.godot`로 완전히 동일함을 확인했다.
4) `docs/feedback/INBOX.md`의 `#105`를 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   `#104`(DESIGN)로 1이 된 뒤, 이번 `#105`(BUILD)로 **2**가 됐다(5 도달까지 3개 남음
   — `#106`/`#107`/`#108`... 순).

## 스스로 판단해서 고친 부분 (INBOX #105)
- 지시받지 않았지만, 테스트 편의 상자(`world.gd`의 `DEBUG_STARTER_CHEST_ITEMS`,
  INBOX #97)에 `sand`/`copper_ore`를 추가했다. 근거: 그 목록은 #97 시점(모래/구리광석
  아이템이 아직 없던 때)에 고정된 15종이라, 이번 제련로 레시피(모래→유리, 구리광석→
  구리)를 실제 플레이로 검증하려면 상자에서 재료를 구할 방법이 없었다 — #103/#104가
  이 아이템을 필드 채광으로 얻을 수 있게 했지만, 테스트 편의 상자를 갱신하는 항목은
  따로 없어서 누락돼 있었다. 이 상자는 "정식 밸런스가 아닌 개발 편의용"이라고 이미
  코드 주석에 명시돼 있으므로, 새 원재료가 생길 때마다 이 목록도 같이 늘려주는 게
  그 편의 기능의 취지에 맞다고 판단했다(다음 바퀴가 새 원재료를 또 추가한다면 이
  목록도 같이 챙길 것).

## 어려움/에러 (INBOX #105)
- 없음 — `CraftingStation` 공용 베이스가 이미 있어서 `RECIPES` 배열 확장 + 아이템
  등록만으로 끝났고, 기존 QA 스크립트(`batch_crafting_check.gd`)의 패턴을 그대로
  재사용할 수 있어 API/엔진 관련 시행착오는 없었다.

## 지난 바퀴 기록 (바퀴 158, 그대로 보존)

바퀴 158 / 2026-09-04 ([DESIGN] **INBOX #104 완료 — 모래/구리광석 전용 아이템
아이콘(32×32)과 채광 포인트 월드 그림(64×64)을 파이썬 절차적 생성(Pillow)으로
만들어 #103이 남긴 임시 modulate 색칠(기존 돌/철광석 텍스처 재사용)을 실제 그림으로
교체했다.** AI 생성 도구(PixelLab/SpriteCook) 크레딧 전혀 안 씀 — 항목이 명시했으므로
잔액 확인 절차도 필요 없었다.)
1) **`/tmp/gen_104.py`(커밋 대상 아님, 관례대로 `/tmp`에 둠)로 32×32/64×64 PNG 4장을
   제작** — `/tmp/gen_ore_meat.py`(#86)의 `supersample_canvas`/`finalize`/
   `jitter_polygon`/`stretch` 헬퍼(4배 슈퍼샘플 + 가우시안 블러 2 + 알파 재스냅)를
   그대로 재사용했다. **모래**: 넓적하게 눌러 늘인(`sx=1.35, sy=0.5`) 언덕/모래더미
   실루엣(돌의 둥근 덩어리, 유황광석의 위로 솟은 크리스탈, 철광석의 각진 블록과
   전부 다른 형태) + 연한 베이지 그라디언트 + 던 물결 선 2줄 + 알갱이 질감(점
   90개를 다각형 내부 판정(`point_in_poly`)으로 무작위 산포). **구리광석**: 어두운
   적갈색 암석 받침 위에 짧고 두꺼운 다이아몬드형 크리스탈 5개를 유황광석과 달리
   위쪽만이 아니라 사방(다양한 각도)으로 박아 넣은 형태로 만들고, 각 크리스탈을
   어두운 절반/밝은 주황 절반으로 나눠 칠해 "광택 있는 결정형"이 블롭처럼 보이지
   않고 각진 단면처럼 보이게 했다(1차 시도는 원형 반점 3개짜리 뭉툭한 덩어리였는데
   Read로 직접 보고 "크리스탈이 아니라 초콜릿에 박힌 사탕 같다"고 판단해 다시 그림).
2) **파일 배치**: `assets/sprites/items/sand.png`/`copper_ore.png`(32×32),
   `assets/sprites/mining_point/mining_point_sand.png`/`mining_point_copper.png`
   (64×64) 4개를 새로 추가했다.
3) **연결**: `mining_point_sand.tscn`/`mining_point_copper.tscn`의 `texture_path`를
   전용 그림으로 바꾸고 `Sprite`의 임시 `modulate`(#103이 넣었던 베이지/붉은 갈색
   틴트)를 제거했다. `dropped_item.gd`의 `ITEM_ICONS["sand"]`/`["copper_ore"]`도
   전용 아이콘으로 바꾸고, 더 이상 아무 항목도 쓰지 않게 된 `ITEM_ICON_MODULATE`
   딕셔너리와 `_ready()`의 관련 분기를 완전히 삭제했다.
4) **검증**: 새 QA 스크립트 `game/qa/inbox104_check.gd`로 실제 렌더링 확인 —
   필드 스크린샷, 드롭 확대 스크린샷, 인벤토리 스크린샷 세 가지로 실루엣/색 구별과
   32px 이상 크기를 직접 확인, `QA_INBOX104_CHECK_PASS` 출력.
5) `docs/feedback/INBOX.md`의 `#104`를 `[x]`로 갱신. BUILD/DESIGN 완료 카운터는
   이 바퀴로 1이 됐음(자세한 경위는 위 "마지막 갱신"에 요약됨).

## 지난 바퀴 기록 (바퀴 157, 그대로 보존)
...
패널이 진짜 원인"으로 결론 내렸던 것과 같은 계열 — 이번엔 반투명이 아니라 아예
스타일이 없는 게 원인).)
1) **원인**: `world.gd`의 `_build_crafting_window()`/`_build_storage_window()`가
   `PanelContainer.new()`로 창 패널을 코드로 조립하면서 `add_theme_stylebox_override
   ("panel", ...)`를 한 번도 호출하지 않았다 — 그래서 이 프로젝트 테마
   (`theme/main_theme.tres`)에 `PanelContainer/styles/panel` 정의가 없는 채로 엔진
   기본 테마의 패널 스타일로 대체되는데, 이게 배경을 사실상 안 그리는 수준이라 뒤에
   있는 월드(카메라가 항상 플레이어를 따라가므로 창이 뜨면 거의 항상 플레이어가 화면
   중앙, 즉 패널 자리에 있음)가 그대로 다 비쳤다. 반면 인벤토리 창(E키,
   `world.tscn`의 `UI/InventoryWindow/.../Panel`)은 씬 파일에서
   `theme_override_styles/panel = StyleBoxFlat_pause_panel`(불투명)을 명시하고 있어서
   이 문제가 없었다 — 그래서 #112 원문이 "인벤토리 창만 재현 안 됨"이라고 정확히
   짚어뒀던 것.
2) **수정**: `world.gd`에 `_make_window_panel_style() -> StyleBoxFlat` 헬퍼를 새로
   추가했다 — 값은 `world.tscn`의 `StyleBoxFlat_pause_panel`(인벤토리 창이 이미 쓰고
   있는 것)과 완전히 동일하게 맞췄다(불투명 bg `Color(0.157,0.212,0.184,1)`, 테두리,
   둥근 모서리, 여백 — 새 색을 지어내지 않고 이미 검증된 값을 재사용). 가공대/저장
   상자 두 창의 `PanelContainer`에 각각 `panel.add_theme_stylebox_override("panel",
   _make_window_panel_style())` 한 줄씩만 추가했다.
3) **검증**: `project.godot [autoload]`에 `game/qa/full_sweep.gd`를 임시 등록하고
   (저장 파일 먼저 삭제) `godot --path .`로 37스텝 전체 스윕을 재실행 — 5개 창
   (`storage_chest_open`/`processing_table_open`/`smelting_furnace_open`/
   `cooking_table_open`/`cooking_stove_open`) 스크린샷을 전부 Read 도구로 직접
   봤다. 다섯 장 모두 패널이 완전히 불투명한 녹회색 배경으로 나오고, 플레이어나
   근처 월드 오브젝트(사슴/나무/그루터기 등)가 패널 위/뒤로 전혀 비치지 않음을
   확인했다 — #112가 지적한 증상이 다섯 창 모두에서 사라짐. 검증 후
   `project.godot`의 임시 autoload 등록을 원상복구하고 `diff <(git show
   HEAD:game/project.godot) game/project.godot`로 완전히 동일함을 확인했다(커밋
   대상은 `world.gd` 한 파일뿐).
4) `docs/feedback/INBOX.md`의 `#112`를 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   이전 3(#101 완료 시점)에서 이번 #112(BUILD)로 **4**가 됨(5 도달까지 1개 남음 —
   다음 BUILD/DESIGN 항목 하나가 더 끝나면 QA 전체 스윕 신규 등록).

## 지난 바퀴 기록 (바퀴 155, 그대로 보존)

바퀴 155 / 2026-09-04 ([QA] **INBOX #102 완료 — 전체 스윕, 결함 1건 발견 → #112로
티켓 발행.** 범위: 메인 메뉴부터 도구/인벤토리/농사/채집/채광/벌목/목장/사냥/낮밤·날씨
+ 이번에 처음 스윕 경로에 추가한 저장 상자·가공대·제련로·조리대·조리용 화로.)
1) **`game/qa/full_sweep.gd`에 저장 상자/가공대/제련로/조리대/조리용 화로를 여는
   5개 스텝(`storage_chest_open`/`storage_chest_transfer`/`processing_table_open`/
   `processing_table_batch_collect`/`smelting_furnace_open`/`cooking_table_open`/
   `cooking_stove_open`)을 새로 추가했다** — #96~#101에서 이 오브젝트들이 생겼는데도
   그동안 전체 스윕이 한 번도 실제로 열어본 적이 없었다. 저장 상자 이전은
   `try_transfer_to_player()`로, 배치 제작은 `start_batch()` → `_process(6.0)`로
   5초 타이머를 즉시 완료시킨 뒤 `_on_collect_pressed()`로 수령까지 실제로 확인했다.
2) **`game/qa/inventory_safety_check.gd`를 확장** — 원래 INBOX #98 검증은 (a) 바닥
   드롭 줍기, (b) 제작 "시작" 거부만 봤는데, **저장 상자 이전(#96)과 제작 "수령"
   (#99 `collect_output()`, 시작과는 다른 코드 경로)은 인벤토리가 꽉 찬 상태로 한
   번도 검증된 적이 없었다** — #102 원문이 정확히 이 구멍을 짚어서 이번에 채웠다.
   **실행 중 이 파일 자체가 깨져 있는 걸 발견**: `_on_craft_pressed()`를 부르고
   있었는데, 이 함수는 INBOX #99(바퀴 152)가 "즉시 완성" 방식을 통째로 없애면서
   이미 삭제된 함수였다(다른 QA 스크립트 5개 — `cooking_check.gd`/
   `gunpowder_ammo_check.gd`/`processing_table_check.gd`/`smelting_furnace_check.gd`/
   `starter_chest_check.gd`도 같은 이유로 stale하지만, 이번에 실제로 재사용한
   `inventory_safety_check.gd`만 새 API(`start_batch`/`_process`/`collect_output`)로
   고쳤다 — 나머지는 이번 범위 밖이라 손대지 않음). 새 API로 다시 쓰고 4개 체크
   (드롭 줍기 거부/성공, 상자 이전 거부/성공, 제작 수령 거부/성공)를 전부 통과시켰다
   — **결과: #98 안전 패턴이 세 경로(드롭 줍기/상자 이전/제작 수령) 전부에서 실제로
   유지된다.**
3) **결함 1건 발견 (#112로 티켓 발행)**: 저장 상자/가공대/제련로/조리대/조리용
   화로 — 5개 창을 열면 플레이어 캐릭터(와 근처 월드 오브젝트)가 창 뒤가 아니라
   **창 패널 위에 또렷하게** 겹쳐 그려진다. 카메라가 항상 플레이어를 따라가고 창도
   화면 중앙에 뜨므로 실제 플레이에서 거의 매번 발생하는 문제다. `world.gd`의 두
   창 빌더 모두 `ui_layer`(`layer=10`, INBOX #68에서 고정)에 창을 붙이는데도
   재현됐다 — INBOX #68/#75(바퀴 120/127) 때의 CanvasLayer 순서 문제와 같은 계열일
   수도, 다른 원인(Player 쪽 z_index/top_level)일 수도 있어 원인 조사는 #112로
   넘겼다. **이미 커밋된 예전 스크린샷(`/tmp/qa96/02_window_open_filled.png`,
   INBOX #96/바퀴 149)에도 같은 현상이 이미 찍혀 있었다** — 그동안 아무도 지적하지
   않고 넘어간 회귀/누락으로 보인다. 인벤토리 창(E키)은 같은 문제가 재현되지 않음을
   확인했다(불투명 배경으로 완전히 가려짐) — 저장/제작 두 창만의 문제다.
4) 이 하네스는 그림/코드를 고치지 않는다는 규칙대로 #112 조사/수정은 하지 않았다.
   `docs/feedback/INBOX.md`의 `#102`를 `[x]`로 갱신하고 `#112`([BUILD])를 새로 등록.
   전체 스윕이라 문제를 찾았어도 "QA 재확인" 항목은 추가하지 않았다(전체 스윕은
   5개마다 자동으로 다시 생기므로 — PROMPT_QA.md ③ 규칙).

## 지난 바퀴 기록 (바퀴 154, 그대로 보존)

바퀴 154 / 2026-09-04 ([DESIGN] **INBOX #101 완료 — 나무벽/나무문/석제벽 3종의 인벤토리
아이템 아이콘을 파이썬 절차적 생성(Pillow)으로 제작.** AI 생성 도구(PixelLab/SpriteCook)
크레딧을 전혀 쓰지 않았다 — 항목 자체가 그렇게 명시했으므로 잔액 확인 절차도 필요 없었다.)
1) **`/tmp/gen_101.py`(커밋 대상 아님, 관례대로 `/tmp`에 둠)로 32×32 PNG 3장을
   제작** — `#79`/`#86`/`#91`과 같은 스타일(수직 그라디언트 음영 `vgrad_rect()` +
   얇은 어두운 외곽선)을 새로 구현해서 재사용: `wood_wall.png`(가로 4단 나무판을
   이어붙인 벽 패널, 판자 사이 이음매 어두운 선 + 세로 결 하이라이트),
   `wood_door.png`(같은 갈색 톤이되 위/아래 두 패널로 들어간 문짝 모양 + 작은 금속
   손잡이(황동색 원)로 wood_wall과 실루엣 차별화), `stone_wall.png`(회색 그라디언트
   바탕에 어긋쌓기(running bond) 벽돌 모르타르 선 패턴, 상단 하이라이트로 입체감).
   기존 `plank.png`/`stone_block.png`과 같은 32×32 RGBA(배경 투명) 규격을 그대로
   따랐다.
2) **`scenes/dropped_item/dropped_item.gd`의 `ITEM_ICONS`에 3개 항목 추가** —
   `wood_wall`/`wood_door`/`stone_wall`. 별도 스케일 오버라이드 불필요(이미 32px
   규격이라 `ITEM_ICON_SCALE_OVERRIDES` 손대지 않음).
3) **검증**: 새 QA 스크립트 `game/qa/wall_door_icon_check.gd`(커밋, 재사용 가능)로
   `project.godot [autoload]`에 임시 등록 후 `godot --path .`로 실제 렌더링 실행 —
   새 아이템 3개를 바닥에 드롭해서 한 화면에 모아 찍고(`/tmp/qa101/00_dropped_icons.png`),
   인벤토리 창에 채워 넣어 라벨 표시도 확인(`/tmp/qa101/01_inventory.png`). 4배 확대
   크롭(`/tmp/qa101_crop.png`)으로 Read 도구로 직접 봐서 세 아이콘이 뚜렷이 다른
   실루엣(판자 격자/손잡이 달린 문/벽돌 패턴)과 색(갈색 2종 vs 회색)으로 구별되고,
   화면에서 32px 이상으로 잘 보이며, 인벤토리 라벨("나무벽 x3" 등)도 기존 아이템과
   같은 스타일로 정상 표시됨을 확인 — ①의 세 기준(정적 오브젝트라 실루엣 구별로 판정)
   전부 통과.
4) **새 PNG를 추가한 뒤 `godot --headless --path . --import`로 강제 재임포트가
   필요했다** — 임포트 전에는 `Preload file "..." has no resource loaders` 파싱
   에러가 나서 `dropped_item.gd` 전체가 로드 실패했다(기존 STATUS.md 체크리스트
   그대로 재현, 새로 배운 것 아님). 재임포트 후 `window/stretch/aspect="keep"` 줄이
   그대로 남아있는지 확인했다(사라지는 부작용 없음).
5) **원상복구**: `project.godot`에 임시로 넣은 autoload 등록을 제거하고
   `diff <(git show HEAD:game/project.godot) game/project.godot`로 완전히 동일함을
   확인했다. 캐릭터/인벤토리 세이브 파일을 지우고 처음부터 재검증했다.
6) `docs/feedback/INBOX.md`의 `#101`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**:
   #100 완료 시 2였던 것이 이번(#101)으로 **3**이 됐다(5 도달까지 2개 남음 — `#102`
   [QA] 전체 스윕은 이미 큐에 있으므로 카운터와 무관하게 다음 QA 하네스 차례에 처리됨).

## 스스로 판단해서 고친 부분 (INBOX #101)
- 카메라를 드롭된 아이템 위치로 수동 이동시키기 전에 `world.set_physics_process(false)`를
  호출했다(지시받지 않음, 검증 스크립트에만 해당). 근거: 처음 시도에서 `world.gd`의
  `_physics_process`가 매 프레임 `camera.global_position`을 플레이어 위치로 되돌려버려서
  (마우스 추종 로직이 `_facing`을 덮어쓰는 것과 같은 종류의 문제, STATUS.md "다음에 할 것"에
  이미 있던 경고와 동일 원인) 카메라가 드롭 아이템이 아니라 멀리 치워둔 플레이어를 따라가
  스크린샷에 아이템이 전혀 안 보이는 결함을 실제로 겪었다 — `set_physics_process(false)`로
  고치자 정상적으로 보였다. **다음 DESIGN 바퀴가 참고할 것**: 드롭 아이템/오브젝트를 향해
  카메라를 수동 배치하는 검증 스크립트를 짤 때는 항상 이 호출을 먼저 넣을 것(플레이어
  위치를 강제 조작하는 모든 검증에 적용되는 규칙으로 일반화해도 됨).

## 어려움/에러 (INBOX #101)
- 새 PNG 3장을 만들고 바로 `godot --path .`를 실행했다가 `Preload file "..." has no
  resource loaders (unrecognized file extension)` 에러로 `dropped_item.gd` 로드 자체가
  실패했다 — `.import` 메타파일이 아직 없어서였다. `godot --headless --path . --import`를
  먼저 실행해 강제 재임포트한 뒤에야 정상 로드됐다(기존 체크리스트에 이미 있던 절차,
  이번에 실제로 재현해서 재확인).

## 지난 바퀴 기록 (바퀴 153, 그대로 보존)

바퀴 153 / 2026-09-04 ([BUILD] **INBOX #100 완료 — 나무벽/나무문/석제벽을 "아이템으로
제작할 수만 있게" 가공대 레시피에 추가.** 설치(월드에 실제로 배치해서 방을 짓는 것)는
이번 범위가 아니다(사용자가 명시적으로 범위를 좁힘, 나중에 별도 지시).)
1) **`scenes/processing_table/processing_table.gd`의 `RECIPES` 상수에 항목 3개만
   추가했다** — INBOX #99가 만든 배치(수량 지정+타이머+수동 수령) 프레임워크를 그대로
   타므로 새 오브젝트나 새 UI가 필요 없었다: `{"inputs": {"plank": 4}, "output":
   "wood_wall", "amount": 1}`, `{"inputs": {"plank": 3}, "output": "wood_door",
   "amount": 1}`, `{"inputs": {"stone_block": 4}, "output": "stone_wall", "amount": 1}`.
   재료 개수는 재량값(INBOX 원문 명시, 나중에 밸런스 조정 예정).
2) **`scenes/world/world.gd`의 `ITEM_LABELS`/`ITEM_CATEGORIES`에 새 아이템 3종을
   추가했다** — `wood_wall`("나무벽")/`wood_door`("나무문")/`stone_wall`("석제벽"), 셋 다
   카테고리 "완성품". 텍스처는 이번 범위가 아니라서(후속 [DESIGN] #101) 인벤토리 창에는
   기존 도구/가공물처럼 텍스트로만 표시된다 — `ITEM_LABELS`에만 있으면 인벤토리 UI가
   자동으로 텍스트를 그리므로 추가 코드가 필요 없었다.
3) **검증**: 새 QA 스크립트 `game/qa/wall_door_recipe_check.gd`(커밋, 재사용 가능)로
   `project.godot` `[autoload]`에 임시 등록 후 `godot --path .`로 실제 렌더링 실행 —
   세 레시피 각각 재료를 채우고 `start_batch()`로 시작해 재료가 정확히 소모되는지,
   `_process(6.0)`으로 타이머(5초)를 완주시켜 출력 버퍼에 정확한 개수가 쌓이는지,
   `collect_output()`으로 인벤토리에 들어가는지 전부 확인
   (`QA_WALL_DOOR_RECIPE_CHECK_PASS`). 인벤토리 창을 열어 스크린샷
   (`/tmp/qa100/01_inventory_with_new_items.png`)도 Read로 직접 확인 — 나무벽/나무문/
   석제벽이 판자/석재 등 기존 아이템과 같은 스타일(칸+이름+수량)로 정상 표시되고 서로
   안 겹침.
4) **원상복구**: `project.godot`에 임시로 넣은 autoload 등록을 제거하고 `git diff`로
   완전히 동일함을 확인했다(`window/stretch/aspect="keep"` 줄도 그대로 남음). 캐릭터/
   인벤토리 세이브 파일을 지우고 처음부터 재검증했다.
5) `docs/feedback/INBOX.md`의 `#100`을 `[x]`로 갱신. **BUILD/DESIGN 완료 카운터**: #99
   완료 시 1이었던 것이 이번(#100)으로 **2**가 됐다(5 도달까지 3개 남음 — 다음 자동 QA
   스윕이 등록되기 전에 이미 `#102`가 큐에 있다).

**다음에 할 것**: **다음 DESIGN 세션은 INBOX #101**(나무벽/나무문/석제벽 3종의 인벤토리
아이템 아이콘을 파이썬 절차적 생성(Pillow)으로 제작, AI 생성 도구 크레딧 안 씀)을
처리한다. 참고할 것: (1) `#79`/`#86`/`#91`과 같은 절차적 생성 기법(도형 레이어링+
그라디언트+안티에일리어싱)을 재사용. (2) 나무벽=갈색 나무판 질감, 나무문=같은 톤이되
문 형태(손잡이 등)로 구별, 석제벽=회색 돌 블록 질감 — 서로 실루엣이 확실히 구별되게.
(3) `scenes/dropped_item/dropped_item.gd`의 `ITEM_ICONS`에 연결. (4) 그 다음은 `#102`
([QA] 전체 스윕, 이미 큐에 등록돼 있음 — #94~#98 구간 확인용으로 등록됐지만 #99/#100도
그 사이 완료됐으니 이번 스윕에서 배치 제작(#99)과 나무벽/문/석제벽 레시피(#100)도 함께
확인할 것). (5) BUILD/DESIGN 완료 카운터는 이번(#100)으로 **2** — #101이 완료되면 3이
된다.

## 지난 바퀴 기록 (바퀴 152, 그대로 보존)
...
## 스스로 판단해서 고친 부분 (INBOX #99, 지난 바퀴 기록 — 그대로 보존)
...
## 어려움 / 에러와 해결 (INBOX #99, 지난 바퀴 기록 — 그대로 보존)
...
것을 확인 없이 썼다.)
1) **`InventoryData.add_item(item_name, amount)`의 반환형을 `void`에서 `int`로 바꿔
   실제로 넣은 개수를 반환**하게 했다(기존 호출부는 전부 반환값을 무시하는 statement라
   시그니처 변경만으로는 하나도 깨지지 않음, 사전에 전체 호출부를 grep으로 확인).
   `has_room(item_name, amount) -> bool` 공용 메서드도 새로 추가했다.
2) **`storage_chest.gd`의 `try_transfer_to_player()`가 자체적으로 갖고 있던
   `_inventory_has_room()`을 지우고 새 `InventoryData.has_room()`을 그대로 호출**하도록
   통합했다(INBOX #98 원문의 "중복 제거, 선택" 요구사항 — 로직이 완전히 같아서 통합 안
   할 이유가 없었다).
3) **`dropped_item.gd`의 `_process()`**: `add_item()`의 반환값(실제로 넣은 개수)만큼
   `item_amount`에서 차감하고, `item_amount`가 0보다 크게 남아있으면 `queue_free()`하지
   않는다(다음 프레임에 남은 만큼 다시 시도 — INBOX #98 원문 그대로).
4) **`world.gd`의 `_on_craft_pressed()`**: 재료 소모 **전에** `InventoryData.has_room(output,
   amount)`를 먼저 확인하고, 공간이 없으면 재료를 건드리지 않고 그대로 반환한다. 추가로
   `_storage_message_label`과 같은 패턴으로 `_crafting_message_label`/
   `_crafting_message_timer`를 새로 만들어 실패 시 "인벤토리에 공간이 없습니다"를 1.5초간
   보여주게 했다(INBOX #98 원문 "필수는 아님"이라고 했지만, 버튼을 눌러도 조용히
   아무 일도 안 일어나면 사용자가 버그로 오인하기 쉬워 품질 기준(①) 차원에서 추가함 —
   지시받지 않았지만 추가한 개선).
5) **검증**: 새 QA 스크립트 `game/qa/inventory_safety_check.gd`(커밋, 재사용 가능)로
   `project.godot` `[autoload]`에 임시 등록 후 `godot --path .`로 실제 렌더링 실행 —
   (a) 일반 슬롯 18칸을 서로 다른 필러 아이템 99개씩으로 완전히 채운 뒤 바닥에 돌 5개를
   드롭해도 사라지지 않고 그대로 남는지, (b) 그 상태로 가공대 레시피(재료는 이미 보유)를
   눌러도 재료가 안 사라지고 결과물도 안 생기는지(+ 메시지 표시), (c) 슬롯 3칸을 비워
   공간을 만든 뒤 같은 드롭 아이템이 마저 자동으로 주워지는지, (d) 같은 방식으로 공간이
   생긴 뒤 제작(목재→판자)이 정상적으로 재료 소모/결과물 생산까지 성공하는지 — 4개 전부
   확인(`QA_INVENTORY_SAFETY_CHECK_PASS`). 최종 상태 스크린샷(`/tmp/qa98/01_after_checks.png`)
   도 Read로 직접 확인 — 인벤토리 표시가 "돌 x5, 판자 x1, ..."로 기대한 그대로 나옴.
6) **원상복구**: `project.godot`에 임시로 넣은 autoload 등록을 제거하고 `diff`로 완전히
   동일함을 확인(`window/stretch/aspect="keep"` 줄도 그대로 남아있음). 저장/캐릭터 세이브
   파일을 지우고 재확인했다.
7) `docs/feedback/INBOX.md`의 `#98`을 `[x]`로 갱신. **INBOX #99가 다음 차례** — #99
   원문이 "#98을 먼저 완료한 뒤 진행"이라고 명시했으니 이번 완료로 막힘이 풀렸다.
   **BUILD/DESIGN 완료 카운터**: #97에서 4였던 것이 이번(#98)으로 **5에 도달** — 규칙대로
   `docs/feedback/INBOX.md`에 `#102 [QA] 전체 스윕` 항목을 새로 추가하고 카운터를 **0으로
   리셋**했다.

**다음에 할 것**: **다음 BUILD 세션은 INBOX #99**(제작 방식을 수량 지정+타이머+제작대
내부 출력 버퍼+수동 수령 방식으로 전면 개편)를 처리한다. 참고할 것: (1) 이번에 만든
`InventoryData.has_room()`/`add_item()`의 int 반환값을 #99의 "수령" 단계(출력 버퍼 →
인벤토리)가 그대로 재사용하면 된다 — #98이 요구했던 "공간 확인 후 이전, 부족하면 그대로
남김" 패턴이 이미 완성돼 있다. (2) `_on_craft_pressed()`는 #99에서 통째로 다른 흐름(타이머
기반)으로 바뀔 예정이니 이번에 추가한 `_crafting_message_label` 패턴은 그대로 새 흐름에도
재사용 가능(제작 시작 실패/수령 실패 메시지 등). (3) `game/qa/inventory_safety_check.gd`는
#99 이후에도 InventoryData 안전 패턴 자체를 재검증할 때 그대로 재사용 가능. (4) BUILD/DESIGN
완료 카운터는 0부터 다시 세기 시작 — `#102`(QA 전체 스윕)는 INBOX 큐에 이미 등록돼 있고
번호 순서상 `#99`/`#100`/`#101` 뒤에 처리된다.

## 지난 바퀴 기록 (바퀴 150, 그대로 보존)
...
- INBOX #113 완료 (바퀴 166, QA 전체 스윕): 위 "마지막 갱신" 참고. 43장 캡처, 결함
  1건(리소스 포인트 간 최소 거리 미보장 + 좌클릭 한 번에 겹친 포인트 전부 채집되는
  버그) 발견해 #115로 티켓 발행. #103/#104가 만든 모래/구리광석 채광 포인트 자체는
  결함 없음(격리된 좌표에서 재확인 완료).
- INBOX #106 완료 (바퀴 160, BUILD): 위 "지난 바퀴 기록 (바퀴 159)" 위 "마지막 갱신"
  참고. 가공대 `RECIPES`에 부재료 5종(못/경첩/톱니바퀴/구리선/유리병) 레시피를
  추가했다. 레시피 재료는 전부 1종만 사용. `game/qa/processing_bymaterials_check.gd`
  로 5개 레시피 전부 소모/타이머/수령까지 검증.
- INBOX #105 완료 (바퀴 159, BUILD): 제련로 `RECIPES`에 강철/유리/구리 레시피 3개를
  추가했다. `game/qa/steel_glass_copper_check.gd`로 검증.
...
- INBOX #112 완료 (바퀴 156, BUILD): 위 "마지막 갱신" 참고. 저장 상자/가공대/제련로/
  ...
  탓에 플레이어/월드가 패널 위에 그대로 비치던 버그를 고쳤다. `game/qa/
  full_sweep.gd` 37스텝으로 5개 창 전부 재확인.
- INBOX #100 완료 (바퀴 153, BUILD): 위 "마지막 갱신" 참고. 나무벽/나무문/석제벽을
  "아이템으로 제작 가능"하게만 가공대 `RECIPES`에 항목 3개 추가(설치는 범위 밖).
  새 완성품 아이템 3종을 `ITEM_LABELS`/`ITEM_CATEGORIES`에 추가. `game/qa/
  wall_door_recipe_check.gd`로 세 레시피 전부 소모/타이머/수령까지 검증.
- INBOX #99 완료 (바퀴 152, BUILD): 위 "지난 바퀴 기록 (바퀴 152)" 참고. 제작 방식을
  즉시 완성에서 수량 지정+타이머(5초/개)+작업대 내부 출력 버퍼+수동 수령 방식으로
  전면 개편. 공용 베이스 `scripts/crafting_station.gd`(`CraftingStation`)로 가공대/
  제련로/조리대/조리용 화로 4개를 통합. `game/qa/batch_crafting_check.gd`로 검증.
...
- **(2026-09-04 갱신, 바퀴 164) INBOX #110([BUILD]) 완료 — 가공대 전기 계열
  완성품 3종(배터리/조명/발전기) + 사료 + 급수 장치 레시피 추가, 부가로 가공대(및
  제련로/조리대/조리용 화로 공용) 제작 창에 `ScrollContainer`를 추가해 20개로 늘어난
  레시피 목록이 화면 밖으로 넘쳐 클릭 불가능하던 문제를 고침.** 위 "마지막 갱신"
  참고. **INBOX에 남은 미완료 항목(번호 순): `#111`([DESIGN] #108~#110의 완성품
  아이콘 11개+가공식품 1개, 다음 차례) → `#113`/`#114`([QA] 전체 스윕, 아래 참고).**
  **BUILD/DESIGN 완료 카운터**: #109(BUILD)=1 → 이번 #110(BUILD)으로 **2**가 됐다
  (5 도달까지 3개 남음 — `#111`이 DESIGN이라 완료되면 3이 됨). `#113`/`#114`([QA]
  전체 스윕)는 이미 큐에 있고 번호 순서상 `#111`보다 뒤에 자연스럽게 대기한다 —
  처리될 때 `game/qa/full_sweep.gd`(바퀴 155가 저장 상자/가공대/제련로/조리대/조리용
  화로 창까지 스텝을 추가해둔 버전)를 재사용하되, 모래/구리광석 채광 포인트를 실제로
  캐보는 스텝도 하나 추가하면 좋다(INBOX #113 원문에 이미 이렇게 적어뒀음).
- **(신규, 바퀴 164) 가공대/제련로/조리대/조리용 화로가 공유하는 범용 제작 창
  (`world.gd`의 `_build_crafting_window()`)에 `ScrollContainer`(높이 420px)가
  추가됐다 — 레시피가 계속 늘어나도 이제 스크롤로 전부 접근 가능하다.** 이전에는
  레시피가 화면(720px) 세로 길이를 넘으면 아래쪽 레시피 버튼이 화면 밖에 위치해
  실제 플레이어가 클릭할 수 없었다(가공대가 20개 레시피에 도달한 뒤 INBOX #110에서
  발견/수정). 새 QA 스크립트를 짤 때 레시피가 많은 작업대를 검증한다면
  `crafting_scroll.scroll_vertical = 100000`으로 맨 아래까지 스크롤한 뒤 스크린샷을
  찍는 패턴(`game/qa/electric_check.gd` 참고)을 재사용해서 새로 추가한 레시피가
  실제로 보이고 클릭 가능한 위치에 있는지 확인할 것.
...
- **(신규, 바퀴 155) `game/qa/inventory_safety_check.gd`를 재사용할 때 참고**: 이
  ...
  **한 체크가 끝난 뒤 다음 체크를 위해 또 다른 prefix로 다시 "완전히 채우려" 하면,
  이미 꽉 찬 상태에서는 새 아이템 스택이 하나도 안 들어가서(`add_item`이 0을 반환)
  그 이름으로 된 아이템이 실제로는 존재하지 않게 된다** — 그 상태에서 "특정 이름의
  필러를 지워서 공간을 만든다"고 하면 없는 아이템이라 `remove_item`이 조용히
  아무것도 안 하고 false만 반환해 "공간을 만들었다"고 착각한 채 뒤 체크가 계속
  실패한다(이번에 실제로 겪음). **해결: 이름을 지목해 지우지 말고, 슬롯 배열의
  마지막(또는 아무) 실제 내용물을 그대로 지우는 방식(`_free_last_occupied_slot()`,
  이 파일에 이미 있음)을 쓸 것.** 또한 재료를 새로 넣어야 하는 체크(배치 제작
  시작 등)는 **인벤토리를 완전히 채우기 "전에"** 그 재료를 먼저 넣어둘 것 — 꽉 찬
  뒤에는 새 아이템을 위한 빈 칸이 없어 그 자체로 실패한다.
- **(신규, 바퀴 155) `game/qa/cooking_check.gd`/`gunpowder_ammo_check.gd`/
  `processing_table_check.gd`/`smelting_furnace_check.gd`/`starter_chest_check.gd`
  5개는 전부 INBOX #99(바퀴 152)가 지운 `world._on_craft_pressed()`를 그대로 호출하고
  있어 지금 실행하면 존재하지 않는 함수 호출로 실패한다(stale).** `game/qa/
  batch_crafting_check.gd`(#99가 만든 새 API 예시)나 이번에 고친
  `inventory_safety_check.gd`를 새 패턴(`station.start_batch(recipe, qty)` →
  `station._process(CraftingStation.CRAFT_SECONDS_PER_UNIT * qty + 1.0)`로 타이머
  즉시 완료 → `station.collect_output()` 또는 `world._on_collect_pressed()`)의 참고
  예시로 삼을 것 — 이번 바퀴는 예산상 저 5개를 직접 고치지 않았다(범위 밖, 필요해지면
  그때 고칠 것).
...
- (바퀴 181) `game/qa/processing_table_check.gd`가 `world.gd`에 없는
  `_on_craft_pressed()`를 호출해서 실행하면 바로 실패한다 — INBOX #128 검증 중
  회귀 확인차 돌려보다가 발견했으며, 이 항목이 건드린 파일과는 무관해서(제작 UI가
  이후 다른 API로 개편되면서 남은 낡은 스크립트로 추정) 이번 바퀴에서 고치지 않고
  기록만 한다. 다음에 제작대류(가공대/제련로/조리대/조리용 화로) 관련 항목을 다룰
  때 이 스크립트를 최신 `open_crafting_window()` API에 맞게 고치거나, 이미 대체하는
  다른 QA 스크립트가 있다면 지울 것.

## 결정 로그

(바퀴 중 내린 크고 작은 결정과 이유. 다음 바퀴가 되돌리지 않도록)

- (2026-09-05, 바퀴 173) INBOX #120 검증 중 확인: `world.gd`의
  `_move_player_with_grid_collision(motion)`은 이동 시작~끝 사이의 **목적지 지점만**
  막혔는지 검사하고, 그 사이 지나가는 중간 격자 칸은 전혀 보지 않는다. 실제
  게임에서는 `_physics_process`가 프레임당 아주 작은 이동량(`MOVE_SPEED * delta`,
  60fps 기준 약 3.6유닛으로 격자 크기 64보다 훨씬 작음)만 넘기기 때문에 사실상 연속
  충돌처럼 동작해서 문제가 없지만, **QA 스크립트가 검증 편의를 위해 이 함수에
  한 번에 큰 이동량(예: 500)을 통째로 넘기면, 벽/문 한 칸을 그냥 건너뛰어 목적지가
  벽 너머 먼 칸에 떨어지면서 "충돌 없음"으로 잘못 통과 처리될 수 있다.** 실제로
  `inbox120_check.gd` 작성 중 이 방식으로 "열린 문을 지나갈 수 있는지" 검증이 원인
  불명으로 실패해서 원인을 추적한 결과 발견함 — `inbox119_check.gd`(#119)의 벽 차단
  검증도 같은 500유닛 단일 호출 패턴을 쓰고 있어 이 문제를 안고 있었지만 그때는
  우연히(목적지가 벽 칸 밖으로 딱 벗어나지 않아서) 통과했던 것으로 보인다. **앞으로
  플레이어 이동/충돌을 다루는 새 QA 스크립트를 짤 때는 `_move_player_with_grid_collision()`
  을 한 번에 큰 값으로 부르지 말고, 격자 크기보다 훨씬 작은 값(예: 8유닛)으로 여러 번
  (예: 80회) 나눠 호출해서 실제 물리 루프를 흉내낼 것** — `inbox120_check.gd`의 패턴을
  재사용. 이 자체는 실제 플레이에서 벌어지는 게임 버그가 아니라(정상 프레임레이트에서는
  절대 발생하지 않음) QA 스크립트 작성 시의 함정이라 INBOX 티켓으로 등록하지 않았다.
- (2026-09-04, 바퀴 163) INBOX #109 검증 중 확인: 캐릭터 슬롯 0을 재사용하는 QA
  스크립트(`*_check.gd`)가 `#105`~`#108`처럼 반복해서 이 슬롯으로 아이템을 지급하기만
  하고 정리하지 않으면, 저장된 인벤토리 일반 슬롯(18칸)이 결국 꽉 차서 다음 QA
  스크립트가 "재료 부족"이 아니라 "받을 공간이 없어서" 수령에 실패하는 것처럼 보이는
  거짓 실패를 겪는다(실제로 #109가 이 문제로 4/5 레시피가 처음엔 실패로 나왔다 —
  코드 결함 아님, DESIGN.md "인벤토리 안전" 규칙대로 정상 동작이었음). **앞으로 새
  가공/제작 QA 스크립트를 짤 때는 검증 시작 전에
  `InventoryData._general_slots.fill(null); InventoryData._save()`로 일반 슬롯을 비우고
  시작하는 것을 기본으로 넣을 것** — `steel_construction_check.gd`(#109)에 이미 적용된
  패턴을 그대로 재사용하면 된다.
- (2026-09-04, 바퀴 156) INBOX #112 조사 결과, "코드로 새 팝업 창을 만들 때
  `PanelContainer.new()`에 반드시 `add_theme_stylebox_override("panel", ...)`로
  불투명 배경을 명시해야 한다"는 걸 확인함 — 이 프로젝트 테마
  (`theme/main_theme.tres`)는 `PanelContainer`용 기본 패널 스타일을 정의해두지
  않아서, 스타일을 안 주면 엔진 기본 테마로 대체되고 사실상 배경이 거의 안 그려진다.
  `world.gd`에 `_make_window_panel_style()` 헬퍼를 만들어뒀으니, 앞으로 새 팝업 창을
  코드로 만들 때는(예: 나중에 광산/방 UI 등) 이 헬퍼를 재사용할 것 — 매번 새
  `PanelContainer.new()`를 쓸 때마다 이 스타일 override를 빠뜨리지 않았는지 확인
  하는 습관을 들일 것. **참고로 이 증상(월드가 UI 위에 그려지는 것처럼 보임)은
  #75(바퀴 127)가 겪었던 것과 겉보기엔 같은 계열이지만 원인은 서로 다르다** — #75는
  "패널이 반투명(alpha 0.72)이라 28%만큼 비쳐 보인 것"이었고, 이번엔 "패널 스타일
  자체가 사실상 없어서 거의 안 그려진 것"이었다. 둘 다 CanvasLayer 순서 자체는
  정상이었다 — 이런 증상을 보면 항상 CanvasLayer 구조를 의심하기 전에 먼저 패널의
  스타일/알파값부터 확인할 것(#75가 남긴 교훈을 이번에 다시 확인함).
...
- 바퀴 39: INBOX #37(총 들기/발사 모션 분리) 코드와 `gun_firing.png`가 세션 시작 전부터
  ...
  스크립트에만 해당).
- (바퀴 186, INBOX #133) **충돌 크기와 시각 크기를 별도 상수로 분리하는 방식(#132)을
  폐기하고, 다시 `BUILD_GRID_SIZE` 하나로 합치기로 확정함.** 근거(사용자 스크린샷):
  두 값이 다르면(16 vs 32) "격자 기준 비겹침"과 "화면 기준 비겹침"이 서로 어긋나서,
  어느 쪽 기준으로 배치하든 반대쪽 기준에서 문제(겹쳐 보이거나 실제로 틈이 생김)가
  생기는 게 구조적으로 불가피했다. 원본 벽/문/창문 아이콘이 전부 32×32라는 사실을
  그대로 활용해 `BUILD_GRID_SIZE=32.0` 하나로 고정하면 scale=1.0(원본 해상도, 확대/
  축소 없음)과 충돌=표시 1:1이 동시에 만족된다 — 이후 벽/문/창문류에 새 상수를
  추가로 도입하지 말고 이 값 하나만 참조할 것. `ROOM_FLOOD_CELL_CAP`도 격자 한 칸의
  면적 변화(4배)에 반비례해서 함께 조정해야 한다는 것을 재확인함(칸수×칸면적을
  불변량으로 유지) — 앞으로 `BUILD_GRID_SIZE`를 또 바꿀 일이 있으면 이 상한도 반드시
  같이 재계산할 것.
