...
- **(2026-09-04 갱신, 바퀴 145) INBOX #92([QA] #84~#86 구간 전체 스윕) 완료 —
  결함 2건 발견, `#94`/`#95`([DESIGN])로 발행함.** 위 "마지막 갱신"/"발견한 문제"
  참고. **다음 `[DESIGN]` 항목은 `#94`**(고기 드롭 아이콘 32px 미달 재작업)와
  **`#95`**(유황광석 채광 포인트 그림을 불꽃처럼 안 보이게 다시 그리기) — 둘 다
  이번 QA 바퀴가 신규 발행했고 아직 처리 안 됨. **`#93`([QA] #87~#91 가공/조리
  라인 전체 스윕)은 INBOX 원문에 "`#92`가 먼저 처리된 뒤에 진행"이라고 못박혀
  있었는데, 이번 바퀴로 `#92`가 끝났으니 다음 QA 하네스 세션은 `#93`을 진행하면
  된다.** **BUILD/DESIGN 완료 카운터는 이번 QA 바퀴로 변하지 않는다(그대로 0)** —
  `#94`/`#95` 완료 시 다시 카운트가 올라간다. 이번 바퀴가 새로 만든 검사 스크립트는
  없다(기존 `mining_variety_check.gd`/`deer_meat_check.gd`/`inbox86_check.gd`/
  `full_sweep.gd` 4개를 그대로 재사용).
