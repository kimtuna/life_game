...
**다음에 할 것**: **다음 BUILD 세션은 INBOX #106**(가공대 부재료 5종 — 못/경첩/
톱니바퀴/구리선/유리병, 재료 항상 1종만)을 처리한다. 참고할 것: (1) `#106`의
톱니바퀴(강철 2개→1개)/구리선(구리 1개→2개)/유리병(유리 1개→1개)은 이번 `#105`가
만든 강철/구리/유리에 의존하는데 `#105`가 이제 완료됐으니 바로 진행 가능. (2)
`processing_table.gd`의 `RECIPES`만 확장하면 되고 새 오브젝트/UI는 필요 없다(이번
`#105`와 같은 패턴). (3) 이어서 `#107`([DESIGN], 이번 `#105`가 만든 강철/유리/구리 +
`#106`의 5개 부재료 총 8개 아이콘을 절차적 생성으로 한 번에 제작, `#105`/`#106` 완료
후 진행 — 원문 그대로). (4) `#108`(강철 도구 3종)은 `#105`(강철) 완료 후 진행 가능,
`#109`(강철 건축+갑옷)와 `#110`(전기 계열+사료/급수)은 `#105`/`#106` 둘 다 완료된
후 진행. (5) `#113`([QA] 전체 스윕)은 이미 큐에 있고 처리 순서상 `#106`~`#111`보다
뒤에 온다(번호 순서대로 처리되므로 자연스럽게 대기). (6) BUILD/DESIGN 완료 카운터는
이번(#105)으로 **2**(5 도달까지 3개 남음).
