...
- **(2026-09-04 갱신, 바퀴 140) INBOX #87([BUILD] 가공대) 완료.** 위 "마지막 갱신"
  참고. **다음 `[BUILD]` 항목은 `#88`**(제련로 — 가공대와 같은 프레임워크
  재사용, `world.gd`의 `open_crafting_window()`/`RECIPES` 패턴을 그대로 새 씬
  `scenes/smelting_furnace/`에 적용하면 된다). **다음 `[QA]` 항목은 `#92`**(전체
  스윕, 바퀴 139에서 이미 큐에 등록돼 대기 중 — 별도 QA 하네스 세션 몫이라 이번
  바퀴는 건드리지 않았다). **BUILD/DESIGN 완료 카운터는 바퀴 139(#86)에서 5에
  도달해 0으로 리셋된 뒤 이번(#87)으로 1이 됐다** — 다음 QA 스윕 자동 등록까지 4개
  더 필요. `[DESIGN]` 큐 `#91`(가공대/제련로/조리대/조리용 화로 4개 오브젝트 그림 +
  가공물/완성품/가공식품 아이콘 일괄 제작)은 여전히 `#88`~`#90`이 먼저 끝나야
  의미가 있어 대기. **참고**: 가공대의 월드 그림은 아직 임시 갈색 사각형
  (`Polygon2D`)이고, 판자/석재 아이템도 아직 전용 아이콘이 없어 핫바/인벤토리에
  텍스트로만 표시된다 — 둘 다 결함이 아니라 `#91`이 처리할 범위(INBOX 원문이
  명시).
