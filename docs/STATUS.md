...
- INBOX #18 완료: 사슴이 월드 경계(특히 구석)로 몰리면 도주 상태인데도 멈춰버리던 버그 수정.
  - 원인: `deer.gd`의 도주 로직이 매 프레임 "플레이어 반대 방향"(`away`)만 계산해서
    `_move()`에 넘겼는데, `_move()`는 이동 후 `position`을 `WORLD_BOUNDS`로 clamp만
    할 뿐 `away` 방향 자체는 절대 바꾸지 않았다. 플레이어가 사슴을 구석으로 몰면
    `away`가 두 축 모두 경계 바깥을 향하게 되고, clamp가 매 프레임 원래 자리로 되돌려
    실제 이동량이 0이 되는 상태가 계속됐다.
  - 수정: `deer.gd`에 `_wall_slide_direction()`을 추가해서, 도주 방향 중 "이미 경계
    근처(`FLEE_WALL_MARGIN`=80 이내)에 있으면서 그 경계 쪽으로 파고드는 성분"을 축
    단위로 0으로 눌러(벽을 따라 미끄러지듯) 실제로 이동 가능한 방향으로 바꾼다. 두 축
    다 막힌 구석(코너)에서는 원래 방향에 수직인 방향(벽을 타고 옆으로)으로 우회한다.
    `flee` 상태 분기에서 `away`를 `_move()`에 넘기기 직전에 이 함수를 거치도록 한 줄만
    추가했다.
  - 시행착오: 처음엔 "이 프레임에 실제로 움직일 수 있는가"를 미리 계산해서 막혔으면
    수직/반대 방향 후보를 고르는 방식(`_effective_move_length` 기반)으로 짰는데,
    검증 스크립트로 확인해보니 경계 바로 앞에서 매 프레임 "막힘→탈출 방향" 판정이
    뒤집히며 제자리에서 미세하게 진동만 하고 실제로는 거의 전진하지 못했다(2초에
    2.4유닛). 축 성분을 아예 0으로 눌러 여러 프레임에 걸쳐 안정적으로 미끄러지는
    현재 방식으로 바꿔서 해결했다.
  - 검증: `godot --headless --path . --script <임시스크립트>.gd`로 사슴을 (1) 월드
    구석 정확히 그 지점(3800,3800)에 두고 바로 옆(50,50 안쪽)에 정지한 플레이어를 붙여
    도주를 강제 발동시킨 경우, (2) 사슴을 한쪽 변(경계선, 코너 아님)에 두고 같은 방식
    강제 발동시킨 경우, 두 경우 모두 물리 프레임을 실제로 진행시키며(엔진이 자동으로
    `_physics_process`를 호출하도록 두고 위치만 관찰 — 처음엔 직접 `_physics_process`를
    수동 호출해서 엔진의 자동 호출과 이중으로 겹쳐 측정값이 왜곡되는 실수를 했다가
    바로잡았다) 프레임 단위 이동 거리를 누적 기록했다. 결과: 두 경우 모두 처음
    ~2초 동안 거의 매 프레임 꾸준히 이동해 600유닛 이상 실제 이동(순변위=누적경로
    길이, 즉 한 방향으로 곧게 탈출)했고, 3초(도주 최소 지속시간)가 지나 플레이어와
    충분히 멀어진 뒤 정상적으로 `idle`로 돌아가 멈추는 것까지 확인했다(이건 버그가
    아니라 기존 도주 종료 조건이 의도대로 동작한 것). `--quit-after 3` 헤드리스
    스모크 테스트로 파싱 에러 없음도 확인. 임시 스크립트(`_tmp_verify18*.gd`)는
    커밋 전 삭제했다.
  - 변경 파일은 `game/scenes/deer/deer.gd` 하나뿐이다(23줄 추가, 기존 로직 삭제 없음).
    세션 시작 전부터 미커밋 상태였던 `game/project.godot`(아래 "막힌 것/보류" 참고)와
    untracked `.uid` 파일들은 이번 커밋에 포함하지 않았다.

## 다음에 할 것

- `docs/feedback/INBOX.md` "처리 대기"에 #19~#20이 미완료로 남아있다. 번호가 가장 작은
  **#19(사슴 체력바 UI)** 가 다음 바퀴가 처리할 항목이다. 사슴 위에 체력바(또는 숫자)를
  표시하면 되는 비교적 단순한 UI 작업 — `deer.gd`의 `health`/`MAX_HEALTH`를 참고해서
  `Deer` 씬에 체력바 노드(예: `ProgressBar` 또는 `TextureProgressBar`, 또는 간단한
  `ColorRect` 두 개로 배경/전경)를 추가하고 `take_hit()`에서 갱신하면 될 것으로 보인다.
  포획 가능 임계값(체력 10%)을 시각적으로 강조하면(예: 10% 밑에서 색이 바뀜) 지시문의
  "포획 타이밍을 알기 어렵다"는 문제를 더 잘 풀 수 있다 — 다만 이건 지시문이 요구하지
  않은 추가 UX이니 시간이 되면 고려.
- #20(전체 그래픽 통일성 — 바닥 텍스처 등, PixelLab 사용)도 대기 중이다. #19보다 번호가
  크므로 #19를 먼저 처리한 뒤에 순서대로 처리할 것.
