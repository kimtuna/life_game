...
## 끝난 것 (바퀴 14, 커밋은 바퀴 15가 처리)

- INBOX #12 완료: 목장을 새로 만들었다.
  - `deer.gd`에 `is_ranched`/`zone_center`/`zone_radius` 상태를 추가했다. 포획(`_capture()`)
    시 사슴이 그냥 사라지는 대신 `InventoryData.add_item("captured_deer", 1)`로 "재고"에
    쌓인다(밭의 씨앗/수확물과 같은 패턴 — 포획된 사슴을 즉시 필드에 별도 오브젝트로 남기지
    않고 인벤토리 카운트로 관리). `is_ranched`가 true인 사슴은 플레이어 접근으로 도주하지
    않고(`_physics_process`의 flee 트리거를 건너뜀), 이동 범위가 `WORLD_BOUNDS` 사각형 대신
    `zone_center` 기준 `zone_radius` 원형으로 제한되며, `take_hit`도 무시한다(길들여진 가축은
    다시 사냥/포획 대상이 되지 않음 — 아래 결정 로그 참고).
  - `scenes/ranch_zone/ranch_zone.gd` + `.tscn`: 목장 구역 새 씬. farm_plot과 같은 F키
    상호작용 패턴(플레이어가 반경 안에 들어오고 `captured_deer` 보유량이 있으면 프롬프트
    표시)을 쓰되, "심기/수확" 대신 "F: 사슴 풀어놓기" 한 동작만 있다. F를 누르면
    `captured_deer`를 하나 소비하고, `deer.tscn`을 구역의 자식으로 인스턴스화해
    `is_ranched=true`, `zone_center=Vector2.ZERO`(구역 로컬 좌표 기준), `zone_radius=100`을
    채워서 풀어놓는다. 여러 번 풀어놓으면 구역 안에 사슴이 계속 쌓인다(개체 수 제한 없음).
  - `world.gd`에 `_spawn_ranch_zone()`을 추가해 밭과 같은 방식(스폰 지점 기준 고정 오프셋)
    으로 목장 구역 하나를 배치했다. HUD 인벤토리 `ITEM_LABELS`에 `captured_deer: "포획된 사슴"`
    을 추가해 재고가 보이게 했다.
  - 그림은 PixelLab로 1장 생성했다: `pasture.png`(64x64, 배경 있음) — 갈색 흙바닥에 풀숲이
    둘러싼 목재+가시덤불 울타리가 이미 타일 안에 통째로 그려져 나와서, 여러 장을 이어붙이는
    타일링 대신 이미지 1장을 4배로 확대해 구역 전체를 표현하는 "울타리 펜" 단일 스프라이트로
    채택했다(farm의 soil.png처럼 반복 타일링하면 각 칸마다 울타리가 따로 생겨 이상해 보임).
  - 검증: `godot --headless --path . --script <임시스크립트>.gd`로 (1) 사슴 포획 시
    `captured_deer`가 1 늘고 필드 사슴이 `queue_free` 예약되는지, (2) 목장 구역이
    `captured_deer` 보유량에 따라 프롬프트를 켜고 끄는지, F 입력 시 재고가 소비되고
    `is_ranched=true`인 사슴이 구역에 스폰되는지, (3) 스폰된 사슴이 190 물리 프레임 동안
    `zone_radius`를 벗어나지 않고 한 번도 `flee` 상태에 들어가지 않는지, 전 과정을 `assert`로
    통과시켰다. 이어서 `godot --path . --script <임시스크립트>.gd`(헤드리스 아님)로
    world.tscn을 띄워 목장 구역과 그 안에 방생된 사슴을 실제 렌더링 PNG로 캡처해 눈으로
    확인했다. 임시 스크립트와 캡처 이미지는 게임 코드가 아니므로 커밋 전에 삭제했다.

## 스스로 판단해서 고친 부분 (바퀴 14 — 기존 버그, INBOX #12엔 없던 내용)

- 목장 구역을 스크린샷으로 확인하려는데 `Pasture` 스프라이트가 안 보였다(초록 배경만
  보임). 원인은 `world.tscn`의 `Ground`(Polygon2D, 배경 전체를 덮는 초록 사각형)가
  `z_index` 기본값 0인데, `farm_plot.tscn`의 `Soil`과 이번에 만든 `Pasture`는(바퀴 13이
  "플레이어보다 아래로" 고치면서 준) `z_index=-1`이라서 Ground(0)가 이 스프라이트들(-1)
  보다 나중에 그려져 전부 가려지고 있었다 — **밭 타일도 다시 스크린샷을 찍어보니 똑같이
  안 보이는 기존 버그였다**(바퀴 13 스크린샷에선 우연히 드러나지 않았던 것으로 보임).
  `world.tscn`의 `Ground`에 `z_index = -2`를 줘서 Ground(-2) < Soil/Pasture(-1) <
  Player(기본값 0) 순서로 그려지도록 고쳤다. 내가 만든 목장이 안 보이는 채로는 ①의
  합격 기준을 통과할 수 없고, 같은 원인으로 밭도 이미 안 보이고 있었으므로 함께 고쳤다.
  다시 찍은 스크린샷으로 밭과 목장 둘 다 정상적으로 보이는 것을 확인했다.

## 다음에 할 것

- `docs/feedback/INBOX.md` "처리 대기"의 다음 항목은 **#13(낮/밤·계절·날씨)**이다.
  하루 20분(낮/밤 10분씩) 타이머와 계절(1·2·3월 봄 ... 10·11·12월 겨울), 강수 확률
  공식(계절 기본값 + 마지막 비 이후 일수×1%p, 비 오면 리셋)을 구현해야 한다. 비주얼은
  최소한(화면 밝기 변화, 비 표시 정도)으로 충분하다고 지시돼 있다.
- 목장(ranch_zone)은 개체 수 제한이나 "목장 안 사슴을 다시 꺼내는" 기능이 없다 —
  한 번 풀어놓으면 영구히 그 구역에 머문다. INBOX #12 지시문이 "기를 수 있게 한다"까지만
  요구했고 번식/판매/수확 같은 후속 기능은 DESIGN.md에 없어서 임의로 만들지 않았다.
  다음 지시가 오면 그때 추가할 것.
...
- 성장 시간은 60초로 정했다(임의 — DESIGN.md 미정). 낮/밤 시스템(#13)이 구현되면
  성장 시간을 게임 내 날짜 기준으로 바꿀지 재검토할 것(지금은 실시간 초 단위).
- **새 배경/바닥용 스프라이트를 world.tscn에 추가할 때는 `Ground`(z_index=-2)보다는
  위, `Player`(기본값 0)보다는 아래인 `z_index=-1`을 기본으로 줄 것** — 이번 바퀴에
  Ground z_index를 고쳐서 이 관례가 성립하게 만들어뒀다.
