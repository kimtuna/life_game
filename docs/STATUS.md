...
**다음에 할 것**: 다음 미완료 항목은 `#131`([QA] 전체 스윕, `loop/PROMPT_QA.md`
하네스가 처리 — 이번 `#130`을 포함한 `#125`~`#130` 구간을 특히 확인할 것: 건설
해제 모드가 벽/문뿐 아니라 상자/제작대류/밭/목장까지 정상 동작하는지, 방 카테고리
재계산이 실제 플레이에서도 잘 되는지). BUILD/DESIGN 완료 카운터는 1(전체 스윕까지
4개 남음, `#131` 처리 후에도 계속 세어나감).
