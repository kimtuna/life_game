...
- INBOX #16 완료: 스프라이트 크기 전반 축소.
  - 7곳의 Sprite scale을 정확히 절반으로 줄였다: 플레이어(`world.tscn`의 `Player` 노드)
    3→1.5, 사슴(`deer.tscn`) 3→1.5, 목장(`ranch_zone.tscn`의 `Pasture`) 4→2, 채집/채광
    포인트(`gathering_point.tscn`/`mining_point.tscn`) 2.2→1.1, 밭(`farm_plot.tscn`의
    `Soil`) 1.8→0.9, 밭의 `Crop`(작물, 심기 전엔 안 보임) 1.6→0.8.
  - **지시받지 않았지만 추가로 고친 부분**: `world.gd:364`의 `_ensure_remote_sprite()`가
    멀티플레이 다른 접속자용 스프라이트에 별도로 `Vector2(3, 3)`을 하드코딩하고 있었다 —
    INBOX #16 원문은 이 코드를 언급하지 않았지만, 로컬 플레이어만 줄이고 이걸 그대로
    두면 "내 캐릭터와 다른 접속자 캐릭터 크기가 다르게 보이는" 새 버그가 생기므로
    같이 1.5로 줄였다.
  - 검증: (1) `godot --headless --path . --quit-after 3`로 씬 파싱 에러 없음 확인.
    (2) `world.tscn`을 실제로 띄워 스크린샷 — 플레이어가 눈에 띄게 작아져 화면 대비
    적당한 비율이 됨을 확인. (3) 사슴/채집포인트/채광포인트/밭/목장을 한 화면에 나란히
    배치하는 임시 검증 씬으로 스크린샷을 찍어 서로 비율이 어색하지 않은지 확인 —
    목장(구역)만 다른 소품보다 크게 보이는 건 의도된 것(개별 오브젝트가 아니라
    "구역"이라서)이라 정상 판단했다. 임시 스크립트(`_tmp_shot.gd`, `_tmp_shot2.gd`)와
    캡처 이미지는 커밋 전에 삭제했다.
  - 값을 추가로 미세조정하지는 않았다 — 지시받은 정확히 절반 값 그대로 적용했고,
    스크린샷상 어색한 지점을 발견하지 못했다.
  - 사슴의 `Hurtbox`(CircleShape2D radius=34) 등 충돌/상호작용 반경 상수는 건드리지
    않았다 — INBOX #16이 명시적으로 "Sprite scale"만 지목했고, 이 반경들은 Sprite의
    자식이 아니라 Deer/포인트 자체의 자식이라 스프라이트 scale에 자동으로 영향받지
    않는다(하드코딩된 별도 값). 스크린샷상 명백히 어긋나 보이지도 않아 범위를 넘겨
    손대지 않았다 — 실제 플레이 중 "빈 공간에서 맞는" 느낌이 들면 다음 바퀴가 반경도
    같이 줄이는 걸 검토할 것.

## 다음에 할 것

- `docs/feedback/INBOX.md` "처리 대기"에 #17~#20이 미완료로 남아있다. 번호가 가장 작은
  **#17(밭 위 플레이어 렌더 순서)** 이 다음 바퀴가 처리할 항목이다 — 아래 "새 배경/바닥용
  스프라이트..." 규칙과 바퀴 13/14 z_index 결정 로그를 먼저 확인하고 시작할 것(기존
  z_index 순서를 깨지 않도록).
- #18(사슴이 경계에서 도주하다 멈춤), #19(사슴 체력바 UI), #20(전체 그래픽 통일성 —
  바닥 텍스처 등)도 대기 중이다.
- (신규, 바퀴 19 발견) `game/project.godot`에서 `window/stretch/aspect="keep"` 한 줄이
  이번 바퀴 시작 전부터 이미 삭제된 채 미커밋 상태로 남아있다 — 아래 "막힌 것/보류"
  참고. 다음 바퀴가 `project.godot`를 건드리는 작업을 하게 되면 먼저 이 상태가 여전한지
  확인할 것.
