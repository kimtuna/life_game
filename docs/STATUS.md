...
바퀴 139 / 2026-09-04 ([DESIGN] **INBOX #86 완료 — 돌/유황광석/고기 아이템 아이콘
3종과 돌/유황광석 채광 포인트 월드 그림 2종을 파이썬(Pillow) 절차적 생성으로 만듦,
AI 생성 크레딧 불필요. BUILD/DESIGN 완료 카운터: 바퀴 134(#82, QA)에서 0으로 리셋된
뒤 135(#81)=1 → 136(#83)=2 → 137(#84)=3 → 138(#85)=4 → 이번(#86)=**5**, QA 스윕
`#92` 큐에 추가함.**
1) **생성 방식**(`/tmp/gen_ore_meat.py`, 커밋 안 함 — `tree.png`(#79)/캐릭터(#81)와 같은
   관례): Pillow만 사용, 4배 슈퍼샘플 후 `LANCZOS` 다운스케일 + `GaussianBlur(radius=2)`
   + 알파 40 임계값 재스냅(#79와 동일 기법). **텍스트/노이즈 함수 없이 순수 다각형/타원
   레이어링만 사용**(#79가 남긴 "글리프처럼 보이는 반복 패턴" 회피 원칙 그대로 적용).
   - **돌(stone)**: 8각 불규칙 다각형(`jitter_polygon`, irregularity 0.16~0.18) 4겹
     (외곽선 어두운 남색 → 그림자 → 중간톤 → 좌상단 하이라이트) + 무작위 절단선(facet)
     2~3개로 각진 느낌을 강조. `iron_ore.png`(기존 원재료 아이콘)와 팔레트가 비슷해서
     스타일이 어긋나지 않는다.
   - **유황광석(sulfur_ore)**: 어두운 회색 다각형 받침(암석 베이스) 위에 노란 결정
     5개(뾰족한 사각 다이아몬드, 각도를 부채꼴로 분산)를 얹고 각 결정마다 외곽선(짙은
     갈색)→몸통(진노랑)→하이라이트(연노랑) 3겹으로 음영. 결과물이 "왕관"처럼 보이지만
     실제 게임 화면(64px 이하)에서는 "돌 위에 솟은 노란 결정 다발"로 충분히 읽힌다.
   - **고기(meat)**: 처음엔 둥근 블롭으로 만들었다가 돌/철광석과 실루엣이 너무 비슷해서
     사용자 판단 없이 스스로 조정함(아래 "스스로 판단해서 고친 부분" 참고) — 가로로
     늘인 쐐기형(`stretch()`로 x축 1.28배/y축 0.86배 비등방 스케일) + 대각선 지방
     마블링 줄무늬(연분홍) 4개 + 한쪽 끝에 뼈 조각(크림색, 어두운 외곽선)을 추가해서
     돌/철광석류와 구분되는 실루엣을 만들었다.
   - 아이템 아이콘 3종은 32×32(기존 `iron_ore.png`/`rice.png`와 동일 규격), 채광 포인트
     월드 그림 2종은 64×64(기존 `mining_point.png`와 동일 규격, `mining_point_stone.tscn`/
     `mining_point_sulfur.tscn`이 이미 쓰던 `scale=Vector2(1.1,1.1)`을 그대로 유지해서
     화면 크기 변화 없음).
2) **코드 연결**: `dropped_item.gd`의 `ITEM_ICONS`에 `stone`/`sulfur_ore`를 새 전용
   PNG로 교체하고 `meat`를 새로 추가했다. 기존에 `stone`/`sulfur_ore`를 `iron_ore.png`
   재사용 + `ITEM_ICON_MODULATE`(회색/노랑 틴트)로 임시 처리하던 것(#84가 남긴 부채)을
   정리했다 — 새 아이콘이 이미 올바른 색을 갖고 있어서 `ITEM_ICON_MODULATE` 상수 자체를
   삭제했다(더 이상 아무 아이템도 안 씀). `mining_point_stone.tscn`/`mining_point_sulfur
   .tscn`의 `texture_path`를 각각 전용 PNG로 바꾸고, 같은 이유로 `Sprite.modulate` 줄도
   제거했다.
3) **헤드리스 임포트**: PNG 5장을 프로젝트 폴더에 저장한 직후 `godot --headless --import`
   를 실행해 `.import` 메타파일을 생성시켰다(바퀴 135 노트가 남긴 필수 절차 — 안 하면
   `load()`가 조용히 실패한다).
4) **실제 게임 렌더링으로 검증(⑥의 필수 절차)**: `game/qa/inbox86_check.gd`를 새로
   만들어(다른 qa 스크립트와 같은 패턴) `project.godot` `[autoload]`에 임시로 넣고
   `godot --path .`로 실제 실행 → 메인 메뉴 → 캐릭터 슬롯 0 → 싱글플레이 진입까지 실제
   화면 전환을 그대로 탄 뒤, 플레이어 좌우에 돌/유황광석 채광 포인트를 스폰하고 아래쪽에
   돌/유황광석/고기 드롭 아이템 3개를 스폰해서 광각 1장(`/tmp/qa86/wide.png`) + 카메라
   zoom 3배 근접 1장(`/tmp/qa86/close.png`)을 캡처했다. Read 도구로 두 장 다 직접 확인:
   (a) 돌(회색 각진 덩어리)과 유황광석(노란 결정 다발)이 나무·플레이어와 비교해도 화면에서
   충분히 크고(플레이어 키와 비슷하거나 그 이상, 최소 32px 기준 여유 있게 충족) 서로
   실루엣/색이 뚜렷이 구별된다, (b) 3배 확대해도 이음매/글리프 아티팩트 없음, (c) 하단
   드롭 아이템 3개(작은 회색 돌/노란 결정/붉은 고기 쐐기)가 32px 크기에서도 서로 색·형태로
   구별된다. 합격 판정. **다음 바퀴가 주의할 점**: 이 스크린샷에서 카메라를 3배 줌한
   상태로 찍었더니 월드 좌표의 드롭 아이템(화면 하단 근처에 스폰함)이 화면 고정 위치인
   하단 핫바 UI와 겹쳐 보이는 현상이 있었다 — 이건 그림 결함이 아니라 검증 스크립트가
   억지로 확대한 탓의 좌표 우연 — 실제 플레이(기본 줌 1배)에서는 발생하지 않는다.
5) **검증용 임시 변경 원상복구**: `project.godot`의 `QAInbox86Check` autoload 줄을
   되돌렸다(`git diff game/project.godot` 결과 없음으로 확인). `game/qa/inbox86_check.gd`
   자체는 `deer_meat_check.gd`(#85)/`mining_variety_check.gd`(#84)와 같은 관례로 삭제하지
   않고 커밋 대상에 포함했다(회귀 테스트 스크립트로 남겨둠).
6) `docs/feedback/INBOX.md`의 `#86`을 `[x]`로 갱신, BUILD/DESIGN 완료 카운터가 5에
   도달해 `#92`([QA] 전체 스윕)를 큐에 새로 등록했다.

## 스스로 판단해서 고친 부분 (INBOX #86)
...
- **(2026-09-04 갱신, 바퀴 139) INBOX #86([DESIGN] 돌/유황광석/고기 아이템 아이콘 +
  돌/유황광석 채광 포인트 월드 그림) 완료.** 위 "마지막 갱신" 참고. **다음 `[BUILD]`
  항목은 `#87`**(가공대). **다음 `[QA]` 항목은 `#92`**(전체 스윕 — BUILD/DESIGN 완료
  카운터가 5에 도달해 이번 바퀴가 자동 등록함). `[DESIGN]` 큐는 `#91`(대기 중이지만
  `#87`~`#90`(가공대/제련로/화약탄약/조리 라인, 전부 `[BUILD]`)이 먼저 끝나야 순서상
  의미가 있는 그림 작업이라 실질적으로는 `#87`~`#90` 완료 후에 처리됨). **참고**:
  `meat`/`stone`/`sulfur_ore` 아이템 아이콘과 돌/유황광석 채광 포인트 월드 그림이 모두
  전용 PNG로 교체됐다 — `#84`가 남겼던 `iron_ore.png` 재사용 + `ITEM_ICON_MODULATE`
  임시 틴트 처리를 정리하고 삭제했다. 새 PNG를 저장한 뒤에는 반드시
  `cd game && godot --headless --import`를 먼저 실행할 것 — 안 하면 `.import`
  메타파일이 없어서(또는 리네임 시 캐시가 옛 파일명을 가리켜서) `load()`가 조용히
  실패하고 스프라이트가 화면에 아예 안 보이는데, 에러 로그가 무관한 메시지와 섞여
  있어 원인을 코드 버그로 오인하기 쉽다(바퀴 135/136/139가 각각 겪거나 미리 대비함,
  아래 "헤드리스 CLI 검증" 절 참고).
